...
This skill is repo-agnostic. Everything specific to a source repository — what
is in scope, how files map to pages, what a new page looks like, where it gets
registered — comes from that repo's **profile**, `SOURCE_DOC_MAPPING.md`. The
skill never hardcodes a source path, a page template, or a navigation group.
