...
client.startBotAndConnect(
    APIRequest(
        endpoint = "https://your-server.com/api/start",
        requestData = Value.Object(mapOf(
            // Optional data forwarded to your server
            "initialPrompt" to Value.Str("You are a helpful assistant.")
        ))
    )
).withCallback { result ->
...
// Completes when the bot reaches 'ready' state
```
...
With SmallWebRTC, pass the endpoint URL in `SmallWebRTCTransportConnectParams`:
...
client.connect(SmallWebRTCTransportConnectParams(
...
let connectionParams = await client.startBot(startBotParams: startParams)
// inspect or modify connectionParams here
await client.connect(connectionParams)
```
...
```kotlin
client.startBot(
    APIRequest(endpoint = "https://your-server.com/api/start", requestData = Value.Object())
).withCallback { result ->
    val connectionParams = result.valueOrNull ?: return@withCallback
    // inspect connectionParams here
...
function MyComponent() {
  useRTVIClientEvent(
    RTVIEvent.Error,
    useCallback((message) => {
      const { error: text, fatal } = message.data;
...
client.startBotAndConnect(APIRequest(endpoint = "https://your-server.com/api/start", requestData = Value.Object()))
    .withCallback { result ->
        result.errorOrNull?.let { error ->
            Log.e("Bot", "Connection failed: ${error.description}")
        }
