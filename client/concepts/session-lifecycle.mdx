...
With SmallWebRTC, pass the endpoint URL directly to the transport factory and call `connect()`:

```kotlin
val client = RTVIClient(
    SmallWebRTCTransport.Factory(context, "/api/offer"),
    callbacks,
    options
)
client.connect()
...
Override `onBotReady` in your `RTVIEventCallbacks`:
...
Override `onBotDisconnected` in your `RTVIEventCallbacks`. Call `release()` in `onDisconnected` to free resources:
...
Override `onBackendError` in your `RTVIEventCallbacks`. For startup errors, handle the `Future` result:
...
class VoiceClientManager {
    ...
    private val callbacks = object : RTVIEventCallbacks() {
