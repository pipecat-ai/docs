...
Enable the camera via `PipecatClientOptions` and toggle it with `enableCam()`:

```kotlin
val options = PipecatClientOptions(callbacks = callbacks, enableCam = true)
...
Audio routing (speakerphone vs. earpiece) is controlled through the mic device API — like on iOS, input and output are linked, so switching the "mic" switches the audio route. The SmallWebRTC, Gemini, and OpenAI transports expose static constants for the two routes:
...
The same `AudioDevices` constants are available on `GeminiLiveWebsocketTransport` and `OpenAIRealtimeWebRTCTransport`. With the Daily transport, enumerate the available audio routes (including Bluetooth and wired headsets) with `getAllMics()` and pass the chosen device id to `updateMic()`.
...
function AudioViz() {
  ...
  return (
    <View style={{ width: 200, height: 8, backgroundColor: "#eee" }}>
      <View
        style={{ width: 200 * level, height: 8, backgroundColor: "#3b82f6" }}
      />
    </View>
  );
}
...
<CardGroup cols={2}>
  <Card
    title="Client Methods — Devices"
    icon="js"
    href="/api-reference/client/js/client-methods#devices"
  >
  ...
  <Card
    title="React Hooks"
    icon="react"
    href="/api-reference/client/react/hooks"
  >
    `usePipecatClientMediaDevices`, `usePipecatClientMicControl`,
    `usePipecatClientCamControl`, `usePipecatClientMediaTrack`
  </Card>
  <Card
    title="React Components"
    icon="puzzle-piece"
    href="/api-reference/client/react/components"
  >
    `PipecatClientAudio`, `PipecatClientVideo`, `PipecatClientMicToggle`,
    `PipecatClientCamToggle`, `VoiceVisualizer`
  </Card>
</CardGroup>
...
<CardGroup cols={1}>
  ...
  >
    Full API reference including `PipecatEventCallbacks` and device management
  </Card>
</CardGroup>
