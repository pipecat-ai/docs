...
Android uses `RTVIEventCallbacks` rather than a callbacks map — see [Event listeners](#event-listeners) below.
...
Extend `RTVIEventCallbacks` and pass it to the `RTVIClient` constructor. Override only the methods you need:
...
val options = RTVIClientOptions(params = RTVIClientParams(baseUrl = null), enableMic = true)
...
Override `onLLMFunctionCall` in your `RTVIEventCallbacks` to handle function calls. Invoke `onResult` with the function's return value:

```kotlin
val callbacks = object : RTVIEventCallbacks() {
    override fun onLLMFunctionCall(data: LLMFunctionCallData, onResult: (Value) -> Unit) {
        if (data.functionName == "get_weather") {
            val city = (data.args as? Value.Object)?.value?.get("city")
                ?.let { (it as? Value.Str)?.value } ?: ""
...
For multiple functions, use `registerFunctionCallHandler` to register per-function handlers:
...
<CardGroup cols={1}>
  <Card title="Android SDK Reference" icon="android" href="/api-reference/client/android/overview">
    Full `RTVIEventCallbacks` class and API reference
  </Card>
</CardGroup>
