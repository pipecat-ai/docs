...
The Pipecat client emits events throughout the session lifecycle — when the bot connects, when the user speaks, when a transcript arrives, and more. You can respond to these in two equivalent ways: **callbacks** defined at construction time, or **event listeners** added at any point.

## Two patterns, same events
...
Add handlers with `.on()` at any point — useful for component-level subscriptions or when you want to add/remove handlers dynamically:
...
Both patterns are equivalent. Mix them freely — there's no functional difference between a callback and its corresponding event listener.

### React: `useRTVIClientEvent`

In React, use the `useRTVIClientEvent` hook to subscribe within a component. It handles registration and cleanup automatically:
