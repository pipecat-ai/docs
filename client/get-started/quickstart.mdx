---
title: "Pipecat Client Quickstart"
sidebarTitle: "Quickstart"
description: "Get a Pipecat client voice app running in minutes with the CLI, with React, JavaScript, and mobile SDK variants."
---
...
<Note>
  ...
  UI](/client/guides/building-a-voice-ui) to get started without the CLI.
</Note>
...
- `client/` — a React front-end with voice UI components

<Note>
  The CLI's React template currently uses the legacy Voice UI Kit npm package.
  [Pipecat UI](/client/pipecat-ui) replaces it with components installed as
  source through the shadcn CLI. The walkthrough below describes the existing
  template; see Pipecat UI for new UI development.
</Note>
...
The generated React app uses the legacy Voice UI Kit API. These components differ from the new [Pipecat UI](/client/pipecat-ui) registry. Here's what each file does:
...
**`src/components/App.tsx`** — the UI. Built from legacy Voice UI Kit components:
...
<CardGroup cols={2}>
  <Card title="Pipecat UI" icon="palette" href="/client/pipecat-ui">
    Build your UI with customizable React components from the shadcn registry
  </Card>
  <Card
