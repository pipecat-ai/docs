...
These messages let a server-side agent observe and drive a GUI on the client. The client streams the screen as accessibility snapshots and dispatches user events; the agent acts on the page with commands and surfaces long-running work as job-group cards. On the server, [`UIWorker`](/api-reference/server/workers/ui-worker) builds higher-level abstractions on top of these messages, but single-LLM apps can target the wire format directly. The matching client implementation lives in `@pipecat-ai/client-js` and `@pipecat-ai/client-react`.
