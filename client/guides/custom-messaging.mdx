...
client.startBotAndConnect(
    APIRequest(
        ...
        requestData = Value.Object(
            mapOf(
                "initial_prompt" to Value.Str("You are a pirate captain"),
                "preferred_language" to Value.Str("en-US"),
            )
        )
    )
)
...
client.sendClientMessage(
    "set-language",
    Value.Object(mapOf("language" to Value.Str("en-US")))
)
```
...
```kotlin
client.sendClientRequest("get-language", Value.Object())
    .withCallback { result ->
        val response = result.valueOrNull
        val language = (response?.data as? Value.Object)
            ?.value?.get("language")
            ?.let { (it as? Value.Str)?.value }
        Log.d("Bot", "Current language: $language")
    }
```
...
Override `onServerMessage` in your `PipecatEventCallbacks` to receive unsolicited server messages:

```kotlin
override fun onServerMessage(data: Value) {
    val obj = data as? Value.Object ?: return
    val msg = (obj.value["msg"] as? Value.Str)?.value
    if (msg == "language-updated") {
        val language = (obj.value["language"] as? Value.Str)?.value
        Log.d("Bot", "Language updated to: $language")
    }
}
```
