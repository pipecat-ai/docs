...
pipecatClient.registerHelper("llm", llmHelper);

// Events
pipecatClient.on(LLMHelperEvent.LLMContextUpdate, (messages) => console.log(messages));
...
`pipecatClient.registerHelper(service, helper);`
...
```typescript
const llmHelper = pipecatClient.getHelper("llm") as LLMHelper;
```
...
```typescript
pipecatClient.unregisterHelper("llm");
```
...
// --- Events
// E.g. pipecatClient.on(MyHelperEvents.SomeEvent, (message) => { ... });
export enum MyHelperEvents {
SomeEvent = "some-event",
}

// --- Message types
// E.g. await pipecatClient.action({ service: "myService", action: MyHelperMessageType.SOME_MESSAGE, arguments: [...]});
