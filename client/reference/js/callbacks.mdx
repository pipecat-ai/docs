...
```typescript
pipecatClient.on("botReady", () => console.log("Bot ready via event"));
// or using the enum
pipecatClient.on(RTVIEvent.BotReady, () => console.log("Bot ready via event"));
```
...
<ParamField path="onDisconnected">
  ...
  `pipecatClient.disconnect()` or due to an error.
</ParamField>
...
<ParamField path="onAvailableMicsUpdated" type="mics:MediaDeviceInfo[]">
  ...
  becomes available or in response to `pipecatClient.initDevices()`.
</ParamField>
