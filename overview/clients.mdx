...
## How Clients Connect to Pipecat

```
┌──────────────────┐                    ┌──────────────────────┐
│  Client App      │     WebRTC /       │  Pipecat Pipeline    │
│                  │     WebSocket      │                      │
│  PipecatClient   │ ◄───────────────► │  Transport           │
│  + Transport     │     Audio/Video    │  STT → LLM → TTS    │
└──────────────────┘                    └──────────────────────┘
```

1. Your client app creates a `PipecatClient` with a transport (e.g., Daily WebRTC)
2. The client connects to your backend, which starts a Pipecat pipeline
3. Audio streams bidirectionally between client and server in real-time
