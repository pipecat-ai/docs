...
A typical bot is a single agent, but Pipecat is a multi-agent system: you can coordinate many agents that talk to each other over a shared message **bus**. The same `PipelineRunner` you use for one bot coordinates many agents, so any Pipecat app is multi-agent ready.
