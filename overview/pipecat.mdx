...
A typical bot is a single agent, but Pipecat is a multi-agent system: you can coordinate many agents that each run their own pipeline and talk to each other over a shared message **bus**. The same `PipelineRunner` you use for one bot coordinates many agents, so any Pipecat app is multi-agent ready.

- The **`PipelineRunner`** owns the bus and manages every agent's lifecycle and discovery.
- Each agent is a **worker** (`PipelineWorker`, `LLMWorker`) running its own pipeline.
- A **`BusBridgeProcessor`** sits where an LLM would go in the main agent's pipeline and routes frames to whichever agent is currently active.
- Agents transfer control through **handoff** (`activate_worker(..., deactivate_self=True)`) and dispatch background work as **jobs**.
...
<Card
  ...
  icon="users"
  href="/pipecat/learn/agents-and-runner"
>
