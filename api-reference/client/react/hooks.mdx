---
title: "Hooks"
"og:title": "Hooks - React SDK"
description: "React hooks from the Pipecat React SDK for client access, media device management, and bot event handling, like usePipecatClient."
---
...
<ParamField path="botOutputFilter" type="BotOutputFilter">
  Restrict which bot output reaches `messages`, as
  `{ spoken?: boolean; unspoken?: boolean }`. Use it to render only what the
  bot says aloud, or only the text it produces without speaking.
</ParamField>
...
<ParamField path="botOutputEvents" type="BotOutputEvent[]">
  The raw bot-output events behind `messages`, in arrival order. Useful for
  debugging or replaying how a response was assembled.
</ParamField>
...
<ParamField
  path="handler"
  type="(payload: T) => void | Promise<void>"
  required
/>
