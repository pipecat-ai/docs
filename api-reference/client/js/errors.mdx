---
title: "Errors"
"og:title": "Errors - JavaScript SDK"
description: "Error types thrown by the Pipecat JavaScript client: RTVIError and its subclasses, and how to catch and handle each."
---
...
Thrown by `startBot()`, `connect()` or `startBotAndConnect()` when the client is already started. Call `disconnect()` before starting again.

### MessageTooLargeError

Thrown when a message exceeds the transport's maximum size — 64 KiB by default. Most likely when sending images or a large context through `sendClientMessage()`.
