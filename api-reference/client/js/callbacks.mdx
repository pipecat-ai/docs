...
description: "Handle bot events and state changes in the Pipecat JavaScript client with PipecatClient callbacks and event listeners."
---
...
<ParamField path="onUnsupportedFeature" type="error:UnsupportedFeatureError">
  A feature was requested that this transport or bot doesn't support — enabling
  a camera on an audio-only transport, or sending DTMF to a bot below RTVI
  protocol 2.0.0. The error carries a `feature` naming what was unavailable.
  Without this callback such a request appears to succeed and simply does
  nothing.
</ParamField>

<ParamField path="onScreenShareError" type="errorMessage:string">
  Screen sharing failed to start — most often because the user dismissed the
  browser's picker.
</ParamField>
...
<ParamField path="onError" type="message:RTVIMessage">
  ...
  an `error` string that describes the error and a `fatal` boolean indicating if
  the error is unrecoverable and resulted in a bot disconnection. If `fatal` is
  true, the client will automatically disconnect. The `message` field is
...
<ParamField path="onUserLlmText" type="UserLLMTextData">
  The text the bot received as user input, as `{text}`. Useful when the user
  typed rather than spoke, or to see the transcript the LLM actually acted on.
</ParamField>
...
<ParamField path="onUIJobGroup" type="data:UIJobGroupData">
...
| `BotStarted`            | `onBotStarted`            | `unknown`        |
| `BotReady`              | `onBotReady`              | `BotReadyData`   |
| `BotConnected`          | `onBotConnected`          | `Participant`    |
...
| `UnsupportedFeature` | `onUnsupportedFeature` | `UnsupportedFeatureError` |
...
| `AvailableSpeakersUpdated` | `onAvailableSpeakersUpdated` | `MediaDeviceInfo[]`             |
| `ScreenTrackStarted`       | `onScreenTrackStarted`       | `MediaStreamTrack, Participant` |
| `ScreenTrackStopped`       | `onScreenTrackStopped`       | `MediaStreamTrack, Participant` |
| `ScreenShareError`         | `onScreenShareError`         | `string`                        |
...
| `UserLlmText`     | `onUserLlmText`     | `UserLLMTextData` |
