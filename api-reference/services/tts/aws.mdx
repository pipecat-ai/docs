...
`AWSTTSService` provides text-to-speech capabilities using Amazon's Polly service. It supports multiple voices, languages, and speech customization options through SSML.

## Installation

To use `AWSTTSService`, install the required dependencies:
...
tts_service = AWSTTSService(
    ...
    region="us-east-1",
    voice_id="Joanna",
    params=AWSTTSService.InputParams(
        engine="neural",
...
```mermaid
graph TD
    A[Text Input] --> B[AWSTTSService]
    B --> C[SSML Construction]
    C --> D[AWS Polly API]
    D --> E[Audio Processing]
    E --> F[TTSStartedFrame]
    E --> G[TTSAudioRawFrame]
    E --> H[TTSStoppedFrame]
    B --> I[ErrorFrame]
```
