---
title: "Types"
"og:title": "Types - Pipecat Flows"
description: "Type definitions and configuration schemas for Pipecat Flows: NodeConfig, task messages, and flow configuration structures."
---
...
It applies to the Python behind a tool, so it works the same in both forms of flow: decorate a handler a [flow config](/pipecat/flows/flow-configs) names, or a function listed directly in a `NodeConfig`.
...
Sentinel value used in [`ConsolidatedFunctionResult`](#consolidatedfunctionresult) to indicate that a function should finish without an immediate response or node transition.

```python
from pipecat.flows import NO_RESPONSE

async def handoff_to_router(flow_manager: FlowManager, reason: str):
    # Transfer control to another worker
    await flow_manager.worker.activate_worker("router", deactivate_self=True)
    # Don't respond - the other worker will handle the next turn
...
This is useful for multi-worker handoffs where control is passed to another worker that should respond next, or when a function needs to pause the conversation until an external event occurs.

For functions that transition to a new node, use `respond_immediately: False` in [`NodeConfig`](#nodeconfig) instead.

## TRANSITION_IN_YAML

Sentinel value returned in the next-node slot by a tool that a [flow config](/pipecat/flows/flow-configs) names, deferring the transition to the config.
...
The config's `transition_to` for that entry decides the next node — by name, or with a branch table keyed on a field of the result. An entry with no `transition_to` stays on the current node.

Only a flow built from a config accepts it. A node built in Python rejects it, since nothing there can decide the transition. In the other direction, a tool a config names must return `TRANSITION_IN_YAML` or [`NO_RESPONSE`](#no_response); returning a node name, a `NodeConfig`, or `None` raises [`FlowError`](/api-reference/pipecat-flows/exceptions#flowerror). See the [call-time contract](/api-reference/pipecat-flows/flow#call-time-contract).
...
<Warning>
  **Deprecated.** No replacement. `FlowResult` is no longer required or
  referenced by any handler type, and Pipecat's upstream function-call-result
  contract is `Any` — define your own `TypedDict` or return any
...
```python
ConsolidatedFunctionResult = tuple[Any, NodeConfig | None | NO_RESPONSE]
```
...
- Second element: The next node as a `NodeConfig`, `None` to stay on the current node and respond, or [`NO_RESPONSE`](#no_response) to finish without transitioning or responding
