...
Sentinel value used in [`ConsolidatedFunctionResult`](#consolidatedfunctionresult) by tools in a flow built from a `FlowConfig`. Return `(result, TRANSITION_IN_YAML)` to defer the transition decision to the flow config's `transition_to` entry for that tool.
...
async def select_pizza_order(flow_manager: FlowManager, size: str, pizza_type: str):
    ...
    return {"size": size, "type": pizza_type}, TRANSITION_IN_YAML
```

In a flow config YAML file:
...
Only flows built from a `FlowConfig` accept this sentinel. A node built in Python (by directly constructing `NodeConfig` dicts) rejects it, since nothing there can decide the transition — return a `NodeConfig` or `None` instead.

The config's `transition_to` for a tool may name a node directly or use a branch table to route on a field of the result. See the [FlowConfig](/api-reference/pipecat-flows/flow-config) reference.
...
```python
ConsolidatedFunctionResult = tuple[Any, NodeConfig | None | NO_RESPONSE | TRANSITION_IN_YAML]
```
...
- Second element: The next node as a `NodeConfig`, `None` to stay on the current node and respond, [`NO_RESPONSE`](#no_response) to finish without transitioning or responding, or [`TRANSITION_IN_YAML`](#transition_in_yaml) to defer the transition to a flow config
