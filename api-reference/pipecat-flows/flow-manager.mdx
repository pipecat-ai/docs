---
title: "FlowManager"
"og:title": "FlowManager - Pipecat Flows"
description: "FlowManager orchestrates Pipecat Flows conversations: node transitions, function registration, and flow state."
---

## Overview

`FlowManager` orchestrates conversation flows by managing state transitions, function registration, and message handling across different LLM providers. It drives both forms of flow: nodes built in Python, and nodes a [`Flow`](/api-reference/pipecat-flows/flow) produced from a [flow config](/api-reference/pipecat-flows/flow-config).

### Placeholder Rendering

On entering a node, the manager fills `{{ key }}` placeholders in the node's prompts from [`state`](#state), before anything is sent to the LLM. It renders the node's `role_message`, the `content` of each of its `task_messages`, and the `text` of any `tts_say` action.

Rendering happens on **every** entry, not once at load, so a prompt sees whatever handlers have stored by then — including after a context reset, and again each time a node is re-entered. `{{ order.size }}` walks into a stored mapping, values are rendered with `str()`, and `\{{ key }}` is left as the literal `{{ key }}`. A placeholder naming a key that is not in `state` raises [`FlowError`](/api-reference/pipecat-flows/exceptions#flowerror) on entry.
...
<ParamField
  ...
  default="ContextStrategyConfig(strategy=ContextStrategy.APPEND)"
>
...
With a flow config, pass
[`flow.global_functions`](/api-reference/pipecat-flows/flow#global_functions),
...
With a flow config, the starting node comes from the flow:
...
Set any state the config's prompts refer to as `{{ key }}` before calling `initialize()`, since the initial node is rendered as it is entered.
