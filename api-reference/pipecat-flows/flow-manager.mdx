...
A node's prompts may refer to `state` with `{{ key }}` placeholders. These are filled in each time the node is entered, so a value a handler stored earlier in the conversation can appear in a later prompt, including after a context reset.

Placeholders are rendered in:
- `role_message`
- `task_messages` content
- `tts_say` action text

Use dot notation to walk into stored mappings: `{{ order.size }}`. To show the LLM a literal `{{ key }}`, escape it as `\{{ key }}`.

```python
# A handler stores data
async def collect_party_size(flow_manager: FlowManager, size: int):
    flow_manager.state["party_size"] = size
    return {"size": size}, next_node

# A later node's prompt uses it
NodeConfig(
    task_messages=[
        {"role": "developer", "content": "Ask what time they'd like to dine for their party of {{ party_size }}."}
    ]
)
```
...
Values stored here can be referenced in prompts using `{{ key }}` placeholders. See [Prompt Placeholders](#prompt-placeholders).
...
# Reference in prompts: "Welcome back, {{ user_name }}!"
# Walk into mappings: "Confirm the {{ order.size }} pizza."
```
