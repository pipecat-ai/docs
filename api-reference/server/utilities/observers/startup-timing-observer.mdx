...
| `setup_phase_secs`    | `float`                        | How long getting every processor ready took. Setup runs concurrently                           |
...
| `warmup`              | `StartupWarmupTiming \| None`  | **Deprecated since 1.12.0.** Always `None`. Will be removed in 2.0.0.                          |
...
<Note>
  **Deprecated since 1.12.0.** The `StartupWarmupTiming` class and the `warmup`
  field in `StartupTimingReport` are deprecated and will be removed in 2.0.0.
  Nothing warms deferred imports at startup, so `warmup` is always `None` and
  these fields are never populated.
</Note>

| Field                    | Type    | Description                                                            |
| ------------------------ | ------- | ---------------------------------------------------------------------- |
| `duration_secs`          | `float` | **Deprecated since 1.12.0.** No longer used. Will be removed in 2.0.0. |
| `blocking_duration_secs` | `float` | **Deprecated since 1.12.0.** No longer used. Will be removed in 2.0.0. |
