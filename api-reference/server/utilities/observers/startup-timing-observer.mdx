...
| `warmup`              | `StartupWarmupTiming \| None`  | Always `None` (warmup was removed in 1.11.0)                                                |
...
<Note>
  Framework warmup was removed in 1.11.0. The `warmup` field in
  `StartupTimingReport` is always `None`, and these fields are no longer
  populated.
</Note>

| Field                    | Type    | Description                           |
| ------------------------ | ------- | ------------------------------------- |
| `duration_secs`          | `float` | No longer used (warmup removed)       |
| `blocking_duration_secs` | `float` | No longer used (warmup removed)       |
