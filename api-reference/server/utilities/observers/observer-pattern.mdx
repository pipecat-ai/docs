...
- `on_startup_warmup(data: StartupWarmup)`: Called when the framework has warmed its deferred imports
...
`StartupWarmup` carries `started_at_ns` / `finished_at_ns` for the framework's deferred import warming. Warming runs alongside processor setup, so what it costs a pipeline is the part of it that outlasts them.

```python
from pipecat.observers.base_observer import BaseObserver, FramePushed, FrameProcessed, StartupWarmup
...
class CustomObserver(BaseObserver):
    ...
    async def on_startup_warmup(self, data: StartupWarmup):
        # Called when deferred imports have been warmed
        pass
