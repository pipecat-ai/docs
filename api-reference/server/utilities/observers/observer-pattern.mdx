...
description: "The Pipecat observer pattern: watch frames flow through a pipeline without modifying them, and write your own observers."
---
...
A frame is pushed again by every processor that passes it along. By default, an observer receives every push. An observer that handles a frame once (such as one that reports the moment a frame represents) should set `observe_every_push=False` and will be told about a frame only on its first push.

The `FramePushed` event includes a `first_push` field that identifies whether this is the first time a frame is being pushed, allowing observers to distinguish the initial push from subsequent relays.
...
class CustomObserver(BaseObserver):
    def __init__(self):
        # observe_every_push=True by default
        super().__init__()

    async def on_push_frame(self, data: FramePushed):
        # data.first_push tells you if this is the first push
...
class EventObserver(BaseObserver):
    def __init__(self):
        # Only observe the first push of each frame
        super().__init__(observe_every_push=False)

    async def on_push_frame(self, data: FramePushed):
        # This is called once per frame, on its first push
...
- **[ServiceMetricsObserver](/api-reference/server/utilities/observers/service-metrics-observer)**: Reports service latency and usage metrics as structured records
