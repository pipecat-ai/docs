---
title: "LLMContextSummarizer"
sidebarTitle: "Context Summarization"
description: "Reference for LLMContextSummarizer, LLMAutoContextSummarizationConfig, LLMContextSummaryConfig, and SummaryAppliedEvent."
---
...
<ParamField path="max_context_tokens" type="int | None" default="8000">
  ...
  Tokens are estimated using the heuristic of 1 token per 4 characters. This
  measures the context messages only: the LLM service's `system_instruction` is
  sent on every inference but is not part of the context, and summarization
  cannot compress it. Size this limit against the model's window minus the
  instruction. Set to `None` to disable token-based triggering. At least one of
  `max_context_tokens` or `max_unsummarized_messages` must be set.
</ParamField>

<ParamField path="max_unsummarized_messages" type="int | None" default="20">
  Maximum number of conversation messages added since the last summary before
  triggering a new summarization, even if the token limit has not been reached.
  Counts only user and assistant messages — the leading system message (at
  `messages[0]` if present) and the summary message itself are excluded. The
  count resets after each summary is applied. Set to `None` to disable
  message-count triggering. At least one of `max_context_tokens` or
