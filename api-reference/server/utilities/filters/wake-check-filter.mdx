...
<Warning>
  **Deprecated:** `WakeCheckFilter` is deprecated in favor of
  [`WakePhraseUserTurnStartStrategy`](/api-reference/server/utilities/turn-management/user-turn-strategies#wakephraseuserturns
  tartstrategy). The new strategy provides better integration with turn
  management and supports both timeout and single-activation modes.
</Warning>
