...
A tool call that finds its connection gone drops that session and returns the error to the model, and the call after it connects again.
...
`MCPClient` works with MCP SDK versions 1.24.0+ and 2.x.
...
<ResponseField name="close" type="async method">
  ...
  from a different task than the one that called `start()`. The client stays
  closed until `start()` or `tools()` opens it again: a tool call on a closed
  client raises rather than connect.
</ResponseField>
