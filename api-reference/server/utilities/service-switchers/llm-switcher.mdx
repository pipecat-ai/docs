...
<ParamField path="**kwargs" type="Any">
  ...
  Common options include `max_tokens` (maximum tokens to generate),
  `system_instruction` (override the system prompt for this inference), and
...
<ParamField path="cancellable_by_llm" type="bool | None" default="None">
  Whether the LLM may cancel this call while it runs, through the
  `cancel_<name>` tool advertised alongside it. Pair it with
  `cancel_on_interruption=False`. Defaults to `None`, which falls back to the
  handler's `@tool_options` value, then to `False`.
</ParamField>
