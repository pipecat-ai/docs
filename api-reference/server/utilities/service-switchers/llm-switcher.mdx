...
<Warning>
  **Deprecated since 1.4.0.** Direct functions now register automatically on
  every member LLM (active or not) when you list them in `LLMContext(tools=[...])`,
  so explicit registration is no longer needed — the tools keep working across
  service switches. Push an `LLMSetToolsFrame` to change tools mid-session. This
