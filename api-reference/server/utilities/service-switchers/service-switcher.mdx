...
`ServiceUpdateSettingsFrame` is the exception to the gating. A settings update addressed to a member service (`service=`) reaches it whether or not it is active, and one marked `reach_inactive_services` reaches every member, so whichever service becomes active later is already configured. Any other settings update applies to the active service alone.
