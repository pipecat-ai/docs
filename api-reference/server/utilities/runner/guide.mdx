...
The development runner operates as a FastAPI web server that automatically discovers and executes your bot code. By default it starts a **single server that supports all transports simultaneously** — WebRTC, Daily, LiveKit, telephony (Twilio, Telnyx, Plivo, Exotel), and plain WebSocket. Clients choose which transport they want per-request by sending a `"transport"` field in the `POST /start` body; you no longer restart the server to switch transports.
...
- **LiveKit integration**: It mints room tokens and spawns bots as participants in LiveKit rooms
...
- **`LiveKitRunnerArguments`**: Contains `room_name`, `url`, `token`, `body` (Optional), `session_id` for joining LiveKit rooms
...
- **`handle_sigint`**: Whether the bot should handle SIGINT (Ctrl+C) signals. Pass this to `WorkerRunner(handle_sigint=runner_args.handle_sigint)` so bots run directly can install their own signal handler while hosted bots leave signals to the host.
...
The development runner supports nine transport types, each designed for different use cases. By default all are available from one server; the client selects one per session via the `"transport"` field in the `/start` request. The `-t <transport>` form below is the optional way to **restrict** the server to a single transport.
...
Integration with LiveKit for production WebRTC conferencing with rooms and participant management.

```bash
python bot.py
# POST /start with {"transport": "livekit"}

# Restrict to LiveKit only
python bot.py -t livekit
```

**Runner Arguments**: `LiveKitRunnerArguments`
...
- `POST /start`: RTVI-compatible endpoint for programmatic access (send `"transport": "livekit"`)
...
- `LIVEKIT_ROOM_NAME`: Room name to use (optional, defaults to `pipecat-{random}`)
...
Options:
  ...
  -t, --transport            Restrict to a single transport and set it as the default
                             for /start: daily, livekit, webrtc, twilio, telnyx, plivo,
                             exotel, websocket, moq.
...
- `livekit`: LiveKit integration with room management
- `twilio`, `telnyx`, `plivo`, `exotel`: Telephony provider integration
- `websocket`: Plain WebSocket server
- `moq`: [Media over QUIC](/api-reference/server/services/transport/moq), serving its own socket or dialing a relay
...
- `LIVEKIT_API_KEY`: LiveKit API key (required)
- `LIVEKIT_API_SECRET`: LiveKit API secret (required)
- `LIVEKIT_ROOM_NAME` (Optional): Room name to use (defaults to `pipecat-{random}`)
...
Now your bot supports all nine transport types with just two lines of code in the bot() function.
...
| LiveKit       | `python bot.py -t livekit`                  | `POST /start`                   | `LIVEKIT_URL`, `LIVEKIT_API_KEY`, `LIVEKIT_API_SECRET` (All required) |
