...
With [`EagerUserTurnStrategies`](/api-reference/server/utilities/turn-management/user-turn-strategies#eageruserturnstrategies), this event may fire speculatively from a predicted end of turn before the turn has ended. The `UserTurnController` adds a third parameter, `speculation`, carrying the turn text the inference was run against when the turn is still open. Handlers registered on `LLMUserAggregator` or `UserTurnProcessor` remain unchanged and do not receive this parameter.
...
For `UserTurnController`, the handler signature is:
...
async def on_user_turn_inference_triggered(controller, strategy, speculation):
    if speculation:
        print(f"Speculative inference from predicted turn: {speculation.text}")
    else:
        print(f"Confirmed inference triggered by {strategy}")
```
...
| Parameter     | Type                          | Description                                                                                           |
| ------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------- |
| `aggregator`  | `LLMUserAggregator`           | The user aggregator instance (for handlers on the aggregator)                                         |
| `controller`  | `UserTurnController`          | The turn controller instance (for handlers on the controller)                                         |
| `strategy`    | `BaseUserTurnStopStrategy`    | The strategy that triggered the inference                                                             |
| `speculation` | `UserTurnSpeculation \| None` | Set when the inference answers a turn that hasn't ended yet, carrying the turn text (controller only) |
...
<Note>
  ...
  the user message isn't finalized at turn-stop time, so `message.content` is
  `None`. Subscribe to
  [`on_user_turn_message_added`](#on_user_turn_message_added) instead to get the
  finalized user text. Behavior in cascade (STT → LLM → TTS) pipelines is
  unchanged.
</Note>
...
async def on_user_turn_stop_timeout(aggregator):
    logger.info("User turn ended by the stop timeout")
```

<Tip>
  A turn that times out often has no transcript at all, for example a cough,
  background noise, or speech the STT could not recognize. To get the bot to
  respond after such a turn, use
  [`empty_user_turn`](/api-reference/server/utilities/turn-management/user-turn-strategies#param-empty-user-turn)
  instead of running the LLM from this event. It's on by default for turns that
  interrupted the bot, and `idle_prompt` extends it to turns while the bot was
...
<ParamField path="content" type="str | None">
  The complete transcribed text from the user's turn. `None` in realtime mode
  (`realtime_service_mode=True`), where the user message isn't finalized at
  turn-stop time — subscribe to
  [`on_user_turn_message_added`](#on_user_turn_message_added) for the finalized
  text instead.
</ParamField>
