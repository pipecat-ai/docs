...
description: "Control when Pipecat suppresses user input: mute strategies during bot speech and other pipeline operations."
---
...
- Unmutes permanently after first speech completes
- Also unmutes if an `ErrorFrame` occurs before `BotStartedSpeakingFrame`, preventing a failed first turn from leaving the user muted
