...
<ParamField path="match_policy" type="EagerEndOfTurnMatchPolicy" default="NormalizedMatch">
...
<ParamField path="speculation_timeout" type="float" default="3.0">
  Seconds a prediction may go unresolved before it is withdrawn and the turn
  falls back to answering the committed transcript. Guards against a service that
  stops sending turn signals mid-speculation, which would otherwise leave the
  response held and the bot silent.
</ParamField>
