...
When using an `LLMSwitcher`, the startup configuration reaches every LLM the switcher manages, ensuring that whichever LLM becomes active later is already configured to output turn markers.

### Stale Completions

A `✓` (complete) marker that arrives while VAD still hears the user speaking is treated as stale. This happens when an inference is triggered, the user resumes speaking before the LLM responds, and then the LLM returns `✓`. Since the user is still speaking, the turn isn't actually over. The system:

- Treats the stale `✓` as `○` (incomplete short)
- Suppresses the response text
- Re-arms the 5-second timeout
- Waits for the user to finish speaking before the next inference

This prevents the bot from talking over the user in cases where the completion verdict resolved too late.
