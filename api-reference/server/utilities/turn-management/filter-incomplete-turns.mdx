...
### With User Idle Detection

Combine with user idle detection to re-engage users who have gone silent. The `user_idle_timeout` parameter on `LLMUserAggregatorParams` triggers an `on_user_turn_idle` event when the user has been quiet for a specified duration after the bot stops speaking. Use `LLMMessagesAppendFrame(..., run_llm=True)` in the idle handler to generate a check-in message:
...
user_aggregator, assistant_aggregator = LLMContextAggregatorPair(
    ...
    user_params=LLMUserAggregatorParams(
        ...
        user_idle_timeout=8.0,  # Trigger after 8 seconds of silence
        user_turn_strategies=FilterIncompleteUserTurnStrategies(),
    ),
)
...
async def on_user_turn_idle(aggregator):
    # Push from the aggregator to reach the LLM service downstream
    await aggregator.push_frame(
        LLMMessagesAppendFrame(
            ...
            run_llm=True,  # Run inference immediately
        )
    )
```

<Note>
  The idle timeout should be longer than the incomplete-turn timeouts (default
  5s and 10s) since it covers a user who never started speaking, while those
  timeouts cover a user who was cut off mid-turn. A typical `user_idle_timeout`
  is 8-15 seconds.
</Note>
