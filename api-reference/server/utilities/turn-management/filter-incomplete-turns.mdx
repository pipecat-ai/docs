...
<ParamField path="complete_marker" type="str" default="●">
  Marker the LLM emits when the user turn is complete. It is generated before
  any speakable text, so prefer a character that is a single token in the
  model's tokenizer.
</ParamField>

<ParamField path="incomplete_short_marker" type="str" default="◐">
  Marker for a turn cut off mid-thought.
</ParamField>

<ParamField path="incomplete_long_marker" type="str" default="○">
  Marker for a user who needs more time.
</ParamField>
...
<ParamField path="instructions" type="str" default="...">
  ...
  this to customize how the LLM determines turn completeness. If not provided,
