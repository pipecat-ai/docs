...
description: "Collect user and assistant transcripts in Pipecat with turn-management transcription events and aggregators."
---
...
<Note>
  The examples below assume a cascade (STT → LLM → TTS) pipeline. With a
  realtime (speech-to-speech) service and `LLMContextAggregatorPair(context,
  realtime_service_mode=True)`, collect the user transcript from
