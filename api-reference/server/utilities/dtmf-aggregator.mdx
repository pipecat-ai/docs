...
<ParamField path="TranscriptionFrame" type="Frame">
  Contains the aggregated DTMF sequence as text with the configured prefix. The
  frame is marked as `finalized=True` to ensure proper integration with
  user-turn stop strategies.
</ParamField>
