...
`LLMWorker` extends [`PipelineWorker`](/api-reference/server/agents/base-agent) with an LLM pipeline and automatic tool registration. Pass an `LLMService` to the constructor and define tools with the [`@tool`](/api-reference/server/agents/decorators#tool) decorator. Decorated methods are registered as direct functions on the LLM and tracked so frames queued during tool execution can be deferred until all tools complete.
...
<ParamField path="bridged" type="tuple[str, ...] | None" default="None">
  ...
  bridged agent. See
  [`PipelineWorker`](/api-reference/server/agents/base-agent#configuration) for
  details.
</ParamField>
...
Inherits all properties from [`PipelineWorker`](/api-reference/server/agents/base-agent#properties).
...
Return the tools for this agent's LLM. By default, returns all methods decorated with [`@tool`](/api-reference/server/agents/decorators#tool). Override to provide additional or different tools.
...
| --------- | ------------- | ------------------------------------------------------------------------------------------------------------ |
| `args`    | `dict | None` | Activation arguments (see [`LLMWorkerActivationArgs`](/api-reference/server/agents/types#llmworkeractivationargs)) |
