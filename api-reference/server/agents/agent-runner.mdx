...
`PipelineRunner` runs agents to completion and acts as the host for a multi-agent session. It owns the shared [`WorkerBus`](/api-reference/server/agents/bus#workerbus) and [`WorkerRegistry`](/api-reference/server/agents/types#workerregistry) that back the whole session, and responds to bus messages for lifecycle coordination.
...
<ParamField path="bus" type="WorkerBus | None" default="None">
  The [`WorkerBus`](/api-reference/server/agents/bus#workerbus) instance.
  Creates an
  [`AsyncQueueBus`](/api-reference/server/agents/bus#asyncqueuebus) if not
  provided.
</ParamField>
...
| ---------- | ------------------------------------------------ | ------------------------- |
| `*workers` | [`BaseWorker`](/api-reference/server/agents/base-agent) | One or more agents to add |

### run

```python
async def run(self, worker: PipelineWorker | None = None, *, auto_end: bool = True) -> None
```
...
| ---------- | ----------------------- | ------- | -------------------------------------------------------------------------------------------------------------------------------- |
| `worker`   | `PipelineWorker | None` | `None`  | Optional agent to run. Deprecated -- register with `add_workers()` before calling `run()` instead.                              |
