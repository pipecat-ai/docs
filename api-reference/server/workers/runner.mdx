...
`PipelineRunner` runs agents to completion and acts as the host for a multi-agent session. It owns the shared [`WorkerBus`](/api-reference/server/bus/bus#workerbus) and [`WorkerRegistry`](/api-reference/server/workers/types#workerregistry) that back the whole session, and responds to bus messages for lifecycle coordination.
...
<ParamField path="bus" type="WorkerBus | None" default="None">
  The [`WorkerBus`](/api-reference/server/bus/bus#workerbus) instance.
  Creates an
  [`AsyncQueueBus`](/api-reference/server/bus/bus#asyncqueuebus) if not
  provided.
</ParamField>
