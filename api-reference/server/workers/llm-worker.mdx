...
<ParamField path="bridged" type="tuple[str, ...] | None" default="None">
  ...
  bridged agent. See
  [`PipelineWorker`](/api-reference/server/workers/base-worker#configuration) for
  details.
</ParamField>
...
Return the tools for this agent's LLM. By default, returns all methods decorated with [`@tool`](/api-reference/server/workers/decorators#tool). Override to provide additional or different tools.
