...
When `metrics_enabled` is `True` (the default), the observer forwards metrics from `MetricsFrame` to RTVI clients. The following metric types are forwarded under their respective keys in `RTVIMetricsMessage`:

| Metric Key      | Source                 | Description                                      |
| --------------- | ---------------------- | ------------------------------------------------ |
| `ttfb`          | `TTFBMetricsData`      | Time To First Byte latency measurements          |
| `ttfa`          | `TTFAMetricsData`      | Time To First Audio latency measurements         |
| `processing`    | `ProcessingMetricsData` | Processing time metrics                          |
| `tokens`        | `LLMUsageMetricsData`  | LLM token usage (prompt, completion, total)      |
| `characters`    | `TTSUsageMetricsData`  | TTS character usage                              |

Each metric entry in the message includes the `processor` field identifying the source service, along with metric-specific fields like `value`, `model`, and timing breakdowns.
