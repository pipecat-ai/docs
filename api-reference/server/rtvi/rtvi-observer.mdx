...
<ParamField path="bot_llm_marker_enabled" default="False">
  Indicates if the sideband markers the bot's LLM emits (`LLMMarkerFrame`, e.g.
  the turn-completion markers of `filter_incomplete_user_turns`) should be sent.
  Markers are meant for the conversation context, not for clients, so this is
  off by default.
</ParamField>
...
| `LLMMarkerFrame`              | `bot-llm-marker` (when enabled)             |
| **TTS Events**                |
