...
description: "GoogleRTVIObserver adds support for sending Google search responses to RTVI clients in Pipecat pipelines."
---

## Overview

The `GoogleRTVIObserver` extends the [base `RTVIObserver` type](./rtvi-observer), to add support for the `bot-llm-search-response` message type and providing clients with the search results from the `GoogleLLMService`.
...
To use `GoogleRTVIObserver`, pass a `GoogleRTVIProcessor` to your `PipelineWorker`. This automatically creates and attaches the Google-specific observer:
