...
The processor handles `dtmf` client messages for keypad input (e.g., phone menu navigation). Sending `{type: "dtmf", data: {button: "1"}}` makes the processor push an `InputDTMFFrame` downstream, reaching the bot's DTMF handling (e.g., a `DTMFAggregator`) exactly as a telephony transport's keypress would.

Clients send one `dtmf` message per key, matching how telephony transports deliver keypresses. Valid `button` values are `0`-`9`, `*`, and `#`.
