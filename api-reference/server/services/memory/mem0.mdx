...
sidebarTitle: "Mem0"
description: "Mem0MemoryService adds long-term conversation memory to Pipecat agents, recalling context across sessions with Mem0."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/rag/rag-mem0.py"
  >
...
| `api_version`           | `str \| None` | `None`                                              | **Deprecated in 1.4.0**. No replacement. Mem0 2.0.0 removed the `api_version`/`output_format` parameters from the client. Will be removed in 2.0.0. |
| `system_prompt`         | `str`         | `"Based on previous conversations, I recall: \n\n"` | Prefix text for memory context messages.                                                                                                                                     |
| `add_as_system_message` | `bool`        | `True`                                              | Whether to add memories as instruction messages (`"developer"` role) rather than user messages. Providers without a `"developer"` role receive them as `"user"`. |
...
- **Mem0 2.0.0 upgrade**: Pipecat 1.4.0 upgraded to `mem0ai>=2,<3` to support Mem0 2.0.0. The `api_version` parameter is deprecated (no replacement). Additionally, `add()` operations are now async server-side, meaning newly stored memories are only queryable once processed by Mem0.
