...
- **Pronunciation hints**: `ElevenLabsTTSService` and `ElevenLabsHttpTTSService` support SSML `<phoneme>` tags on v2 models (`eleven_flash_v2`, `eleven_turbo_v2`) only. The WebSocket service requires `enable_ssml_parsing=True`; the HTTP service always reads SSML. `ElevenLabsDialogueTTSService` (v3 models) reads IPA between slashes instead of SSML tags.
