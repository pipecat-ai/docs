...
description: "TTS with ElevenLabsTTSService (WebSocket streaming, word-level timestamps) and ElevenLabsHttpTTSService for HTTP synthesis."
---
...
- **Language support varies by model**: Only specific models accept a `language` parameter. `eleven_flash_v2_5` and `eleven_turbo_v2_5` support 32 languages. `eleven_v3` and `eleven_v3_conversational` support 74 languages (including Farsi, Pashto, and Sindhi, which no other ElevenLabs model covers). Other models (like `eleven_multilingual_v2`) use automatic language detection and ignore the `language` parameter. If you specify a language that the selected model doesn't support, it's dropped with a warning rather than sent to the API.
