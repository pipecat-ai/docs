...
sidebarTitle: "Azure"
description: "TTS on Azure Cognitive Services: AzureTTSService for real-time WebSocket streaming and AzureHttpTTSService for simpler HTTP synthesis."
---
...
| `effect`           | `str`             | `NOT_GIVEN` | Audio effect processor that compensates for playback distortion on a target device: `"eq_car"` for enclosed vehicles, `"eq_telecomhp8k"` for narrowband telephony (pair it with an 8 kHz pipeline). Azure ignores unrecognized values. |
| `emphasis`         | `str`             | `NOT_GIVEN` | Emphasis level for SSML.                                                                                                                                                                                                               |
| `force_locale`     | `bool`            | `NOT_GIVEN` | Wraps text in SSML `<lang>` to force the configured `language` accent instead of auto-detection.                                                                                                                                       |
...
| `voice_parameters` | `str`             | `NOT_GIVEN` | Model parameters for HD voices, as SSML's semicolon-separated `parameters` attribute on `<voice>` (e.g., `"temperature=0.7;top_p=0.8;top_k=22;cfg_scale=1.4"` or `"enhancePronunciation=true"`). Standard voices ignore.               |
