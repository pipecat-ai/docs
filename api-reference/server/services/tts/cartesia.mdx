...
| `generation_config`     | `GenerationConfig` | `NOT_GIVEN` | Generation configuration for Sonic-3 and Sonic-3.5 models. See below.                               |
| `pronunciation_dict_id` | `str`              | `NOT_GIVEN` | ID of the pronunciation dictionary for custom pronunciations.                                       |

#### GenerationConfig (Sonic-3 and Sonic-3.5)

Configuration for Sonic-3 and Sonic-3.5 generation parameters:
...
<Note>
  ...
  for `sonic-3`. Sonic 3.5 has significantly improved natural expressiveness, so
  most users will get better results by relying on the model and the context of
  their input text rather than manually tuning emotion, speed, or volume. These
  controls remain available on the API, but effectiveness may vary on Sonic 3.5.
</Note>
