...
description: "Low-latency TTS with CartesiaTTSService (WebSocket streaming, word timestamps) and CartesiaHttpTTSService for batch synthesis."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/voice/voice-cartesia.py"
  >
...
<ParamField path="model" type="str" default="sonic-3.6" deprecated>
...
<ParamField path="params" type="InputParams" default="None" deprecated>
  _Deprecated in v0.0.105. Use `settings=CartesiaTTSService.Settings(...)`
  instead._
</ParamField>
...
<ParamField path="extra_headers" type="dict[str, str] | None" default="None">
  ...
  supplies its own authentication or routing headers.
</ParamField>
...
<ParamField path="extra_headers" type="dict[str, str] | None" default="None">
  Additional headers to send with each synthesis request.
</ParamField>
...
| ----------------------- | ------------------ | ----------- | --------------------------------------------------------------------------------------------------- |
| `model`                 | `str`              | `None`      | TTS model identifier. Defaults to `sonic-3.6` when not specified. _(Inherited from base settings.)_ |
...
Configuration for Cartesia generation parameters. Applicable to sonic-3 series models:
...
<Note>
  These helper methods use SSML tags and generation config parameters introduced
  for `sonic-3`. `sonic-3.5` has significantly improved natural expressiveness,
  so most users will get better results by relying on the model and the context
  of their input text rather than manually tuning emotion, speed, or volume.
  These controls remain available on the API, but effectiveness may vary on
  `sonic-3.5`.
</Note>
...
### format_pronunciation(word: str, ipa: str) -> str | None

Renders a pronunciation as Cartesia inline phoneme blocks. Cartesia reads `<<…>>` blocks of `|`-separated IPA phones, with stress marks directly before the vowel they stress.

```python
# Format a single word's pronunciation
formatted = CartesiaTTSService.format_pronunciation("Metformin", "mɛtˈfɔɹmɪn")
# Returns: "<<m|ɛ|t|f|ˈ|ɔ|ɹ|m|ɪ|n>>"
```

Most applications should use `pronunciation_transform_ipa()` instead, which creates a text transform that automatically replaces words with their pronunciations:
