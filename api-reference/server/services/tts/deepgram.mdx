...
description: "Aura and Flux TTS with DeepgramTTSService, DeepgramHttpTTSService, DeepgramFluxTTSService, and SageMaker variants for hosted inference."
---

## Overview

Deepgram provides five TTS service implementations:
...
- `DeepgramFluxSageMakerTTSService` for real-time Flux synthesis using Deepgram Flux TTS models deployed on AWS SageMaker endpoints via HTTP/2 bidirectional streaming
...
- `DeepgramSageMakerTTSService` for real-time Aura synthesis using Deepgram Aura TTS models deployed on AWS SageMaker endpoints via HTTP/2 bidirectional streaming
...
<CardGroup cols={2}>
  ...
  >
    Complete example with Deepgram Aura on SageMaker
  </Card>
  <Card
...
For the SageMaker variants, install the SageMaker dependencies:

```bash
uv add "pipecat-ai[sagemaker]"
```

For the Flux SageMaker variant, install both Deepgram and SageMaker dependencies:
...
Before using `DeepgramSageMakerTTSService` or `DeepgramFluxSageMakerTTSService`, you need:
...
3. **Voice Selection**: Choose from available Aura voice models (for `DeepgramSageMakerTTSService`) or Flux voice models (for `DeepgramFluxSageMakerTTSService`)
...
| `speed`        | `float \| None`                   | `None`              | Speech-rate multiplier, from 0.5 to 1.5 in steps of 0.05. `None` leaves Flux at its default rate. Applied to the open connection via Flux's `Configure` message, so a speed change keeps the cross-turn acoustic state. |
...
<ParamField path="endpoint_name" type="str" required>
  Name of the SageMaker endpoint with a Deepgram Flux TTS model deployed (e.g.,
  `"my-deepgram-flux-tts-endpoint"`).
</ParamField>
...
<ParamField
  path="settings"
  type="DeepgramFluxSageMakerTTSService.Settings"
  default="None"
>
  Runtime-configurable settings. See [DeepgramFluxSageMakerTTSService
  Settings](#deepgramfluxsagemakerttsservice-settings) below.
</ParamField>

#### DeepgramFluxSageMakerTTSService Settings

Runtime-configurable settings passed via the `settings` constructor argument using `DeepgramFluxSageMakerTTSService.Settings(...)`. These can be updated mid-conversation with `TTSUpdateSettingsFrame`. See [Service Settings](/pipecat/fundamentals/service-settings) for details.
...
| `language` | `Language \| str`           | `None`      | Language for synthesis. _(Inherited.)_                                                                                                                                  |
| `speed`    | `float \| None \| NotGiven` | `NOT_GIVEN` | Speech-rate multiplier, from 0.7 to 1.5. `None` leaves Aura at its default rate. Supported by Aura-2 English and Spanish voices; Deepgram recommends ≥ 0.9 for Spanish. |
...
- **Choosing a service**: The Aura WebSocket service (`DeepgramTTSService`) and Aura SageMaker service (`DeepgramSageMakerTTSService`) both support real-time streaming with interruption handling, making them suitable for interactive conversations. The HTTP service (`DeepgramHttpTTSService`) is simpler but processes each request as a batch. The Flux services (`DeepgramFluxTTSService` and `DeepgramFluxSageMakerTTSService`) are streaming-first options built for voice agents, with token-level streaming and cross-turn prosody state.
...
- **SageMaker deployment**: The SageMaker services (`DeepgramSageMakerTTSService` and `DeepgramFluxSageMakerTTSService`) require a Deepgram TTS model deployed to an AWS SageMaker endpoint. See the [Deepgram SageMaker deployment guide](https://developers.deepgram.com/docs/deploy-amazon-sagemaker) for setup instructions.
- **Pronunciation hints**: Aura-2 services (`DeepgramTTSService`, `DeepgramHttpTTSService`, and `DeepgramSageMakerTTSService`) support inline pronunciation objects for English and Spanish. Flux has no pronunciation markup, so `pronunciation_transform_ipa()` speaks words as written. See the [voice controls documentation](https://developers.deepgram.com/docs/tts-voice-controls) for which controls each model supports.
...
- **Voice Controls**: `speed` (0.5 to 1.5) is applied to the open connection with Flux's `Configure` message, so a speed change keeps the cross-turn acoustic state. `expressivity` (-2 to 2) is fixed when the connection opens, so a change reconnects.
...
The WebSocket and SageMaker services (`DeepgramTTSService`, `DeepgramFluxTTSService`, `DeepgramSageMakerTTSService`, and `DeepgramFluxSageMakerTTSService`) support the standard [service connection events](/api-reference/server/events/service-events):
