...
description: "SmallestTTSService streams real-time TTS through Smallest AI's Waves WebSocket API with configurable voice parameters."
---
...
<ParamField path="max_buffer_delay_ms" type="int" default="None">
  Upper bound, in milliseconds (0-5000), on how long the server buffers a
  continuation fragment before speaking it. If `None`, the server's default
  (3000ms) applies.
</ParamField>
...
| `math_notation` | `bool`            | `None`               | Read digit-flanked math operators as words (`2 + 2` as "two plus two") instead of leaving them to the default number reader. When `None`, the API default (off) applies. |
...
- **Continuation contexts**: Text fragments within the same LLM turn share a continuation context: they're sent with the turn's `context_id` and `continue: true` so the server joins them into one continuous generation instead of resetting prosody on each fragment. The context is automatically closed with `continue: false` once the turn ends.
