...
sidebarTitle: "Piper"
description: "PiperTTSService and PiperHttpTTSService synthesize neural TTS from a self-hosted Piper server: private, no external API."
---
...
- **Voice validation**: `PiperTTSService` validates the `voice` parameter at construction time. A missing or blank voice raises a `ValueError` before attempting to download the model.
...
- **GPL-3.0 license**: `PiperTTSService` runs Piper in-process via the `piper-tts` package (the `piper` extra), which is GPL-3.0 licensed. Distributing an application that includes it may subject the application to the GPL's source-disclosure terms. To keep Piper out of your application's license scope, use `PiperHttpTTSService` instead, which talks to a separately installed Piper HTTP server.
