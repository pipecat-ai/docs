---
title: "Gemini Live"
description: "GeminiLiveLLMService connects Pipecat to Google's Gemini Live API for real-time speech-to-speech multimodal conversation."
---
...
<ParamField
  path="tools"
  type="ToolsSchema | List[FunctionSchema | DirectFunction] | List[dict]"
  default="None"
>
...
| `seed`                       | `int`                                    | `NOT_GIVEN` | Random seed for reproducibility. _(Inherited from base settings.)_                                                                   |
...
| `turn_coverage`              | `TurnCoverage \| None`                   | `NOT_GIVEN` | Which realtime input a user turn covers. Unset uses the model's own default.                                                         |
...
```python
from pipecat.services.google.gemini_live.llm import GeminiLiveLLMService, GeminiVADParams, ContextWindowCompressionParams
...
- **Model support**: The service supports Gemini 2.5, Gemini 3.x, and the 3.8 Live family (`gemini-3.8-live`, `gemini-3.8-live-extended-thinking`). The service automatically detects and handles model-specific behavior.
- **Function calling behavior**: Functions registered with `cancel_on_interruption=False` use Gemini's NON_BLOCKING tool mechanism, allowing the conversation to continue while the tool runs in the background. The 3.8 Live family defaults to NON_BLOCKING for all function calls; synchronous tools are explicitly declared BLOCKING on `gemini-3.8-live` to maintain blocking semantics. The thinking model `gemini-3.8-live-extended-thinking` accepts only NON_BLOCKING and logs a warning for synchronous tools. Earlier Gemini 3.x models (before 3.8) do not support NON_BLOCKING. Note: An intermittent 1008 error can occasionally occur on Gemini 2.5 during long-running tool calls; the service auto-reconnects when this happens.
- **Thinking level defaults**: Live thinking models (e.g., `gemini-3.8-live-extended-thinking`) require a `thinking_level` in the thinking configuration. When none is set, the service applies `LOW` as the default to keep reply latency down.
- **Turn completion with interaction status**: On thinking models from 3.8 onward, the service holds the bot turn open while Gemini reports `interaction_status: IN_PROGRESS`, ensuring that a reply spanning multiple `turn_complete` messages is recorded as a single assistant turn. This requires `google-genai>=2.19.0`; older SDKs log a warning.
