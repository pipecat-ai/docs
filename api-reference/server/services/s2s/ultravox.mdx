...
description: "UltravoxRealtimeLLMService provides real-time conversational AI with Ultravox Realtime, with agent and one-shot input modes."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/realtime/realtime-ultravox.py"
  >
...
<ParamField
  path="settings"
  type="UltravoxRealtimeLLMService.Settings"
  default="None"
>
...
| `model`         | `str`            | `None`   | Model identifier to use (e.g., `"ultravox-v0.7"`). `None` lets Ultravox pick its current default model.    |
