...
- **Token Usage Metrics**: Automatic tracking and reporting of token consumption
...
<ParamField path="params" type="Params" default="Params()" deprecated>
  Model parameters for audio configuration and inference. See [Settings](#settings)
  below.
...
- **Token usage metrics**: The service automatically emits `LLMUsageMetricsData` from Nova Sonic's `usageEvent`. Each event reports the incremental token delta, with speech and text tokens combined into `prompt_tokens` and `completion_tokens`. Metrics summed over a session match the session total reported by AWS.
