---
title: "HeyGen Video Avatar"
sidebarTitle: "HeyGen"
description: "HeyGenVideoService drives HeyGen LiveAvatar interactive video avatars from your Pipecat agent's speech output."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/video-avatar/video-avatar-heygen-video-service.py"
  >
...
>
  ...
  `"Shawn_Therapist_public"` avatar. See [LiveAvatar video
  settings](#liveavatar-video-settings) for encoding and quality options.
</ParamField>
...
### LiveAvatar Video Settings

`LiveAvatarNewSessionRequest.video_settings` controls the encoding and quality
of the avatar video stream. If you omit `video_settings`, Pipecat requests H264
encoding with high quality.

<ParamField path="video_settings.encoding" type="VideoEncoding" required>
  Video codec for the avatar stream. Use `VideoEncoding.H264` for new sessions.
  `VideoEncoding.VP8` remains available temporarily but is deprecated by
  LiveAvatar.
</ParamField>

<ParamField
  path="video_settings.quality"
  type="VideoQuality"
  default="VideoQuality.high"
>
  Rendering quality for the avatar stream. Options are `low`, `medium`, `high`,
  and `very_high`.
</ParamField>

<ParamField path="max_session_duration" type="int" default="None">
  Maximum session duration in seconds. The ceiling is set by the LiveAvatar
  subscription tier, which also supplies the default when this is unset.
</ParamField>

<Warning>
  LiveAvatar has deprecated VP8. Applications that explicitly select
  `VideoEncoding.VP8` should migrate to `VideoEncoding.H264`.
</Warning>
