---
title: "LemonSlice Transport"
sidebarTitle: "LemonSlice"
description: "LemonSliceTransport joins your Pipecat bot to a room with a LemonSlice avatar, layered on the Daily transport."
---
...
3. **Avatar Selection**: Choose from available interactive avatars, provide an agent image URL, or upload a local image
...
<ParamField path="session_request" type="LemonSliceNewSessionRequest" required>
  Session creation parameters. Exactly one of `agent_id`, `agent_image_url`, or
  `agent_image` must be provided. See
...
<ParamField path="agent_image_url" type="str" default="None">
  URL to an agent image. Provide exactly one of `agent_image_url`, `agent_id`, or
  `agent_image`.
</ParamField>

<ParamField path="agent_id" type="str" default="None">
  ID of a LemonSlice agent. Provide exactly one of `agent_image_url`, `agent_id`,
  or `agent_image`.
</ParamField>

<ParamField path="agent_image" type="PIL.Image.Image" default="None">
  PIL image uploaded as the agent image. Provide exactly one of `agent_image_url`,
