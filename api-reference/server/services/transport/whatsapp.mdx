...
<Warning>
  The single-argument `connection_callback(connection)` signature is deprecated
  as of Pipecat 1.4.0. Update your callbacks to accept both `connection` and
  `call` arguments to receive caller metadata. The old signature still works but
