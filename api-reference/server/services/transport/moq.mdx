...
| -------------------- | ------------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `relay_url`          | `str \| None` | `None`  | Full relay URL, query string included (e.g. `https://relay.example.com:4080/moq?jwt=…`). When unset, composed from the constructor's host/port/path. Ignored in serve mode                                                                                                                             |
...
| `connection_timeout` | `float`       | `60.0`  | How long, in seconds, the peer may be missing before the transport gives up on it. Bounds the wait for the peer to join and, in client mode, an outage during which the transport redials the relay with backoff. When it expires, the transport fires `on_client_disconnected` for a peer it had seen |
...
| Event                    | Description                                                                                                                 |
| ------------------------ | --------------------------------------------------------------------------------------------------------------------------- |
| `on_connected`           | A MOQ session is established: the serve bind, or each session with the relay, the first and every redialed one              |
| `on_disconnected`        | An established session ends, or the transport stops without ever having had one. Client mode redials after it               |
| `on_client_connected`    | The peer's broadcast was announced                                                                                          |
| `on_client_disconnected` | The peer is gone: its broadcast went away, or the session could not be re-established in time                               |
| `on_track_subscribed`    | A remote track subscription succeeded                                                                                       |
| `on_error`               | An error in the underlying transport, with the message, exception, error category (when known), and whether it is permanent |

## Notes

- **Client-mode reconnect**: when the relay session drops, the transport redials with backoff for as long as `connection_timeout` allows. The bot's broadcast persists across dials, and the peer is only reported gone when that time is up. End the call from `on_client_disconnected`, which fires once the peer is gone; `on_connected` and `on_disconnected` fire for each relay session.
