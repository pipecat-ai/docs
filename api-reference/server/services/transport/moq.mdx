...
In client mode a dropped relay session is redialed rather than reported as the peer leaving. Each dial announces the bot's broadcast afresh on the same path, with the transcript so far replayed into it, so the peer sees the same session return, and the transport retries with a delay that doubles from 0.5 s up to 8 s until `reconnect_timeout` runs out, counted from the start of the outage. `on_reconnecting` fires before each attempt with the attempt number, and `on_reconnected` fires once the peer's data flows again on the new session, which is what ends the outage; a relay keeps announcing a path whose route has died, so an announcement alone does not. Only when the window expires does the transport fire `on_client_disconnected` for a peer it had seen, so the usual handler ends the call. With `reconnect_timeout` set to `0`, a dropped session ends the transport the way a hangup does.
...
| `on_reconnected`         | Client mode: the peer's data flows again after an outage                                                             |
