...
"og:title": "Small WebRTC Transport - Pipecat Server"
sidebarTitle: "Small WebRTC"
description: "SmallWebRTCTransport makes serverless peer-to-peer WebRTC connections between a client and your Pipecat bot, no provider needed."
---
...
<Note>
  The `webrtc` extra provides audio and basic video support via `aiortc`. The
  `webrtc-video` extra adds OpenCV support for using `SmallWebRTCTransport` with
  video.
</Note>
