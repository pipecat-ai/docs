...
<Note>
  The transport pushes `InputDTMFFrame` automatically, so you only need this handler for custom DTMF logic. Use `DTMFAggregator` to collect digits into sequences.
</Note>
