...
sidebarTitle: "Tavus"
description: "TavusTransport joins your Pipecat bot to the same virtual room as a Tavus avatar for conversational video sessions."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/video-avatar/video-avatar-tavus-video-service.py"
  >
...
<Tip>
  Use `persona_id="pipecat0"` (the default) to have Tavus use your Pipecat bot's
  TTS voice instead of a Tavus persona voice.
</Tip>
...
<ParamField path="persona_id" type="str" default="pipecat0">
  ID of the Tavus persona. Defaults to `"pipecat0"`, which signals Tavus to use
  the TTS voice of the Pipecat bot instead of a Tavus persona voice, and to
...
<ParamField path="audio_out_faster_than_realtime" type="bool" default="True">
  Whether to send bot audio app messages as fast as possible instead of paced to
  real playback time. Enabled by default: it hands Tavus a larger rendering
  buffer, since TTS audio is typically generated well faster than realtime and
  pacing it throttles delivery to 1x. Set to `False` when a downstream processor
  (e.g. `AudioBufferProcessor`) needs bot audio to arrive at the pipeline at
  roughly real time.
</ParamField>
...
<Tip>
  For local development, set the `TAVUS_SAMPLE_ROOM_URL` environment variable to
  skip the Tavus API call and use a pre-existing Daily room URL instead.
</Tip>
