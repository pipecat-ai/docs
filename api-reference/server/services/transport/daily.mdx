...
"og:title": "Daily WebRTC Transport - Pipecat Server"
sidebarTitle: "Daily"
description: "DailyTransport connects Pipecat pipelines to Daily's hosted WebRTC infrastructure for real-time audio and video sessions."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/transports/transports-daily.py"
  >
...
<ParamField
  path="custom_audio_track_params"
  type="Mapping[str, DailyCustomAudioTrackParams]"
  default="None"
>
...
<ParamField
  path="custom_video_track_params"
  type="Mapping[str, DailyCustomVideoTrackParams]"
  default="None"
>
...
Daily supports two ways to record a call: start and stop it imperatively from
the transport, or configure a meeting token to start cloud recording
automatically when the bot joins.

#### start_recording / stop_recording

Start and stop Daily's cloud recording during a session.

```python
async def start_recording(
    streaming_settings=None, stream_id=None, force_new=None
) -> tuple[str, CallClientError | None]

async def stop_recording(stream_id=None) -> CallClientError | None
```

Typically triggered from an event handler once the session is underway:
...
<Note>
  Cloud recording must be enabled on the room or token via `enable_recording`.
  See [DailyRESTHelper](/api-reference/server/utilities/daily/rest-helper).
</Note>

#### Start recording automatically on join

Instead of calling `start_recording()`, configure a meeting token to begin
cloud recording as soon as the bot joins, using `DailyRESTHelper`:
...
Fired when the dial-in SIP endpoint is ready to receive calls. If `dialin_settings` are configured, Pipecat automatically calls the Daily `pinlessCallUpdate` API. Failures during the update (HTTP errors, timeouts, or exceptions) trigger the [`on_error`](#on_error) event.
