...
When transcription starts successfully, the transport broadcasts an `STTMetadataFrame` with Deepgram's TTFS P99 latency, matching the behavior of standalone STT services. Downstream components like `LLMUserAggregator` and user-turn-stop strategies use this metadata instead of falling back to defaults.
