...
description: "TransportParams is the base configuration class shared by all Pipecat transports: audio, video, and VAD settings."
---
...
<ParamField path="audio_out_write_timeout_secs" type="float" default="10.0">
  Maximum time in seconds a single write to the transport may take before the
  peer is considered gone. A client that stops reading leaves the write waiting
  with nothing to fail, so exceeding this timeout marks the transport as
  unusable.
</ParamField>
