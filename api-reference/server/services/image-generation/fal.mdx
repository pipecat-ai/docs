...
sidebarTitle: "fal"
description: "FalImageGenService generates images from text prompts using fal's fast Stable Diffusion models in Pipecat pipelines."
---
...
| `negative_prompt`       | `str \| None`   | `NOT_GIVEN` | Details to keep out of the generated image. `None` omits it from the request.           |
| `guidance_scale`        | `float \| None` | `NOT_GIVEN` | CFG scale, how closely generation follows the prompt. `None` uses the model's default.  |
