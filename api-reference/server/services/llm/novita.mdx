...
- **Token Usage Reporting**: Novita reports cumulative token usage on every streamed chunk. The service holds the latest snapshot and reports the final total once when the completion finishes, ensuring accurate metrics even if the response is interrupted mid-stream.
