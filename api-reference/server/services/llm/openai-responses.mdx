...
<ParamField path="service_tier" type="str" default="None">
  Service tier to use: `"auto"`, `"default"`, `"flex"`, `"scale"`, `"fast"` or
  `"priority"`. `"fast"` is OpenAI's low-latency tier, the name that replaced
  `"priority"`; both values are accepted.
</ParamField>
...
| `mode`    | `"standard"` \| `"pro"`                 | `None`  | Reasoning mode for models that offer one, such as the gpt-5.6 series: `"standard"` or the slower, more thorough `"pro"`. `None` uses the model's default. `effort` selects the reasoning intensity within the chosen mode. |
...
Pass a `response_schema` to enforce a JSON structure in the reply:
...
# Returns JSON text matching the schema, e.g., '{"answer": "yes"}'
```

The Responses API enforces the schema on models that support it (gpt-4o-mini and later). Older models (gpt-4-turbo, gpt-3.5, o1-preview, o1-mini) ignore it with a warning.
