...
- **Error handling**: Both variants push an `ErrorFrame` when the API reports terminal error events (`response.failed`, `response.incomplete`, or `error` events mid-stream). This enables `ServiceSwitcherStrategy` to fail over to a backup service and ensures failures appear in logs.
