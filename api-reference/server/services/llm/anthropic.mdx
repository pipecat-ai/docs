...
<ParamField path="client" type="AsyncAnthropic" default="None">
  ...
  `AsyncAnthropicBedrock` or `AsyncAnthropicVertex`. When using
  `AsyncAnthropicBedrock`, you must pass `aws_region=` or set `AWS_REGION` in
  the environment.
</ParamField>
...
- **Prompt caching**: When `enable_prompt_caching` is enabled, Anthropic caches repeated context to reduce costs. Cache control markers are automatically added to the system prompt and the most recent user messages, allowing tools and system instructions to be cached once and reused across conversations. This is most effective for conversations with large system prompts or long conversation histories.
...
- **Custom clients**: You can pass custom Anthropic client instances (e.g., `AsyncAnthropicBedrock` or `AsyncAnthropicVertex`) via the `client` parameter to use Anthropic models through other cloud providers. `AsyncAnthropicBedrock` requires an explicit `aws_region=` argument or `AWS_REGION` environment variable.
