...
description: "CerebrasLLMService accesses Cerebras language models for chat completion via an OpenAI-compatible interface with streaming."
---
...
- Cerebras supports the full set of OpenAI chat completion parameters including `frequency_penalty`, `presence_penalty`, `max_tokens`, and `service_tier`.
- Cerebras is known for ultra-fast inference speeds on supported models.
- The service sends `developer`-role messages directly to Cerebras's developer instruction layer, which sits above user instructions in the prompt hierarchy.
