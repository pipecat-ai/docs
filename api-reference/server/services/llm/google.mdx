...
- **Thinking defaults**: By default, Pipecat applies low-latency thinking defaults for Flash models to reduce latency. Gemini 2.5 Flash uses `thinking_budget=0` (disables thinking), while Gemini 3+ Flash uses the lowest `thinking_level` the model accepts (`"minimal"` for most models, `"low"` for Gemini 3.7 Flash, which rejects `"minimal"`). To override this behavior, explicitly pass a `GoogleThinkingConfig` via `settings`.
