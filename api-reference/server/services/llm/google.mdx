...
<Note>
  ...
  `thinking_budget=0` (disables thinking), while Gemini 3+ Flash uses the
  lowest `thinking_level` the model accepts (`"minimal"` for most models,
  `"low"` for Gemini 3.7 Flash).
</Note>
