...
- **Incomplete responses**: The service logs a warning when Gemini stops generating for a notable reason — withheld for safety or recitation, a rejected tool call, or truncated at the output token limit. Whatever partial text did arrive is still passed downstream. Responses that end normally produce no warning.
