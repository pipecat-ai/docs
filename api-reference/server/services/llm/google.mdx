...
description: "GoogleLLMService integrates Google's Gemini models with streaming responses, function calling, and multimodal input support."
---
...
| `frequency_penalty`  | `float`                | `NOT_GIVEN`        | Frequency penalty (0.0 to 2.0). _(Inherited from base settings.)_                                  |
| `presence_penalty`   | `float`                | `NOT_GIVEN`        | Presence penalty (0.0 to 2.0). _(Inherited from base settings.)_                                   |
...
<Note>
  ...
  `thinking_budget=0` (disables thinking), while Gemini 3+ Flash uses the lowest
  `thinking_level` the model accepts (`"minimal"` for most models, `"low"` for
...
| `thinking_level`   | `str`  | `None`  | Thinking level for Gemini 3 models. `"low"`, `"high"` for 3 Pro; `"minimal"`, `"low"`, `"medium"`, `"high"` for most 3 Flash models (3.7 Flash and 3.8 Flash accept only `"low"`, `"medium"`, `"high"`). |
...
<Note>
  ...
  `thinking_level`. Set either one or the other, never both. When
  `thinking_budget` is set on a Gemini 3 model, the service emits a warning
  because the behavior varies by model and backend (it may be honored, ignored,
  or rejected).
</Note>
...
# Update safety settings (accepts SafetySetting objects or plain dicts)
await worker.queue_frame(
    LLMUpdateSettingsFrame(
        delta=GoogleLLMSettings(
            safety_settings=[
...
- **Thinking defaults**: By default, Pipecat applies low-latency thinking defaults for Flash models to reduce latency. Gemini 2.5 Flash uses `thinking_budget=0` (disables thinking), while Gemini 3+ Flash uses the lowest `thinking_level` the model accepts (`"minimal"` for most models, `"low"` for Gemini 3.7 Flash and Gemini 3.8 Flash, which reject `"minimal"`). The same defaults apply on the `run_inference()` path as in the pipeline. To override this behavior, explicitly pass a `GoogleThinkingConfig` via `settings`.
- **Response schema**: `run_inference()` accepts an optional `response_schema` parameter (a JSON schema) that asks Gemini to enforce the reply's structure. Gemini 2.5 and later models support this. When the schema is provided, the reply is JSON text matching it. Older models (Gemini 2.0 and earlier) ignore the schema with a warning.
- **Thinking budget warning**: When `thinking_budget` is set on a Gemini 3 model, the service emits a warning because Gemini 3 models take `thinking_level` instead. Whether a `thinking_budget` set alongside one is honored, ignored, or rejected varies by model and backend. Use `thinking_level` with Gemini 3 models.
- **Incomplete responses**: When a response ends for a notable reason — withheld for safety, recitation, or prohibited content, a rejected tool call, or the output hitting the token limit — the service logs a warning naming Gemini's `finish_reason`. Whatever text did arrive is still pushed downstream, so a turn that comes back short has an explanation in the logs rather than only in the transcript.
- **Safety settings**: Categories left unspecified in `safety_settings` keep the Gemini API defaults. The setting accepts `SafetySetting` objects or plain dicts, and is runtime-updatable via `LLMUpdateSettingsFrame`.
...
- **Multimodal support**: Gemini models natively support image and audio inputs through Google's Content/Part format. Images and audio are automatically converted from OpenAI-style contexts. An `image_url` content part holding a `data:` URI is inlined; any other URL is passed to Gemini as an external reference, so an image already hosted somewhere doesn't have to be fetched and re-encoded into the context.
