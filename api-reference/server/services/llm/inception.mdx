...
description: "InceptionLLMService serves Inception's Mercury-2 diffusion-based reasoning model via an OpenAI-compatible interface."
---
...
<ParamField path="model" type="str" default="mercury-2">
  Model identifier to use. Defaults to "mercury-2", Inception's diffusion-based
  reasoning model.
</ParamField>

<ParamField
  path="reasoning_effort"
  type="Literal['instant', 'low', 'medium', 'high'] | None"
  default="None"
>
  Controls how much reasoning the model applies. Options are "instant", "low",
  "medium", or "high". When unset, the parameter is omitted and Inception's
  server-side default applies.
</ParamField>

<ParamField path="realtime" type="bool | None" default="None">
  When True, reduces time to first diffusion block (TTFT) for faster initial
  response times.
</ParamField>
