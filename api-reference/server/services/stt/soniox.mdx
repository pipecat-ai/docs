...
<ParamField path="should_interrupt" type="bool" default="True">
  ...
  Soniox's built-in endpoint detection. The value is passed to the user turn
  strategies the service recommends, and those strategies do the interrupting. A
  `user_turn_strategies` you pass yourself replaces that recommendation and this
  setting with it.
</ParamField>
...
- **Turn detection modes**:
  ...
  - **Soniox mode** (`vad_force_turn_endpoint=False`): Soniox endpoint detection is enabled and controls turn endings. The service proposes each turn boundary based on Soniox's detection, and the user turn strategies resolve those proposals into `UserStartedSpeakingFrame` and `UserStoppedSpeakingFrame`. The turn opens on the local VAD signal when a VAD analyzer is configured (most responsive) or on the first transcript token otherwise. The `should_interrupt` parameter (default `True`) controls whether the bot is interrupted when the user starts speaking, and the strategies do the interrupting: pinning your own `user_turn_strategies` drops that setting, so use `ExternalUserTurnStrategies(enable_interruptions=False)` instead.
