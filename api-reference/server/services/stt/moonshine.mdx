...
- **Language Support**: Moonshine supports Arabic, Chinese, English, German, Japanese, Korean, Spanish, Tagalog, Ukrainian, and Vietnamese
...
| ---------- | ----------------- | ----------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`    | `str \| Model`    | `Model.SMALL_STREAMING` | Moonshine model architecture. Available models: `TINY`, `BASE`, `TINY_STREAMING`, `BASE_STREAMING`, `SMALL_STREAMING` (default), `MEDIUM_STREAMING`. English publishes the widest range and is the only language with `MEDIUM_STREAMING`; most other languages ship one to four models. An unavailable architecture falls back to the best model for that language. |
| `language` | `Language \| str` | `Language.EN`           | Language for transcription. Moonshine publishes models for Arabic, Chinese, English, German, Japanese, Korean, Spanish, Tagalog, Ukrainian, and Vietnamese; regional variants (e.g., `Language.ES_MX`) resolve to their base code. _(Inherited from base STT settings.)_                                                                                            |
...
- **Model validation**: The service validates the `model` parameter at construction time. A missing or blank model raises a `ValueError` before attempting to load.
...
- **Language support**: Moonshine supports Arabic, Chinese, English, German, Japanese, Korean, Spanish, Tagalog, Ukrainian, and Vietnamese. Regional variants (e.g., `Language.ES_MX`) resolve to their base code. The non-English models are released under the non-commercial Moonshine Community License.
