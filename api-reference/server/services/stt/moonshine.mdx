...
- **Language support**: Moonshine supports Arabic, Chinese, English, German, Japanese, Korean, Spanish, Tagalog, Ukrainian, and Vietnamese. Regional variants (e.g., `Language.ES_MX`) resolve to their base code. Models are MIT-licensed in every language and size, except the legacy non-streaming models for languages other than English (`TINY` and `BASE`), which are under the non-commercial [Moonshine Community License](https://www.moonshine.ai/license).
