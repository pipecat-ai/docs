...
description: "Offline STT with WhisperSTTService: OpenAI Whisper models running locally, with multiple sizes and hardware acceleration."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/transcription/transcription-whisper.py"
  >
  ...
  <Card
    ...
    icon="microphone"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/transcription/transcription-whisper-mlx.py"
  >
...
<ParamField
  ...
  deprecated
>
  Whisper model to use. Can be a `Model` enum value or a string. Multilingual
  models: `TINY`, `BASE`, `SMALL`, `MEDIUM`, `LARGE` (large-v3),
  `LARGE_V3_TURBO`, `DISTIL_LARGE_V2`. English-only model: `DISTIL_MEDIUM_EN`.
...
| ---------------- | ----------------- | ------------------------ | ------------------------------------------------------------------------- |
| `model`          | `str`             | `Model.DISTIL_MEDIUM_EN` | Whisper model to use. _(Inherited from base STT settings.)_               |
| `language`       | `Language \| str` | `Language.EN`            | Default language for transcription. _(Inherited from base STT settings.)_ |
| `no_speech_prob` | `float`           | `0.4`                    | Probability threshold for filtering out non-speech segments.              |
| `hotwords`       | `str \| None`     | `None`                   | Words or phrases to bias transcription towards (space-separated).         |
...
| Parameter        | Type              | Default         | Description                                                               |
| ---------------- | ----------------- | --------------- | ------------------------------------------------------------------------- |
| `model`          | `str`             | `MLXModel.TINY` | MLX Whisper model to use. _(Inherited from base STT settings.)_           |
| `language`       | `Language \| str` | `Language.EN`   | Default language for transcription. _(Inherited from base STT settings.)_ |
...
- **Model validation**: Both services validate the `model` parameter at construction time. A missing or blank model raises a `ValueError` before attempting to load. For `WhisperSTTServiceMLX`, which loads models on demand, the check ensures the model name is valid without blocking on the download.
...
- **English-only model validation**: Models with `.en` in their name (including the default `DISTIL_MEDIUM_EN`) only support English. Constructing the service with an English-only model and a non-English language raises a `ValueError` naming the model and its supported languages. Use a multilingual model (e.g., `LARGE_V3_TURBO`) for non-English transcription.
