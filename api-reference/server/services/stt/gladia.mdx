...
sidebarTitle: "Gladia"
description: "GladiaSTTService streams real-time STT over Gladia's WebSocket API: 99+ languages, custom vocabulary, translation, and sentiment."
---
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/voice/voice-gladia.py"
  >
...
<ParamField path="should_interrupt" type="bool" default="True">
  Whether the bot should be interrupted when Gladia VAD detects user speech. The
  value is passed to the user turn strategies the service recommends, and those
...
- **Built-in VAD**: Set `enable_vad=True` in Settings to use Gladia's server-side VAD. The service proposes each turn boundary and automatically requests `ExternalUserTurnStrategies`, which resolve those proposals into `UserStartedSpeakingFrame` and `UserStoppedSpeakingFrame`, so you don't need to configure turn strategies manually. Pass your own `user_turn_strategies` only to override this. When using Gladia's VAD, do not enable another VAD in your pipeline.
- **Who interrupts the bot**: the turn strategies own the interruption, not the service. `should_interrupt` (default `True`) rides along on the strategies the service recommends, so passing `should_interrupt=False` is all you need to stop barge-in. If you pass your own `user_turn_strategies`, the recommendation is dropped and `should_interrupt` goes with it: use `ExternalUserTurnStrategies(enable_interruptions=False)` instead.
