...
3. **Model Access**:
   - `SarvamSTTService`: Access to Saarika (STT) or Saaras (STT-Translate) models, including the `saaras:v3` and `saaras:v4` models with support for multiple modes (transcribe, translate, verbatim, translit, codemix)
...
<ParamField path="model" type="str" default="saaras:v4" deprecated>
  ...
  (advanced, supports mode and fine-grained VAD), `"saaras:v4"` (advanced,
  supports mode and fine-grained VAD, adds Global English support). *Deprecated
...
| `language`                      | `Language \| str` | `None`  | Target language for transcription. _(Inherited from base STT settings.)_ Behavior varies by model: `saarika:v2.5` defaults to "unknown" (auto-detect), `saaras:v2.5` ignores this (auto-detects), `saaras:v3` and `saaras:v4` default to "unknown" (auto-detect). |
...
| `positive_speech_threshold`     | `float`           | `None`  | VAD probability threshold (0.0-1.0) above which a frame is considered speech. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                           |
| `negative_speech_threshold`     | `float`           | `None`  | VAD probability threshold (0.0-1.0) below which a frame is considered silence. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                          |
| `min_speech_frames`             | `int`             | `None`  | Minimum consecutive speech frames to start a speech segment. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                            |
| `first_turn_min_speech_frames`  | `int`             | `None`  | Minimum speech frames for the first user turn. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                                          |
| `negative_frames_count`         | `int`             | `None`  | Number of silence frames within the window to end a speech segment. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                     |
| `negative_frames_window`        | `int`             | `None`  | Sliding window size (in frames) for counting negative frames. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                           |
| `start_speech_volume_threshold` | `float`           | `None`  | Volume level (dB) below which audio is too quiet to be speech. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                          |
| `interrupt_min_speech_frames`   | `int`             | `None`  | Minimum speech frames to register a barge-in/interruption. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                              |
| `pre_speech_pad_frames`         | `int`             | `None`  | Number of audio frames to prepend before detected speech onset. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                         |
| `num_initial_ignored_frames`    | `int`             | `None`  | Number of leading audio frames to skip at connection start. Only for models with fine-grained VAD support (`saaras:v3`, `saaras:v4`).                                                                                                                             |
...
- **Default model changed**: As of this update, the default model is `saaras:v4` (previously `saaras:v3`). Applications that relied on the previous default should set `settings=SarvamSTTService.Settings(model="saaras:v3")` explicitly.
...
- **Model-specific parameter validation**: The service validates that parameters are compatible with the selected model. For example, `prompt` is only supported with `saaras:v2.5`, `language` is not supported with `saaras:v2.5` (which auto-detects language), and the fine-grained VAD parameters are only supported with models that have VAD capabilities (`saaras:v3`, `saaras:v4`).
- **Fine-grained VAD tuning**: The `saaras:v3` and `saaras:v4` models support server-side VAD with 10 tuning parameters for speech detection thresholds, frame-count controls, pre-speech padding, interruption sensitivity, and initial-frame skipping. These parameters are only available with models that support fine-grained VAD.
