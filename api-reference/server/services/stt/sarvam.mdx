...
- **VAD modes**: When `vad_signals=False` (default), the service relies on Pipecat's local VAD and flushes the server buffer on `VADUserStoppedSpeakingFrame`. When `vad_signals=True`, the service uses Sarvam's server-side VAD and proposes turn starts and stops from the server signals. The user turn strategies resolve those proposals into the speaking frames and own the interruption, so `should_interrupt` is the control to reach for. Pinning your own `user_turn_strategies` drops it: use `ExternalUserTurnStrategies(enable_interruptions=False)` instead.
