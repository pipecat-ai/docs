...
- **VAD modes**: When `vad_signals=False` (default), the service relies on Pipecat's local VAD and flushes the server buffer on `VADUserStoppedSpeakingFrame`. When `vad_signals=True`, the service uses Sarvam's server-side VAD and proposes turn starts and stops from the server signals, which the [user turn strategies](/api-reference/server/utilities/turn-management/user-turn-strategies) resolve.
