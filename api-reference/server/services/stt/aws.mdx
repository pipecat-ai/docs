---
title: "AWS Transcribe Speech-to-Text"
sidebarTitle: "AWS Transcribe"
description: "AWSTranscribeSTTService streams real-time STT over Amazon Transcribe's WebSocket API, with interim results and multi-language support."
---
...
| `partial_results_stability` | `"high" \| "medium" \| "low"` | `"high"`      | How stable interim results should be, trading accuracy against latency: `"low"` is the most accurate, `"high"` the fastest. AWS binds this to a connection, so updating it at runtime triggers a reconnect. See [AWS partial result stabilization docs](https://docs.aws.amazon.com/transcribe/latest/dg/streaming.html#streaming-partial-result-stabilization). |
...
- **Partial results stabilization**: Defaults to `"high"` stability (fastest response, less stable interim results). Configure via `partial_results_stability` in Settings to trade latency for accuracy. Updating it mid-conversation triggers a reconnect.
