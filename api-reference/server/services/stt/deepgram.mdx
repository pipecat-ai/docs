...
| `profanity_filter` | `bool \| None`    | `None`             | Filter profanity from transcripts. When `None`, Deepgram's default (off) applies. |
...
- **Profanity filter default**: The `profanity_filter` setting defaults to `None`, meaning Deepgram's own default (off) applies. Deepgram's filter rewrites matched words rather than tagging them, so a false positive can corrupt transcripts. To enable filtering, pass `settings=DeepgramSTTService.Settings(profanity_filter=True)`.
