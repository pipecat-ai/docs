...
- **Turn management**: Flux provides its own turn detection via `StartOfTurn`/`EndOfTurn` events. It proposes each turn boundary instead of pushing the turn frames itself, and the [user turn strategies](/api-reference/server/utilities/turn-management/user-turn-strategies) resolve that proposal and own the interruption. It automatically requests `ExternalUserTurnStrategies` at start to avoid conflicting VAD-based turn management. See [User Turn Strategies](/api-reference/server/utilities/turn-management/user-turn-strategies) for the interruption controls.
