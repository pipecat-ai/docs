...
<ParamField path="should_interrupt" type="bool" default="True">
  Whether the bot should be interrupted when Flux detects user speech. The value
  is passed to the user turn strategies the service recommends, and those
  strategies do the interrupting. A `user_turn_strategies` you pass yourself
  replaces that recommendation and this setting with it.
</ParamField>
...
| `numerals`            | `bool`            | `None`              | Convert spoken numbers to numeral form (e.g. "twenty three" → "23"). Connection-time only; cannot be updated mid-stream.                                  |            |
...
<Note>
  The split between proposing a turn and resolving it landed after v1.7.0. On
  v1.7.0 and earlier the service pushed the turn frames and broadcast the
  interruption itself, so `should_interrupt` was the only control.
</Note>

- **Turn management**: Flux provides its own turn detection via `StartOfTurn`/`EndOfTurn` events. The service does not push the turn frames itself. It proposes each turn boundary, and the user turn strategies resolve that proposal into the real `UserStartedSpeakingFrame`/`UserStoppedSpeakingFrame`. The service automatically requests `ExternalUserTurnStrategies` at start to avoid conflicting VAD-based turn management.
- **Who interrupts the bot**: the turn strategies own the interruption, not the service. `should_interrupt` (default `True`) rides along on the strategies the service recommends, so passing `should_interrupt=False` is all you need to stop barge-in. If you pass your own `user_turn_strategies`, the recommendation is dropped and `should_interrupt` goes with it: use `ExternalUserTurnStrategies(enable_interruptions=False)` instead, or drop the manual `user_turn_strategies` and let the service recommend them.
...
<ParamField path="should_interrupt" type="bool" default="True">
  Whether to interrupt the bot when Flux detects user speech. The value is
  passed to the user turn strategies the service recommends, and those
  strategies do the interrupting. A `user_turn_strategies` you pass yourself
  replaces that recommendation and this setting with it.
</ParamField>
