...
- **Non-blocking connection**: The SageMaker BiDi connection is established in a background task, so pipeline startup is not blocked if the connection is slow or fails. The first bot turn (e.g., a greeting) can proceed while the STT service connects in the background. Connection failures surface via the `on_connection_error` event handler.
