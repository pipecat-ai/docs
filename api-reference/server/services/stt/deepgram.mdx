...
<ParamField path="enable_eager_end_of_turn" type="bool" default="False">
  ...
  the gap between the two is spent generating a response rather than waiting. The
  response is held until the turn is confirmed, and discarded if the user resumes
  speaking or the committed transcript differs from the predicted one. Requests
  `EagerUserTurnStrategies` in place of `ExternalUserTurnStrategies` when
  enabled. See [User Turn
