...
<ParamField path="should_interrupt" type="bool" default="True">
  ...
  The value is passed to the user turn strategies the service recommends, and
  those strategies do the interrupting. A `user_turn_strategies` you pass
  yourself replaces that recommendation and this setting with it.
</ParamField>
...
- **No runtime model switching**: Unlike the v1 API, the ink-2 model does not support runtime model or language switching. However, `keyterm` can be updated at runtime via `STTUpdateSettingsFrame`, which triggers a reconnect.
...
- **Interruption support**: `should_interrupt` (default `True`) decides whether the bot is interrupted when the user starts speaking. The turn strategies own the interruption, not the service: the value rides along on the strategies the service recommends. If you pass your own `user_turn_strategies`, that recommendation is dropped and `should_interrupt` goes with it, so use `ExternalUserTurnStrategies(enable_interruptions=False)` instead, or drop the manual `user_turn_strategies` and let the service recommend them.
