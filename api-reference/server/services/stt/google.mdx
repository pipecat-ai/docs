...
| `denoiser_config`                      | `dict \| DenoiserConfig`     | `None`             | Background-noise removal config. Pass as a dict (e.g., `{"denoise_audio": True}`) or `DenoiserConfig` message. Only Chirp 3 models support denoising — see [Google's documentation](https://cloud.google.com/speech-to-text/docs/models/chirp-3). |
