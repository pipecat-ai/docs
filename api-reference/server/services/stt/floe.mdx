...
Unlike the LLM and TTS legs, streaming STT is **keyless-only** — it runs on
Floe's managed Deepgram key, so there is no per-request BYOK (`provider_key`) on
this leg. Metering and spend caps still apply.
...
<ParamField
  path="base_url"
  type="str"
  default="wss://credit-api.floelabs.xyz/v1/audio/transcriptions/stream"
>
