...
sidebarTitle: "OpenAI"
description: "STT with OpenAIRealtimeSTTService for live streaming and OpenAISTTService for segmented transcription of OpenAI speech models."
---
...
- **`OpenAISTTService`** (HTTP) -- VAD-segmented speech recognition using OpenAI's transcription API, supporting GPT transcription and Whisper models.
...
<ParamField path="model" type="str" default="gpt-transcribe" deprecated>
  Transcription model to use. Defaults to `"gpt-transcribe"`. *Deprecated in
  v0.0.105. Use `settings=OpenAISTTService.Settings(...)` instead.*
</ParamField>
...
<ParamField path="http_client" type="AsyncHTTPClient" default="None">
  Custom async HTTP client for API requests, e.g. one with a longer request
  timeout. Prefer `openai.DefaultAsyncHttpxClient`, which keeps the SDK's
  connection limits and redirect handling; a bare client uses its own defaults
  instead. The SDK's HTTP client family follows the installed `openai` version,
  so build any `Timeout` you pass it from that same family. Defaults to `None`,
...
| ------------- | ----------------- | ------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| `model`       | `str`             | `"gpt-transcribe"` | Transcription model to use. _(Inherited from base STT settings.)_                                                         |
...
| `keywords`    | `list[str]`       | `None`             | Words or phrases that guide transcription of the input audio, such as product names or acronyms. Supported by `"gpt-transcribe"`. |
...
- **Probability metrics**: With `include_prob_metrics=True`, GPT transcription models report per-token logprobs and Whisper models report per-segment logprobs. Diarization models support neither, so they report no probabilities.
- **Multilingual support**: Whisper and GPT transcription models support many languages. The default is `Language.EN` (English). Set `language=None` in settings to enable automatic language detection, which will transcribe whatever language the user speaks.
...
<ParamField path="model" type="str" default="gpt-realtime-whisper" deprecated>
  ...
  `"gpt-realtime-whisper"`. Other supported models include `"gpt-transcribe"`
  and `"gpt-live-transcribe"`. *Deprecated in v0.0.105. Use
...
<ParamField path="turn_detection" type="dict | Literal[False]" default="False">
  Server-side VAD configuration. Defaults to `False` (disabled), which relies on a local VAD processor in the pipeline. Pass a dict to enable it: at minimum `{"type": "server_vad"}`, optionally with custom settings (e.g. `{"type": "server_vad", "threshold": 0.5}`). Requires a model that supports turn detection — `gpt-transcribe` does, the default `gpt-realtime-whisper` does not. `None` omits the field entirely, leaving the session's own default in place.
</ParamField>
...
<ParamField path="should_interrupt" type="bool" default="True">
  ...
  Only applies when turn detection is enabled. See [User Turn
...
| `delay`    | `Literal["minimal", "low", "medium", "high", "xhigh"]` | `None`                   | How long the model waits before emitting transcription text, trading latency for accuracy. Only supported by `"gpt-realtime-whisper"`. |
...
- **Local VAD vs Server-side VAD**: Defaults to local VAD mode (`turn_detection=False`), where a local VAD processor in the pipeline controls when audio is committed for transcription. Pass `turn_detection={"type": "server_vad"}` for server-side VAD, but do not use a separate VAD processor in the pipeline in that mode.
- **Turn management**: With server-side VAD enabled, the service proposes each turn boundary and automatically requests `ExternalUserTurnStrategies`, so you don't need to configure turn strategies manually. Those strategies resolve the proposals into `UserStartedSpeakingFrame` and `UserStoppedSpeakingFrame` and own the interruption. See [User Turn Strategies](/api-reference/server/utilities/turn-management/user-turn-strategies) for the interruption controls.
...
- **Model selection**: The default `"gpt-realtime-whisper"` model provides low-latency streaming transcription but supports neither the `prompt` parameter nor server-side turn detection. If you need prompt hints for keywords or style guidance, use `"gpt-transcribe"` or `"gpt-live-transcribe"` instead; for server-side VAD, use `"gpt-transcribe"`.
- **Multilingual support**: GPT transcription models support many languages. The default is `Language.EN` (English). Set `language=None` in settings to enable automatic language detection, which will transcribe whatever language the user speaks.
