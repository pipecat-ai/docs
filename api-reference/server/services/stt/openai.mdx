...
<ParamField path="http_client" type="httpx.AsyncClient" default="None">
  Custom `httpx.AsyncClient` for API requests, e.g. one with a longer request
  timeout. Prefer `openai.DefaultAsyncHttpxClient`, which keeps the SDK's
  connection limits and redirect handling; a bare `httpx.AsyncClient` uses
  httpx's defaults instead. Defaults to `None`, which lets the SDK build its own
  client.
</ParamField>
