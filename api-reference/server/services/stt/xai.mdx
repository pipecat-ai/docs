...
sidebarTitle: "xAI"
description: "XAISTTService streams real-time STT over xAI's WebSocket API with interim results, configurable endpointing, and multichannel audio."
---
...
<ParamField path="sample_rate" type="int | None" default="None">
  ...
  48000. If None, uses the input sample rate from the start frame.
</ParamField>
...
| ----------------- | ----------------- | ----------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`           | `str`             | `"grok-voice-transcribe-2.0"` | xAI model to use for transcription. _(Inherited from base STT settings.)_                                                                                   |
...
| `endpointing`     | `int \| None`     | `None`                        | Silence duration in milliseconds that triggers the speech-final event carrying the utterance's `TranscriptionFrame`. Range 0-5000. Server default is 400ms. |
...
- **Connection management**: The WebSocket connection is persistent and automatically reconnects if it drops mid-session. Audio is streamed continuously and the server emits `transcript.partial` events with `is_final` and `speech_final` flags. Interim results (`is_final=false`) and chunk finals (`is_final=true, speech_final=false`) are pushed as `InterimTranscriptionFrame`. Only the utterance final (`speech_final=true`) is pushed as `TranscriptionFrame`, because it restates the entire utterance, including the text of every earlier chunk final.
