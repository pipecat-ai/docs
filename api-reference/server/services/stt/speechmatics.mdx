...
<CardGroup cols={2}>
  <Card
    title="Speechmatics Agent STT Reference"
    icon="code"
    href="https://docs.speechmatics.com/api-ref/agent-stt-websocket"
  >
  ...
  <Card
    ...
    icon="book"
    href="https://docs.speechmatics.com/integrations-and-sdks/pipecat/stt"
  >
...
| ------------------------ | ---------------------------- | ------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `model`                  | `str`                        | `"linden-1"`  | Transcription model to use. `linden-1` is the only model Agent STT accepts. _(Inherited from base STT settings.)_ Supersedes `operating_point`.                           |
...
| `operating_point`        | `Model \| str`               | `None`        | _Deprecated since 1.10.0._ Resolves to `model`, warning when used. If both are given they must name the same value, otherwise a `ValueError` is raised. Removed in 2.0.0. |
| `enable_partials`        | `bool`                       | `None`        | Emit interim segments as `AddPartialSegment`, updating until the final segment arrives. Agent STT enables partials when this is unset.                                    |
