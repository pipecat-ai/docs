...
<Note>
  ...
  boundary and those strategies resolve it, so `should_interrupt` is the control
  for barge-in. See [User Turn
...
| `include_results`                  | `bool`                       | `None`        | Include word-level results in transcript messages.                                                                                                           |
