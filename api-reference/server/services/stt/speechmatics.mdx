...
description: "SpeechmaticsSTTService delivers real-time STT with partial and final results, speaker diarization, and end-of-utterance detection."
---
...
<Note>
  ...
  `TurnDetectionMode.ADAPTIVE` or `TurnDetectionMode.SMART_TURN`, the service
  automatically requests `ExternalUserTurnStrategies` at start, so you don't
  need to configure turn strategies manually. The service proposes each turn
  boundary and those strategies resolve it into a `UserStartedSpeakingFrame` or
  `UserStoppedSpeakingFrame` and own the interruption, so `should_interrupt` is
  the control for barge-in. Pass your own `user_turn_strategies` only to
  override the service's recommendation, and remember that doing so drops
  `should_interrupt`: use
  `ExternalUserTurnStrategies(enable_interruptions=False)` instead. See [User
  Turn
...
<ParamField path="should_interrupt" type="bool" default="True">
  ...
  SMART_TURN). The value is passed to the user turn strategies the service
  recommends, and those strategies do the interrupting. A `user_turn_strategies`
  you pass yourself replaces that recommendation and this setting with it.
</ParamField>
