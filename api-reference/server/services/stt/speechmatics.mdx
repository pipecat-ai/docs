...
<Note>
  ...
  `TurnDetectionMode.VAD` (the default), the service runs its own VAD and
  closes turns itself, automatically requesting `ExternalUserTurnStrategies` at
  start. The service proposes each turn boundary and those strategies resolve
  it, so `should_interrupt` is the control for barge-in. See [User Turn
...
<ParamField path="base_url" type="str" default="None">
  ...
  environment variable, then defaults to `wss://eu2.rt.speechmatics.com/v2/agent`.
</ParamField>
