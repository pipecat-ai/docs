...
<Note>
  ...
  automatically requests `ExternalUserTurnStrategies` at start, so you don't need
  to configure turn strategies manually. Pass your own `user_turn_strategies` only
  to override the service's recommendation. See [User Turn
