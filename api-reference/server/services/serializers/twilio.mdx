...
sidebarTitle: "Twilio"
description: "TwilioFrameSerializer integrates Pipecat with Twilio's Media Streams WebSocket protocol for phone call audio."
---
...
- **Region and edge pairing**: When `base_url` is not provided, both `region` and `edge` must be specified together if either is set. Twilio's API uses the FQDN format `api.{edge}.{region}.twilio.com`. This requirement does not apply when `base_url` is provided, since `base_url` is used verbatim and ignores `region`/`edge`.
