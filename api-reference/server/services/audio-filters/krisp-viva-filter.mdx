---
title: "Krisp VIVA Audio Filter"
sidebarTitle: "Krisp VIVA"
description: "KrispVivaFilter isolates the user's voice in real-time audio using the Krisp VIVA model in Pipecat pipelines."
---
...
<ParamField path="noise_suppression_level" type="float" default="100.0">
...
<ParamField path="api_key" type="str" default='""'>
  ...
  back to the `KRISP_VIVA_API_KEY` environment variable. When multiple Krisp
  components are used in the same process, only the API key from the first
  component initialized is used.
</ParamField>
...
The Krisp SDK is initialized once per process on first use and remains active for the process lifetime. If multiple Krisp components (filter, VAD, turn detection) are used in the same process, they share the SDK instance and only the first component's `api_key` is used.
