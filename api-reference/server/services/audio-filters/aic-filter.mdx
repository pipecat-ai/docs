...
<Warning>
  **Deprecated in 1.4.0**: This method will be removed in Pipecat 1.6.0. Use
  [`AICQuailVADAnalyzer`](/api-reference/server/services/vad/aic-quail-vad-analyzer)
  instead, which provides a standalone Quail VAD 2.0 model that works
  independently of the enhancement filter.
</Warning>
...
- **For VAD**: Use [`AICQuailVADAnalyzer`](/api-reference/server/services/vad/aic-quail-vad-analyzer) instead of the deprecated `create_vad_analyzer()` method
