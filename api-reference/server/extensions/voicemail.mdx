...
Closed gates still admit system frames and worker lifecycle frames (including `EndWorkerFrame`), so event handlers can control the worker state regardless of gate position.
