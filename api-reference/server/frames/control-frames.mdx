...
<Note>
  To read a service's current settings, use the read-only `settings` property on
  the service instance: `service.settings.model`. Settings are changed through a
  `ServiceUpdateSettingsFrame`, which applies the change and keeps the service's
  metrics in sync.
</Note>
