...
ControlFrames signal boundaries, state changes, and configuration updates within the pipeline. They are queued and processed in order alongside DataFrames. ControlFrames are cancelled on `InterruptionFrame` unless their `interruptible` field is False. See the [frames overview](/api-reference/server/frames/overview) for base class details and the full frame hierarchy.
...
This frame is uninterruptible (`interruptible: bool = field(default=False, init=False)`), so it cannot be cancelled by `InterruptionFrame`.
...
This frame is uninterruptible (`interruptible: bool = field(default=False, init=False)`).
...
<ParamField path="skip_tts" type="bool | None" default="None">
  Whether the response should be skipped by the TTS service.
</ParamField>
...
<ParamField path="summarization_timeout" type="float | None">
...
This frame is uninterruptible (`interruptible: bool = field(default=False, init=False)`) to ensure the result is never dropped.
...
<ParamField path="llm" type="str | None" default="None">
...
This frame is uninterruptible (`interruptible: bool = field(default=False, init=False)`), ensuring it reaches downstream processors even during interruption.
...
<ParamField path="cancel_on_interruption" type="bool" default="False">
  ...
  `False`, the call is treated as asynchronous: the conversation is not held
  while it runs, and a result that arrives after the conversation has moved on
  (a new user message, developer message, or intermediate update from the call)
  is injected via a developer message. Assistant messages do not defer the
  result. A result that arrives before then settles in place like a synchronous
  call's.
</ParamField>
...
<ParamField path="context_id" type="str | None">
  Identifier linking this TTS output to its originating context.
</ParamField>

<ParamField path="append_to_context" type="bool" default="True">
  Whether the spoken text for this response will be appended to the LLM context.
  When `True`, the assistant aggregator will track this output as an assistant
  turn, firing `on_assistant_turn_started` and `on_assistant_turn_stopped`
  events.
</ParamField>
...
<ParamField path="service" type="FrameProcessor | None" default="None">
  Target a specific service instance. When `None`, the frame applies to the
  first matching service in the pipeline.
</ParamField>

<ParamField path="reach_inactive_services" type="bool" default="False">
  Whether these settings should reach every service a `ServiceSwitcher` manages,
  rather than only the active one. Set this for provider-neutral settings that
  must survive a service switch.
</ParamField>
...
<Note>
  To read a service's current settings, use the read-only `settings` property
  on the service instance: `service.settings.model`. Settings are changed
  through a `ServiceUpdateSettingsFrame`, which applies the change and keeps
  the service's metrics in sync.
</Note>
...
Instructs `AudioBufferProcessor` to start recording audio. This frame can be pushed through the pipeline to start recording on demand. Alternatively, use `AudioBufferProcessor.start_recording()` or enable `auto_start_recording` in the constructor.

This frame is uninterruptible (`interruptible: bool = field(default=False, init=False)`).

### AudioBufferStopRecordingFrame

Instructs `AudioBufferProcessor` to stop recording and flush any buffered audio. After flushing, triggers the `on_recording_stopped` event.
...
Request graceful pipeline shutdown. The pipeline worker converts this into an `EndFrame` and pushes it downstream. Inherits from `WorkerFrame` and is uninterruptible (`interruptible: bool = field(default=False, init=False)`).
...
Request pipeline stop while keeping processors alive. Converted to a `StopFrame` downstream. Inherits from `WorkerFrame` and is uninterruptible (`interruptible: bool = field(default=False, init=False)`).
