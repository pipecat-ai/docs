...
<ParamField path="broadcast_sibling_id" type="int | None">
  ID of the paired frame when a frame is broadcast in both directions. Set
  automatically by `broadcast_frame()` and `broadcast_frame_instance()`.
</ParamField>
...
<ParamField path="interruptible" type="bool" default="True">
  Whether an interruption may drop this frame from a processor's queue or cancel
  its processing. True by default. A frame class such as `EndFrame` declares
  `interruptible: bool = field(default=False, init=False)` to be uninterruptible
  by default. Set it on a frame before pushing it to decide for that frame alone.
</ParamField>
...
<Warning>
  **Deprecated.** Use the `interruptible` field with `default=False` instead. `UninterruptibleFrame` will be removed in 2.0.0.
</Warning>

Occasionally a `DataFrame` or `ControlFrame` is too important to discard during an interruption. To protect a frame, declare the `interruptible` field with `default=False` in the frame class:
...
You can also set the field on an individual frame to override the class default:

```python
frame = TextFrame(text="Important")
frame.interruptible = False  # This specific frame survives interruption
await self.push_frame(frame)
```

Examples of uninterruptible frames: `EndFrame`, `StopFrame`, `FunctionCallResultFrame`, `FunctionCallInProgressFrame`
...
Your frame travels the pipeline like any built-in frame, in the direction you push it (see [Frame Direction](#frame-direction)). If the frame must survive a user interruption, declare `interruptible` with `default=False`:
...
Push `EndWorkerFrame` **downstream** to gracefully shut down the pipeline. Pair it with a `TTSSpeakFrame` to say goodbye first:

```python
await aggregator.push_frame(
    TTSSpeakFrame("It seems like you're busy. Have a nice day!")
)
await aggregator.push_frame(EndWorkerFrame(), FrameDirection.DOWNSTREAM)
```

Downstream is `push_frame`'s default direction, so `await aggregator.push_frame(EndWorkerFrame())` is equivalent. Pushing it downstream is what lets the goodbye reach the TTS service and the transport before the pipeline ends — the frame queues behind that work instead of jumping ahead of it.

<Tip>
  See [Pipeline Termination](/pipecat/learn/pipeline-termination) for the
  difference between `EndFrame` and `EndWorkerFrame`, [why the direction
  matters](/pipecat/learn/pipeline-termination#incorrect-termination-frame-direction),
