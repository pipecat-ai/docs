...
<ParamField path="interruptible" type="bool" default="True">
  ...
  by default. Set it on a frame before pushing it to decide for that frame
  alone.
</ParamField>
...
<Warning>
  **Deprecated.** Use the `interruptible` field with `default=False` instead.
  `UninterruptibleFrame` will be removed in 2.0.0.
</Warning>
