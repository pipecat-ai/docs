---
title: "LLM Frames"
description: "LLM frame reference for Pipecat: context frames and the function calling helper dataclasses passed through pipelines."
---
...
<ParamField path="speculation" type="bool" default="False">
  Whether this inference is speculative, run from a provisional context that is
  not part of the conversation. Its response must not reach the user or the
  context until the turn is confirmed, and the service must not execute tool
  calls for it.
</ParamField>
...
<ParamField path="is_final" type="bool" default="True">
  Whether this is the final result for the function call. Set to `False` to send
  an intermediate update from an async function call registered with
