...
<Note>
  `PipelineTask` is a `PipelineWorker`, the worker type that runs a pipeline.
  It builds on [`BaseWorker`](/api-reference/server/workers/base-worker), so it
  can take part in a multi-worker system. See [Workers](/api-reference/server/workers/base-worker)
  for the full worker model.
</Note>
