...
<Note>
  Heartbeat monitoring must be enabled by setting `enable_heartbeats=True` in `PipelineParams`. The timeout period is controlled by `heartbeats_monitor_secs` (default: 5 seconds). See [PipelineParams](/api-reference/server/pipeline/pipeline-params) for configuration details.
</Note>
