...
description: "Monitor Pipecat lifecycle changes with the event system: frame processor and service events, and how to subscribe."
---
...
Events on [`PipelineWorker`](/api-reference/server/pipeline/pipeline-worker#event-handlers).
...
| `on_recording_started`    | Recording started (state transitioned to active)          |
| `on_recording_stopped`    | Recording stopped (after final buffered audio is emitted) |
...
| `on_voicemail_detected`    | Voicemail detected and the greeting ended  |
