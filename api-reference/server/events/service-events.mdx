...
<Note>
  ...
  retries, 4-10s waits by default) when connection errors occur. The
  `on_connection_error` event fires for each failed attempt. Configure the wait
  times via `reconnect_backoff_min_wait` and `reconnect_backoff_max_wait`
