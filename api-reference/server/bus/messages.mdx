...
description: "Bus message types for Pipecat multi-agent communication: the pub/sub primitives, organized by priority level."
---
...
The client's screen snapshots and its requests to cancel a job group also arrive as `BusUIEventMessage`s, under reserved event names. A `UIWorker` handles both itself and never dispatches them to `@ui_event` handlers. The names are public for code that needs to recognize these events:

```python
from pipecat.bus.ui import UI_SNAPSHOT_EVENT_NAME, UI_CANCEL_JOB_GROUP_EVENT_NAME
```

| Constant                         | Value                  | Description                                                     |
| -------------------------------- | ---------------------- | --------------------------------------------------------------- |
| `UI_SNAPSHOT_EVENT_NAME`         | `"__ui_snapshot"`      | Reserved event name for the client's screen snapshot            |
| `UI_CANCEL_JOB_GROUP_EVENT_NAME` | `"__cancel_job_group"` | Reserved event name for the client asking to cancel a job group |
