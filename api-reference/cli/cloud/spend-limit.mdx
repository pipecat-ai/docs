...
Reports the limit, current spend, the billing period it covers, which
organization the data belongs to, and whether the organization is currently
blocked. An organization with no limit set shows `no limit set` rather than an
error.

When using `--output json`, the response includes an `organization` field
identifying which organization the limit applies to. An organization with no
spend-limit record returns an empty object (`{}`), omitting the `organization`
field to preserve the empty-object sentinel that consumers test for.
