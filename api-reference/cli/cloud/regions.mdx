...
<ParamField path="region_key" type="string" required>
  The region's key, e.g. `pce-acme-us-east`. Region keys start with `pce-` and
  are unique within your organization. They must be DNS-safe: lowercase letters,
  digits and dashes, at most 63 characters, not ending in a dash. Use the same
  key for the life of the region, including across cluster rebuilds.
</ParamField>
