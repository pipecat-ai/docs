...
<ParamField path="region_key" type="string" required>
  The region's key: a lowercase DNS label starting with `pce-`, e.g.
  `pce-acme-us-east`. Region keys are unique within your organization and must
  be DNS-safe. Use the same key for the life of the region, including across
  cluster rebuilds.
</ParamField>
