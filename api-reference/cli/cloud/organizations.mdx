...
<Tip>
  With `--output json`, the key, the registry host and the login command are
  emitted on stdout as `key`, `registryHost` and `helmLoginCommand`.
</Tip>

### registry-keys list

List registry keys with their lifecycle metadata. Key material is never shown. The **Region** column names the self-hosted region whose cluster holds a key, and **Status** is `active`, `expired` or `revoked`. Only active keys are listed unless you pass `--all`.
