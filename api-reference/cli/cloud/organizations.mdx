...
The `registry-keys` sub-commands manage pull-only registry credentials. A registry key lets `helm` on your workstation fetch the region package for a [self-hosted region](/pipecat-cloud/enterprise/setup). Keys are org-scoped, revocable, and shown exactly once at mint.

An organization can hold ten active keys that its members minted. Keys that are revoked or expired do not count toward that limit, and neither do the keys each self-hosted region's cluster holds. Your cluster receives its own key during enrollment; the region renews that key and retires it when the region is deleted. Keys minted with the CLI do not expire, so `expired` belongs to the previous keys a region leaves behind each time it renews.

### registry-keys mint

Mint a registry key and print the matching `helm registry login` command. The printed command logs in to the registry of the environment you minted against and passes the key to `helm` on stdin, so it never appears in the process list. It needs a POSIX shell such as bash or zsh.
...
<Tip>
  With `--output json`, the key, the registry username, the registry host and
  the login command are emitted on stdout as `key`, `username`, `registryHost`
  and `helmLoginCommand`, alongside the key's `id` and `name`.
</Tip>

### registry-keys list

List registry keys with their lifecycle metadata. Key material is never shown. The **Region** column names the self-hosted region whose cluster holds a key, and **Status** is `active`, `expired` or `revoked`. Revoked and expired keys are hidden unless you pass `--all`.
...
<ParamField path="--all" type="boolean" default="false">
  Include revoked and expired keys. The same filter applies to `--output json`,
  which also reports how many keys it hid as `hidden`.
</ParamField>

<ParamField path="--organization / -o" type="string">
  Organization to list keys for. If not provided, uses the current organization
...
An active key held by a region cannot be revoked on its own while the region exists, because it is the cluster's credential for pulling images: delete the region to revoke it. The expired keys a region leaves behind as it renews can be revoked.
