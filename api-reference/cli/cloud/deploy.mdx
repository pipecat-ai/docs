---
title: "pipecat cloud deploy"
sidebarTitle: "deploy"
description: "pipecat cloud deploy creates or updates a Pipecat Cloud agent deployment, building a deployment manifest from your options."
---
...
In addition to this flag, you also need to enable the `KrispVivaFilter()` for your transport. See the [Krisp VIVA](/api-reference/server/services/audio-filters/krisp-viva-filter) documentation for more information.
...
<ParamField path="--resources" type="string">
  Explicit sizing for agents in [self-hosted
  regions](/pipecat-cloud/enterprise/sizing), as
  `cpu=<quantity>,memory=<quantity>` (e.g. `cpu=2,memory=4Gi`). Mutually
  exclusive with `--profile`: passing one clears the other from
  `pcc-deploy.toml`. Requires `pipecatcloud` {cliMinVersion} or later.
</ParamField>

<ParamField path="--architecture" type="string">
  CPU architecture the agent image requires: `amd64` or `arm64`. Omitted, the
  region's default applies. The choice is validated against the region's
  supported architectures (see [`regions
  list`](/api-reference/cli/cloud/regions#list)) before anything is uploaded.
  Must match how the image was built. Requires `pipecatcloud` {cliMinVersion}
  or later.
</ParamField>
...
<ParamField path="architecture" type="string">
CPU architecture the agent image requires: `amd64` or `arm64`. Omitted, the region's default applies. See [Pipecat Enterprise Agent Sizing](/pipecat-cloud/enterprise/sizing#architecture).
...
<ParamField path="[resources]" type="table">
Explicit sizing for agents in [self-hosted regions](/pipecat-cloud/enterprise/sizing), using Kubernetes quantities. Mutually exclusive with `agent_profile`.
...
<Tabs>
  ...
  <Tab title="Self-hosted region">
    ```toml
    # Basic agent configuration
    agent_name = "my-voice-agent"
    image = "your-dockername/my-voice-agent:0.1"
    region = "pce-acme-us-east"
    architecture = "arm64"

    # Referenced secrets (Kubernetes Secrets in your cluster)
    secret_set = "my-agent-secrets"
    image_credentials = "my-registry-creds"

    # Explicit sizing instead of an agent profile
    [resources]
    cpu = "1"
    memory = "2Gi"
