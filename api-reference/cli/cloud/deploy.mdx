...
<ParamField path="architecture" type="string">
CPU architecture the agent image requires: `amd64` or `arm64`. Omitted, the region's default applies. See [Agent Sizing and Architecture](/enterprise/sizing#architecture).
...
Explicit sizing for agents in [self-hosted regions](/enterprise/sizing), using Kubernetes quantities. Mutually exclusive with `agent_profile`.
