...
<ParamField path="--framework-version" type="string">
  ...
  tag, so results match the release your project runs. Pass a specific tag
  (e.g. `v1.7.0`), `latest` (same as the default), or `head` to track the
  default branch instead — the right choice when developing Pipecat itself or
  building against unreleased code. Also settable with
