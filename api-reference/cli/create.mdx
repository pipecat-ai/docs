...
```shell
pipecat create [TARGET_DIR] [OPTIONS]
```

<Tip>
  `pipecat create` is the scaffolder itself; coding agents and automation call
  it directly. If you're starting a project by hand, use [`pipecat
  init`](/api-reference/cli/init), which makes your project agent-ready.
</Tip>

**Arguments:**

<ParamField path="TARGET_DIR" type="string">
  The exact directory to scaffold into. `pipecat create .` scaffolds into the
  current directory and `pipecat create my-bot` into `./my-bot`, with the
  project name derived from the directory. Omit it to instead create a `<name>`
  subfolder under `--output` (or the current directory). Pass either
  `TARGET_DIR` or `--output`, not both.
</ParamField>
...
<ParamField path="--eval / --no-eval" type="boolean" default="false">
  ...
  scenarios in `server/evals/`, plus the dependencies to run them. See the evals
  docs for the verification workflow.
</ParamField>
...
```shell
pipecat create . --transport daily --mode cascade \
...
The generated `README.md` includes a **"Building with an AI coding agent"** section with [Pipecat Context Hub](/api-reference/context-hub) setup, so a scaffolded project is ready to extend with Claude Code or Codex without a separate agent guide.
