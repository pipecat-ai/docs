...
title: "pipecat eval"
sidebarTitle: "eval"
description: "pipecat eval runs scenario-based behavioral evals against a running Pipecat agent, individually or as a suite from the CLI."
---

Run scenario-based behavioral evals. `pipecat eval run` tests scenarios (scripted or simulations) against an already-running agent; `pipecat eval suite` spawns the agents listed in a manifest and runs their scenarios concurrently. Both exit `0` when everything passes and `1` otherwise.
...
# Only simulations (or only scripts with -k script)
