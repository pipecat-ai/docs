...
<ParamField path="SCENARIOS..." type="path" required>
  One or more scenario YAML files, scripted or simulated, or directories of
  them. A directory expands to its `.yaml` and `.yml` files in filename order,
  non-recursively, skipping `!include` fragments (files with no `name:`).
</ParamField>
