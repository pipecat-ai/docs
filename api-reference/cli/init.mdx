...
- **Scaffold a bot directly.** Run `pipecat init` and choose **Scaffold a runnable bot now** to set it up through an interactive wizard. Or skip the prompts entirely by passing scaffold options (`--bot-type`, `--transport`, the service flags, or `--config`) — the path coding agents and automation use.
