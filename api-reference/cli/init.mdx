...
title: "pipecat init"
sidebarTitle: "init"
description: "pipecat init scaffolds a new Pipecat app from the CLI: the single entry point for starting a project with Pipecat."
---
...
│   ├── src/
│   │   ├── components/
│   │   │   ├── pipecat/   # Pipecat UI components (React only)
│   │   │   └── ui/        # Base UI components (React only)
│   │   └── ...
│   ├── components.json     # shadcn config (React only)
│   ├── package.json
│   └── ...
├── .gitignore
...
When using `--client-framework react`, the generated client uses [Pipecat UI](/client/pipecat-ui) components installed as source. The client renders the Console block (connect flow, transcript, metrics, device and session info, live event stream) with your project's transport picker.

Components live under `src/components/pipecat/` and can be customized directly. The generated `components.json` configures the `@pipecat` shadcn registry, so you can add more components:

```bash
npx shadcn@latest add @pipecat/user-audio-control @pipecat/conversation
```

The vanilla JavaScript client (`--client-framework vanilla`) uses plain JavaScript and CSS styled to match the Console theme.
