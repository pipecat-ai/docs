...
- **Existing guide files are kept.** Re-running `pipecat init` never overwrites an existing `AGENTS.md`, `CLAUDE.md`, or `GETTING_STARTED.md`, so your edits are safe. If a kept guide was written by an older Pipecat version, the command says so, so you can choose to refresh it.
