...
- **Build with a coding agent** (recommended): open your coding tool and let it scaffold and iterate. The agent reads `AGENTS.md` and scaffolds the project for you, non-interactively. This path also writes **`GETTING_STARTED.md`**, developer-facing guidance on setting up the [Pipecat Context Hub](/api-reference/context-hub), writing an effective first prompt, and what to expect from a coding session.
- **Scaffold a runnable bot now**: `init` runs an interactive setup wizard **in-place** in the same directory, so it ends up with `AGENTS.md` + `CLAUDE.md` and a runnable bot. No `GETTING_STARTED.md` here: the bot already exists, and the generated README carries the Context Hub setup (the one piece a just-scaffolded project still needs).
...
>
  The full flow: make a project agent-ready, then build with a coding agent or scaffold a bot
</Card>
