...
Every `pipecat cloud` command accepts a global `--output rich|plain|json` option and exits non-zero on failure, which matters most in CI. See [Output Modes and Exit Codes](/api-reference/cli/cloud/output) for the full contract.
