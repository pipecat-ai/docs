---
title: 'STTMuteProcessor'
description: 'Processor for controlling STT muting and interruption handling during bot speech'
---

## Overview

`STTMuteProcessor` is a general-purpose processor that combines STT muting and interruption control. When active, it prevents both transcription and interruptions during bot speech, providing a cleaner conversation flow by ensuring the bot's speech isn't interrupted or transcribed.
...
<ParamField path="config" type="STTMuteConfig" default="STTMuteConfig()">
...
<ParamField
  ...
  default="STTMuteStrategy.NEVER">
  The muting strategy to use
</ParamField>

<ParamField
  path="should_mute_callback"
  type="Callable[[STTMuteProcessor], bool]"
  default="None">
...
<ParamField path="NEVER" type="STTMuteStrategy">
  Default strategy. Never mute the STT service
</ParamField>
...
# Custom muting logic
def custom_mute_logic(processor: STTMuteProcessor) -> bool:
...
All input frames are passed through except interruption frames when muted.
...
pipeline = Pipeline([
    transport.input(),
    stt_mute_processor,  # Add before STT service
    stt,
    # ... rest of pipeline
])
```
...
```mermaid
graph TD
    A[Transport Input] --> B[STTMuteProcessor]
    B --> C[STT Service]
    B -- "Suppressed when muted" --> D[Interruption Frames]
    B -- "STTMuteFrame" --> C
```
