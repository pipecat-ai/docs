...
On [`LLMWorker`](/api-reference/agents/llm-agent) subclasses, decorated methods are automatically registered with the LLM and included in `build_tools()`.
...
Decorated methods are automatically collected by [`BaseWorker`](/api-reference/agents/base-agent) at initialization and dispatched when matching job requests arrive. Each request runs in its own asyncio task so the bus message loop is never blocked.
...
Job handler methods receive a [`BusJobRequestMessage`](/api-reference/agents/messages#busjobrequestmessage):
...
Decorated methods are automatically collected by [`BaseWorker`](/api-reference/agents/base-agent) at initialization. When the agent starts, it calls `watch_workers` for each decorated handler. When the watched agent registers, the decorated method is called.
...
Agent-ready handler methods receive a [`WorkerReadyData`](/api-reference/agents/types#workerreadydata) instance:
