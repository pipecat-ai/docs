...
On [`LLMAgent`](/api-reference/agents/llm-agent) subclasses, decorated methods are automatically registered with the LLM and included in `build_tools()`.

On [`FlowsAgent`](/api-reference/agents/flows-agent) subclasses, decorated methods are automatically collected as global functions available at every flow node.
...
Decorated methods are automatically collected by [`BaseAgent`](/api-reference/agents/base-agent) at initialization and dispatched when matching task requests arrive. Each request runs in its own asyncio task so the bus message loop is never blocked.
...
Task handler methods receive a [`BusTaskRequestMessage`](/api-reference/agents/messages#bustaskrequestmessage):
...
Decorated methods are automatically collected by [`BaseAgent`](/api-reference/agents/base-agent) at initialization. When `on_ready` fires, the agent calls `watch_agent` for each decorated handler. When the watched agent registers, the decorated method is called.
...
Agent-ready handler methods receive an [`AgentReadyData`](/api-reference/agents/types#agentreadydata) instance:
