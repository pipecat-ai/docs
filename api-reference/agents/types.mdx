...
Activation arguments for [`LLMAgent`](/api-reference/agents/llm-agent). Extends [`AgentActivationArgs`](#agentactivationargs).
...
On normal completion, the result is available via `response`. On worker error or timeout, raises [`TaskError`](/api-reference/agents/exceptions#taskerror).
...
On normal completion, results are available via `responses`. On worker error (with `cancel_on_error=True`) or timeout, raises [`TaskGroupError`](/api-reference/agents/exceptions#taskgrouperror).
...
Tracks all known agents across local and remote runners. Owned by the [`AgentRunner`](/api-reference/agents/agent-runner) and shared with its agents.
