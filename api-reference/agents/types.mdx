...
Activation arguments for [`LLMWorker`](/api-reference/agents/llm-agent). Extends [`WorkerActivationArgs`](#workeractivationargs).
...
On normal completion, the result is available via `response`. On worker error or timeout, raises [`JobError`](/api-reference/agents/exceptions#joberror).
...
On normal completion, results are available via `responses`. On worker error (with `cancel_on_error=True`) or timeout, raises [`JobGroupError`](/api-reference/agents/exceptions#jobgrouperror).
...
Tracks a group of agents launched together by a job request. Exposed via [`BaseWorker.job_groups`](/api-reference/agents/base-agent#job-groups) and returned by `create_job_group_and_request_job`.
...
Tracks all known agents across local and remote runners. Owned by the [`PipelineRunner`](/api-reference/agents/agent-runner) and shared with its agents.
