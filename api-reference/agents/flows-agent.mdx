...
`FlowsAgent` extends [`BaseAgent`](/api-reference/agents/base-agent) with [Pipecat Flows](/api-reference/pipecat-flows/overview) for structured conversation flows. It manages a `FlowManager` for node-based conversations with functions, transitions, and actions.
...
>
  ...
  [`@tool`](/api-reference/agents/decorators#tool) are automatically
