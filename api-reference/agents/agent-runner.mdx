...
`PipelineRunner` runs agents to completion and acts as the host for a multi-agent session. It owns the shared [`WorkerBus`](/api-reference/agents/bus#workerbus) and [`WorkerRegistry`](/api-reference/agents/types#workerregistry) that back the whole session, and responds to bus messages for lifecycle coordination.
...
<ParamField path="bus" type="WorkerBus | None" default="None">
  The [`WorkerBus`](/api-reference/agents/bus#workerbus) instance.
  Creates an
  [`AsyncQueueBus`](/api-reference/agents/bus#asyncqueuebus) if not
  provided.
</ParamField>
...
| ---------- | ------------------------------------------------ | ------------------------- |
| `*workers` | [`BaseWorker`](/api-reference/agents/base-agent) | One or more agents to add |
