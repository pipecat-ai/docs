...
`LLMWorker` extends [`PipelineWorker`](/api-reference/agents/base-agent) with an LLM pipeline and automatic tool registration. Pass an `LLMService` to the constructor and define tools with the [`@tool`](/api-reference/agents/decorators#tool) decorator. Decorated methods are registered as direct functions on the LLM and tracked so frames queued during tool execution can be deferred until all tools complete.
...
<ParamField path="bridged" type="tuple[str, ...] | None" default="None">
  ...
  bridged agent. See
  [`PipelineWorker`](/api-reference/agents/base-agent#configuration) for
  details.
</ParamField>
...
Inherits all properties from [`PipelineWorker`](/api-reference/agents/base-agent#properties).
...
| --------- | ------------- | ------------------------------------------------------------------------------------------------------------ |
| `args`    | `dict | None` | Activation arguments (see [`LLMWorkerActivationArgs`](/api-reference/agents/types#llmworkeractivationargs)) |
