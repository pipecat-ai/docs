...
`LLMAgent` extends [`BaseAgent`](/api-reference/agents/base-agent) with an LLM pipeline and automatic tool registration. Subclasses implement `build_llm()` to provide an LLM service and define tools with the [`@tool`](/api-reference/agents/decorators#tool) decorator.
...
Inherits all parameters from [`BaseAgent`](/api-reference/agents/base-agent#configuration).
...
<ParamField path="bus" type="AgentBus" required>
  The [`AgentBus`](/api-reference/agents/bus#agentbus) for
...
<ParamField path="bridged" type="tuple[str, ...] | None" default="None">
  ...
  [`BaseAgent`](/api-reference/agents/base-agent#configuration) for
  details.
</ParamField>
...
Inherits all properties from [`BaseAgent`](/api-reference/agents/base-agent#properties).
...
Return the tools for this agent's LLM. By default, returns all methods decorated with [`@tool`](/api-reference/agents/decorators#tool). Override to provide additional or different tools.
...
| --------- | ------------ | -------------------------------------------------------------------------------------------------------------------- |
| `args`    | `dict | None` | Activation arguments (see [`LLMAgentActivationArgs`](/api-reference/agents/types#llmagentactivationargs)) |
