...
Proxy agents connect agents running on **different buses**. Unlike [distributed agents](/subagents/fundamentals/distributed-agents) (which all share the same bus), proxy agents bridge two isolated bus instances point-to-point. This is useful when:
...
```
Machine A (transport)              Machine B (LLM)
┌─────────────────────┐            ┌─────────────────────┐
│ AgentRunner          │            │ FastAPI server       │
│   MainAgent          │            │   AgentRunner        │
│   ProxyClient ──── WebSocket ────── ProxyServer        │
│                      │            │   AssistantAgent     │
└─────────────────────┘            └─────────────────────┘
```
