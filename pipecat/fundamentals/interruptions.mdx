...
When the user interrupts but nothing is transcribed — a cough, background noise, or speech the STT could not recognize — the bot stops talking but the context has no new user message. By default, the aggregator runs the LLM once with a developer message asking the user to repeat or resuming where it left off. This prevents the conversation from stalling mid-response.

Configure this behavior with the [`empty_user_turn`](/api-reference/server/utilities/turn-management/user-turn-strategies#additional-parameters) parameter on `LLMUserAggregatorParams`. Set it to `None` to leave such turns unanswered, or customize the prompts with `EmptyUserTurnConfig`.
