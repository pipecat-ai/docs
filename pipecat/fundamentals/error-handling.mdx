...
Use `EndWorkerFrame` to drain queued frames, or `CancelWorkerFrame` to abandon them. Push either one downstream (the default direction) so the frames already queued ahead of it are handled first.
