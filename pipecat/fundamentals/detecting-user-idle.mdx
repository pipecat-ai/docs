...
4. Restarts the timer after a user turn that ends with no transcript and gets no reply (e.g. background noise)
5. Emits an `on_user_turn_idle` event when the timer expires
6. Allows you to implement escalating responses or gracefully end the conversation in your application code
...
<CardGroup cols={2}>
  <Card
    ...
    icon="code"
    iconType="duotone"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/turn-management/turn-management-detect-user-idle.py"
  >
  ...
  <Card
    ...
    icon="book"
    iconType="duotone"
    href="/api-reference/server/utilities/turn-management/turn-events"
  >
