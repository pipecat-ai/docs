...
Every service has a read-only `settings` property with its current settings, so code outside the service can read them:

```python
llm.settings.model  # "gpt-4o-mini"
```

It reflects runtime updates once the service has applied them. Change settings through an update settings frame rather than by assigning to `settings`, so the service applies the change and its metrics report the new model.
