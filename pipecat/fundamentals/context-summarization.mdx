...
3. Context is reconstructed as: `[system_message (if present)] + [summary] + [recent_messages]`
...
`summarize_conversation` is a [direct function](/pipecat/learn/function-calling#1-define-a-tool): its name and description are taken from the function and its docstring. List it in the context's `tools` so the LLM can invoke it when the user asks to summarize — the handler is registered automatically:
