...
| `cache_read_input_tokens`       | `Optional[int]` | Tokens read from cache, if applicable. Billed below the input rate            |
| `cache_creation_input_tokens`   | `Optional[int]` | Tokens written to cache, if applicable. Billed at or above the input rate     |
...
<Note>
  Services differ in how they report `prompt_tokens` relative to the cache
  counts. Anthropic and Bedrock report `prompt_tokens` net of cache, with the
  cache counts alongside it. OpenAI-compatible services report it gross, with
  the cache counts already inside it. `total_tokens` is always the gross figure,
  so it stays comparable across services. Per-bucket cost accounting needs to
  account for this: adding cache counts to `prompt_tokens` double-counts them on
  services that report gross.
</Note>
