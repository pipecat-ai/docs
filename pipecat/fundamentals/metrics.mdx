...
| Field                            | Type            | Description                                                                                                           |
| -------------------------------- | --------------- | --------------------------------------------------------------------------------------------------------------------- |
| `prompt_tokens`                  | `int`           | Number of tokens in the input prompt. Net of the cache on services that report cache reads separately.               |
| `completion_tokens`              | `int`           | Number of tokens in the generated completion                                                                          |
| `total_tokens`                   | `int`           | Total tokens used, including any cached input tokens. Not always `prompt_tokens + completion_tokens` — read the cache fields for the breakdown. |
...
<Note>
Services differ in whether their input count is reported net or gross of the prompt cache. `total_tokens` is the gross figure either way (across all services), so it stays comparable. Read the cache fields for the breakdown rather than subtracting.
</Note>
