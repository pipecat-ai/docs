...
Time To First Byte — measures how long until the first output byte is received from a service. For LLM services, this is the time to the model's first output token (including reasoning tokens from thinking models). For TTS services, this is the time to the first audio byte.
...
<Note>
  ...
  TTFB and TTFA capture the meaningful latency for these services.
</Note>
