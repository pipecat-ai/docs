...
async def on_service_usage(observer, record):
    if record.kind == "llm":
        print(f"LLM: {record.total_tokens} tokens")

worker = PipelineWorker(pipeline, observers=[observer])
```

Records arrive per piece of work (not summed), allowing flexible grouping by turn, session, or model. See the [Service Metrics Observer](/api-reference/server/utilities/observers/service-metrics-observer) reference for full details.
