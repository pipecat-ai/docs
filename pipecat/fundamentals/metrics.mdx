...
<Note>
WebSocket-based TTS services (Cartesia, Deepgram, ElevenLabs WebSocket variants)
and `DeepgramSageMakerTTSService` do not report processing metrics. These services
hand text off for synthesis and receive audio on a separate task, so the measurement
would only cover the send operation and not the actual synthesis. TTFB and TTFA capture
the meaningful latency for these services.
</Note>
