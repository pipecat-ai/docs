...
The smallest UIWorker example: a plain `UIWorker` with a system prompt answers the voice agent's questions about whatever is currently on the page.
...
The voice agent changes and reads a list on screen through custom `UIWorker` jobs, which find each item with a classifier. The screen is the source of truth.
...
An accessibility-first, voice-guided walkthrough of a web form, driven entirely through the `screen` tool.
...
The voice agent asks the `screen` tool for the user's current selection, so "explain this" resolves to what they have highlighted.
...
A `UIWorker` job fans long-running research out to peer agents, and the client shows each one's progress as a cancellable card.
