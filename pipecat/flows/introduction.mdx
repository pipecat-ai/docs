...
description: "Structure a conversation as a graph of nodes with Pipecat Flows: conversation paths, state management, and node transitions."
---

Pipecat Flows builds **conversational pathways**: a conversation defined as a graph of nodes, where each node focuses the LLM on a single task with only the tools it needs.

Flows ships as the `pipecat.flows` module, so the classes you write against are `FlowManager`, `NodeConfig`, and `FlowsFunctionSchema`.
...
## When to Use a Conversational Pathway

A pathway is best suited for use cases where:
