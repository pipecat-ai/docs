...
<Note>
  A flow described as data has since returned, in a different shape. Pipecat
  Flows now ships a [flow config](/pipecat/flows/flow-configs): a YAML or JSON
  document, loaded at runtime and joined to a module of Python handlers. It is
  the successor to 1.0's static flows and the recommended starting point for a
  new flow. The `FlowConfig` name is reused, but the format below is not
  accepted — a config is a Pydantic model with its own schema, and it is passed
  to `Flow`, not to `FlowManager`.
</Note>
