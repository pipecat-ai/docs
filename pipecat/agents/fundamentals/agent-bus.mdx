...
The default bus, created automatically by `PipelineRunner` when you don't provide one. It uses asyncio queues for in-process communication with no serialization overhead.

```python
runner = PipelineRunner()  # Creates AsyncQueueBus automatically
```
