---
title: "The Eval Loop"
description: "Close the loop: Pipecat evals give an AI coding assistant a pass/fail signal it can read, so it writes agent code, runs evals, and iterates."
---
...
1. **Describe the behavior as a scenario.** A scenario file is an executable specification. A [scripted scenario](/pipecat/evals/scripted-scenarios) is the conversation, the expected events, and the criteria a response must meet; a [simulation](/pipecat/evals/simulated-scenarios) is a caller with a goal, what counts as success, and the quality every reply must keep.
...
4. **The assistant reads the result.** A non-zero exit code, a per-assertion failure message ("turn 1 expectation 0 (llm_response): judge said no: ...") or a simulation's verdict ("goal not met: ..." or the metric that fell short, with the judge's reason for each reply it said no to), and a full decision trace in `<scenario>.eval.log`.
...
- **Two kinds of scenario for two kinds of question.** A scripted scenario checks an exact exchange, so an assistant uses it when the task is "call this tool when the user says this". A simulation checks a goal, so an assistant uses it when the task is "get this caller through the flow", and `-v` prints the conversation so it can see where things went wrong.
...
Evals live in `scenarios/`: scripted scenarios in `scenarios/scripted/`
and simulations in `scenarios/simulated/`. To verify any change to the
agent's behavior:

1. Start the agent: `uv run bot.py -t eval` (serves ws://localhost:7860)
2. Run the evals: `pipecat eval run scenarios/scripted/ scenarios/simulated/`

The command exits non-zero on failure and prints each failed assertion,
or for a simulation the judge's verdict. Pass `-v` to see a simulation's
conversation. Each scenario writes a decision trace to
`<scenario>.eval.log`; read it to understand a failure before changing code.

When you add or change agent behavior, add or update a scenario to cover
it: a scripted scenario for an exact exchange, a simulation for a goal.
```
...
A simulation works as an acceptance criterion for a whole flow. "A caller who wants a table for two at 6 PM gets one booked, every reply stays polite, and the agent calls `book_table`" is one scenario with a `persona:`, a `goal:`, a `success:`, and two `metrics:`, and "make this pass three runs in a row" is the task.

## Guardrails

A few practices keep autonomous loops honest:

- **Review scenario changes like code.** An assistant that can edit scenarios can also weaken them, by loosening an `eval:` criterion, a `success:`, or a metric's `min_score`. Failing evals should usually be fixed in the agent, not in the scenario.
...
- **Run simulations more than once.** A persona says something different each run, so set `runs: 3` and let `pipecat eval suite` require every run to pass rather than trusting one.
