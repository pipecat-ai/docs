...
A simulation works as an acceptance criterion for a whole flow. "A caller who wants a table for two at 6 PM gets one booked, every reply stays polite, and the agent calls `book_table`" is one file with a `persona:`, a `goal:`, a `success:`, and two `metrics:`, and "make this pass three runs in a row" is the task.
