...
- **Simulations find, scripts pin.** A failing simulation shows where the agent lost the caller, with the conversation printed under `-v`. The assistant reproduces that exchange as a scripted scenario, fixes against it, and keeps both.
...
When you add or change agent behavior, add or update a scripted scenario
to cover it. When a simulation fails, reproduce the exchange where the
agent lost the caller as a scripted scenario and fix against that.
```
...
- **Review scenario changes like code.** An assistant that can edit scenarios can also weaken them, by loosening an `eval:` criterion, a `success:`, or a metric's `min_quality`. Failing evals should usually be fixed in the agent, not in the scenario.
