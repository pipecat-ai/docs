...
description: "Scenario files for scripted and simulated scenarios: the scenarios: list, file defaults, user: and judge: blocks, factory:, and !include."
---
...
In either modality, the `judge.eval:` block configures what decides the verdicts: `ollama` (the default, `gemma4:12b`, with an optional `endpoint:` for a remote Ollama), or a classifier or LLM through a [`factory:`](#custom-services-with-factory). `service: openai` is deprecated since 1.9.0 in favor of a factory and will be removed in 2.0.0. The judge decides a scripted scenario's [`eval:`](/pipecat/evals/scripted-scenarios#semantic-judging-with-eval) assertions, and a simulation's [`success:`](/pipecat/evals/simulated-scenarios#judging-the-outcome) and judged metrics. An LLM judge also provides the reason behind each verdict; a classifier judge returns verdicts with confidence scores and can optionally use an `explainer:` LLM to generate reasons.
...
When using a classifier for judging (via a `factory:`), verdicts include confidence scores but no reasons. An optional `explainer:` LLM block can be added to generate reasons for verdicts below a confidence threshold:

```yaml
judge:
  eval:
    factory: my_evals.classifier  # returns a BaseClassifier
    explainer:
      service: ollama
      model: gemma4:12b
      extra:
...
The explainer runs only when needed and its reasons accompany the classifier's verdicts. Set `explainer: false` to disable reasons entirely.
...
The built-in `service:` names are the local, keyless services: Ollama for the judge and the persona, Kokoro for the user's voice, Moonshine and Whisper for the agent's audio. Any other provider is a `factory:`, a dotted path to a callable that takes the block's mapping and returns the service or classifier. The scenario stays a description of the test, while the provider, its settings, and its key live in code. Any extra keys you put in the block are passed through to your factory:
...
- `judge.eval:`: a `BaseClassifier` (e.g., `JevClassifier`, `LLMClassifier`) or an OpenAI-compatible LLM service. When returning an LLM service, it will be wrapped in an `LLMClassifier` automatically.
- `simulator:`: an OpenAI-compatible LLM service that supports function calling.
