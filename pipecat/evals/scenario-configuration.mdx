...
user:
  ...
  speech:
    ...
    voice: af_heart
    speed: 1.0 # optional; Kokoro's rate (0.5 to 2.0, default 1.0)
    sample_rate: 16000
```
...
Both the `speech:` and `judge.transcription:` blocks take an optional `language`, so a non-English agent can be driven and judged in the language it speaks. The `speech:` block also accepts an optional `speed` (Kokoro's rate multiplier, 0.5 to 2.0, default 1.0), which scales both the speech rate and pauses:
...
user:
  ...
  speech:
    ...
    language: zh
    speed: 1.2 # optional; makes the caller speak 20% faster
```
