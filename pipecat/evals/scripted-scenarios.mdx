...
A scripted scenario is a YAML file that writes the user's turns out, each with the events you expect your agent to emit in response. A scenario with `turns:` is scripted; one with a `persona:` is a [simulated scenario](/pipecat/evals/simulated-scenarios), where an LLM plays the user instead. This page covers the full scripted format. If you haven't run a scenario yet, start with the [quickstart](/pipecat/evals/quickstart).
...
A scenario file holds one or more scenarios under `scenarios:`, each with a `name:`. Most files hold one scenario; several scenarios per file suit testing one behavior through many short conversations. Any configuration a scenario can have (like `judge:` or `context:`) may also sit at the file's top level as the default for all scenarios:

```yaml
name: multi_turn # file name, used when scenarios share a name or have none

judge: # shared by all scenarios in this file
  eval:
    service: ollama
    model: gemma4:12b
...
scenarios:
  - name: multi_turn # the scenario's name
...
Each scenario is named `<file>/<scenario>`, so the example above is `multi_turn/multi_turn`. Each turn optionally sends a user utterance (`user:`) and lists the events expected in response (`expect:`). Expected events must arrive in the order listed, but the agent may emit other events in between, so you don't have to enumerate everything it does.

Two common shapes for multiple scenarios per file:

**The same turns, judged differently.** The turns are written once at the file level, and each scenario names its own judge or modality. Every judge sees exactly the same conversation:
...
**Short cases of one behavior.** Each scenario is a self-contained check. For example, testing turn-completion markers:
...
Any key a scenario can have may also sit at the top of the file as the default for all scenarios. A scenario that sets the same key replaces the whole value; nothing is merged, so a `context:` is written out in full.
...
| `llm_marker`            | A sideband marker the bot's LLM emitted, such as the turn-completion markers of [filter incomplete user turns](/api-reference/server/utilities/turn-management/filter-incomplete-turns). Sent only when the scenario asks for them. |
...
`llm_marker` is the sideband marker the bot's LLM emitted in a response, such as the turn-completion markers of [filter incomplete user turns](/api-reference/server/utilities/turn-management/filter-incomplete-turns). It arrives when the response ends. The `marker:` field names which one: `complete` (the turn was finished and the bot answers), `short` (the user was cut off and the bot waits), `long` (the user asked for time), or `incomplete` (either of the last two). A bare `llm_marker` asserts only that the bot read a response.

Markers never reach clients by default; a scenario that asserts on one asks the bot to report them:
...
The `eval:` field is a natural-language criterion that the event must satisfy, decided by the [judge LLM](/pipecat/evals/scenario-configuration#judging-with-judge):
...
`eval:` on the agent's text output (`response`, `llm_response`, `tts_response`) judges the content of the reply.

#### Function calls

`eval:` on a `function_call` event judges each matched call by its name and arguments, which is how a scenario checks what `args:` cannot match verbatim:

```yaml
- user: "Submit a session suggestion about OpenTelemetry tracing for Jennifer Smith."
  expect:
    - event: function_call
      calls:
...
Each call that matches the `name:` and `args:` is then put to the judge. The first call the judge rejects fails the expectation. The judge sees the conversation so far as context, but the verdict is about the call itself.

A `function_call_stopped` event carries no arguments (only how the call ended), so it cannot take an `eval:`.

### Substring checks with `text_contains:` and `text_excludes:`
...
`text_excludes:` is the mirror: the event's text must not hold the given substring. This is useful when checking that a marker the LLM should keep out of its reply stayed out:
...
For an `llm_marker` event, several fields check the marker itself and how the LLM's raw response was laid out:

- `marker:` asserts what the marker means: `complete`, `short`, `long`, or `incomplete` (which accepts either `short` or `long`). This checks the marker's `kind` field, not its text, so it works regardless of the configured marker characters.
- `marker_first:` (true/false) checks whether the first marker in the response's raw text comes before any text.
- `markers:` (integer) checks how many markers the response's raw text holds.
- `text_after:` (true/false) checks whether text follows the first marker in the raw text.
...
#### Judging function calls with `eval:`

When argument values can't be matched verbatim — for example, checking that a generated session description is about the right topic — add an `eval:` to judge each matched call:
...
Each call that passes the `name:` and `args:` checks is then put to the judge. The first call the judge rejects fails the expectation. See [Semantic judging with `eval:`](#semantic-judging-with-eval) for details.
...
### Absence checks with `absent: true`

Set `absent: true` to invert the expectation: assert that no event of this type arrives before the `within_ms` budget expires. This matches on event type only, so it cannot be combined with `text_contains:`, `eval:`, or `calls:`. Used for duplicate-output regressions:
...
- user: "I'd go to Japan because"
  expect:
    - event: response
      absent: true
      within_ms: 3000 # assert the bot waits rather than answering the incomplete turn
```

For `response` events, a response that continues the reply an earlier expectation matched is not a new one. Only a reply the bot began after that match counts, so a later segment of the same response does not fail an absence check.
