...
Use a scripted scenario to pin one behavior: the same input every run, so a failure is reproducible and you fix from the assertion. Use a simulation to check that the agent gets a caller to a goal when the caller phrases things their own way.
...
The `user:` and `judge:` blocks, `factory:`, and `!include` are shared with [simulated scenarios](/pipecat/evals/simulated-scenarios), which configure their modalities the same way.
