...
`llm_marker` is a sideband marker the bot's LLM emitted, such as the turn-completion markers of [filter incomplete user turns](/api-reference/server/utilities/turn-management/filter-incomplete-turns). The `marker:` field names which one: `complete` (the turn was finished and the bot answers), `short` (the user was cut off and the bot waits), `long` (the user asked for time), or `incomplete` (either of the last two). A bare `llm_marker` asserts only that a marker arrived.
...
### Marker checks with `marker:`

For an `llm_marker` event, `marker:` asserts what the marker means: `complete`, `short`, `long`, or `incomplete` (which accepts either `short` or `long`). This checks the marker's `kind` field, not its text, so it works regardless of the configured marker characters:
