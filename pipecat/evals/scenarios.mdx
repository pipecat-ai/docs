...
description: "The scripted scenario file format: configuration, turns, events, and assertions."
---

A **scripted scenario** is a YAML file describing a scripted conversation and the events you expect your agent to emit. This page covers the full format. If you haven't run a scenario yet, start with the [quickstart](/pipecat/evals/quickstart).

<Note>
  Pipecat Evals also supports **simulations**, where an LLM plays a caller
  pursuing a goal instead of following a script. See
  [Simulations](/pipecat/evals/simulations).
</Note>
