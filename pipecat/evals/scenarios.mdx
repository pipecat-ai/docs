...
The rest of this page is in four parts: [Configuration](#configuration) for scenario-wide setup (modalities, the judge, context, lifecycle), then [Input](#input) for driving each turn, and [Events](#events) and [Assertions](#assertions) for checking what the agent emits.

## Configuration

Everything in this section is optional. A scenario with no configuration blocks runs entirely in [text mode](#text-and-audio-modes) with the default judge, which is the fastest way to start. To write the turns themselves, skip ahead to [Input](#input) and [Events](#events).
...
Each turn drives the agent one of three ways: a spoken `user:` utterance, a `dtmf:` keypress sequence, or no input at all (observation-only). `send_after:` controls when that input is sent.
...
The aggregator flushes either on the `#` terminator or on its idle timeout. To exercise the idle-timeout path, omit the `#` and pace the keys with a time-based [`send_after:`](#scheduling-input-with-send_after):
...
### Scheduling input with `send_after:`

`send_after:` controls when a turn's input (its `user:` utterance or [`dtmf:`](#dtmf-turns) keypresses) is sent, either relative to a prior event or after a plain delay. Anchoring it to an event is how you script barge-in tests:
...
| `user_transcription`    | The agent's STT finalized a transcription of the user. Audio mode only, except on [DTMF turns](#dtmf-turns), where the aggregated keys become a transcription in either mode.  |
| `user_started_speaking` | The agent's VAD detected the start of user speech. Audio mode, or a [DTMF turn](#dtmf-turns) in either mode.                                                                   |
| `user_stopped_speaking` | The agent's VAD detected the end of user speech. Audio mode, or a [DTMF turn](#dtmf-turns) in either mode.                                                                     |
