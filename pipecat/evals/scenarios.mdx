...
The user side and the judge side each have their own modality, set with `modality:` in the top-level `user:` and `judge:` blocks. When `modality:` isn't specified (or a block is omitted entirely), it defaults to `text`. The two sides are independent: you can drive the agent with text while judging its real speech, or speak to it and judge the LLM text.

### Text mode (the default)

A scenario with no `user:` or `judge:` block runs entirely in text mode, equivalent to spelling out:
