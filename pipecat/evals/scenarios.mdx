...
<Note>
  The built-in speech service is `kokoro`, a local model. For any other TTS
  provider, use a `factory:` (see [Custom services with
  `factory:`](#custom-services-with-factory)).
</Note>
...
In either modality, the `judge.eval:` block selects the judge LLM: `ollama` (the default, `gemma4:12b`), or any OpenAI-compatible provider via a `factory:` (see [Custom services with `factory:`](#custom-services-with-factory)). This is the LLM that decides [`eval:`](#semantic-judging-with-eval) assertions.
...
To use a TTS, STT, or LLM beyond the built-ins, use a `factory:`: a dotted path to a callable that receives the block's mapping (and the resolved sample rate for audio services) and returns the service. Any extra keys you put in the block are passed through to your factory. This is how you use providers like OpenAI, Cartesia, Deepgram, or any other service:
...
judge:
  modality: audio
  eval:
    factory: "my_evals.services.make_judge"
    model: gpt-4o-mini
  transcription:
...
def make_judge(eval_cfg):
    return OpenAILLMService(
        api_key=os.environ["OPENAI_API_KEY"],
        settings=OpenAILLMService.Settings(model=eval_cfg.get("model", "gpt-4o")),
    )
```

<Warning>
  The built-in service names `openai` and `cartesia` are deprecated since 1.9.0
  and will be removed in 2.0.0. Use a `factory:` instead as shown above.
</Warning>
...
A scenario whose spoken turns all name an `audio:` file needs no [`user.speech:` block](#user-delivery-with-user), since nothing is synthesized.
