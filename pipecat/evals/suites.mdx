...
Each entry's runs go one after another on a single slot, so its scenarios finish as a block and a slow provider holds no more than one slot. An entry may set its own `concurrency:` to hold more than one slot.
...
Each entry's scenarios run one after another on the slot it holds, so a slow or rate-limited provider never holds more than that one slot and its scenarios finish as a block. Queues take free slots in manifest order, every entry's first attempt before any entry's second. An entry with its own `concurrency:` holds that many slots across its attempts and runs its queues that wide.
...
Attempts run attempt-major (every entry's first attempt before any entry's second), and artifact filenames gain an attempt suffix (`__001`) only when `repeat` is above 1, so a single pass keeps the filenames it always had.
