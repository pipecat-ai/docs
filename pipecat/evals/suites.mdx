...
A manifest lists both scripted scenarios and simulations under `scenarios:` side by side; the harness distinguishes them by their file structure.
...
suite:
  ...
  - bot: bots/restaurant-agent.py
    scenarios:
      - greeting # scripted scenario
      - book_table_available # simulation
...
`results.jsonl` carries one line per run — its bot, scenario, `kind` (`script` or `simulation`), attempt, outcome, duration, failures, per-turn results, and paths to its artifacts. Lines are appended as each run finishes, so an interrupted sweep keeps everything already done. Runs that didn't pass also carry `events_seen`.
