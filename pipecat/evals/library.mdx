...
The agent must already be running with its eval transport (`python bot.py -t eval`), just as with `pipecat eval run`. When you know the kind up front, `EvalScriptSession.from_scenario()` from `pipecat.evals.script_session` takes the same arguments and returns the typed session.
...
A session runs the simulation once. The `runs:` field is honored by the [suite](#orchestrating-suites), which spawns a fresh agent for each run. The typed counterpart is `EvalSimulationSession.from_scenario()` from `pipecat.evals.simulation_session`.
