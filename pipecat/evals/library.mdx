...
`from_scenario()` builds the judge from the scenario's `judge:` block, but you can inject your own. `EvalJudge` takes a classifier that decides verdicts, and optionally an explainer LLM that provides reasons. The `judge=` parameter applies to either kind:
...
<Note>
  Passing an LLM service directly to `EvalJudge` (`EvalJudge(llm)` or
  `EvalJudge(service=llm)`) is deprecated since 1.3.0 and will be removed in
  2.0.0. Use `EvalJudge(LLMClassifier(llm=llm), explainer=llm)` instead.
</Note>
