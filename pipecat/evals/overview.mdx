...
- **Scripted scenarios** write the user's turns out with expectations for each one
- **Simulations** give the user's side to an LLM playing a caller with a goal
...
**Scripted scenarios** pin specific behavior:
...
**Simulations** hold an autonomous conversation:

- **Goal completion**: a persona LLM pursues a goal and a judge decides whether the bot did its job.
- **Quality metrics**: judge every reply against criteria (politeness, clarity, brevity) and measure latency, turn count, word count, or function calls against bounds.
- **Non-deterministic coverage**: because a persona says something different every run, simulations catch issues that only surface sometimes rather than on a specific script.
...
<Card
  ...
  icon="users"
  iconType="duotone"
  href="/pipecat/evals/simulations"
>
  Let an LLM play a caller pursuing a goal and judge whether the bot did its job.
</Card>
