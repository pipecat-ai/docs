...
A scenario file lists one or more scenarios under `scenarios:`, and every scenario has a **kind**, decided by its keys: `turns:` makes it scripted, `persona:` makes it simulated. A file can hold both, and each scenario runs on its own as `<file>/<scenario>`.
