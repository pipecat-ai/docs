...
```mermaid
flowchart LR
  A["Build or change the agent<br/>prompt, provider, pipeline, tools"]
  B["Pipecat Evals<br/>script expected behaviors<br/>run locally and in CI"]
  C["Simulation platform<br/>cover variation, voice,<br/>transport, load, traces"]
  D["Production monitoring<br/>evaluate real conversations<br/>alert and review"]
  E["New coverage<br/>local scenarios,<br/>simulations, metrics"]

  A --> B
  B -->|"scripted QA gets too narrow"| C
  C -->|"release confidence"| D
  D -->|"production findings"| E
  E --> B
  E --> C
```

Read the loop from left to right. Keep expected behaviors in Pipecat Evals when
they can be scripted and should run on every change. Add simulations when
quality depends on variation, voice conditions, deployed transport, load, traces,
or cross-functional review. After release, evaluate production conversations,
alert on high-signal metrics, use human review to calibrate them, and turn
production findings into the next local scenarios, simulations, and metrics.
