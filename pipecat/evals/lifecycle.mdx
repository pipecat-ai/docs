...
```mermaid
flowchart TB
  A["Build or change the agent<br/>prompt, pipeline, tools"] --> B["Pipecat Evals<br/>local scenarios and CI<br/>known behavior"]
  B --> C{"Can the risk be captured<br/>as a scripted scenario?"}
  C -->|Yes| D["Keep it local<br/>YAML scenario, audio mode,<br/>function-call assertions"]
  C -->|Needs broader coverage| E["Add simulations<br/>caller variation, voice,<br/>transport, load, traces"]
  D --> F{"Is the agent live<br/>with real conversations?"}
  E --> F
  F -->|Pre-launch| B
  F -->|Live with users| G["Add production monitoring<br/>submitted conversations,<br/>dashboards, review, alerts"]
...
Use the two decision points to choose the next layer. If a risk is crisp enough
to script and should block PRs, keep it in Pipecat Evals. If confidence depends
