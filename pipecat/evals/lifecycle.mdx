...
flowchart TB
  ...
  B --> C["Platform simulations<br/>caller variation, audio,<br/>transport, load, traces"]
  C --> D["Production monitoring<br/>submitted conversations,<br/>dashboards, review"]
  D --> E["Findings<br/>failures, trends,<br/>human review notes"]
  E --> F["Feed back into evaluation<br/>new local scenarios,<br/>simulation cases,<br/>trace and monitoring metrics"]
  F --> B
  F --> C
  F --> D
```

Move through the lifecycle as confidence needs expand. Start with local
scenarios that block PRs, use simulations before launch to cover broader
variation, monitor completed production conversations, then feed those findings
back into the next local scenarios, simulation cases, and monitoring metrics.
