...
```mermaid
flowchart TB
  A["Build or change the agent<br/>prompt, pipeline, tools"] --> B["Start local<br/>script core flows in Pipecat Evals<br/>run locally and in CI"]
  B --> C{"What still needs coverage?"}
  C -->|A scripted behavior| D["Keep expanding Pipecat Evals<br/>YAML scenarios, audio mode,<br/>function-call assertions"]
  C -->|Variation or system behavior| E["Add simulations before launch<br/>caller variation, voice,<br/>transport, load, traces"]
  D --> F{"Are real user conversations<br/>now available?"}
  E --> F
  F -->|Pre-launch| B
  F -->|Live usage| G["Add production monitoring<br/>submitted conversations,<br/>dashboards, review, alerts"]
  G --> H["Feed findings back<br/>crisp regressions become local scenarios<br/>broader patterns become simulations"]
  H --> B
  H --> E
  H --> G
```

Use the two decision points to choose the next layer. If a behavior can be
scripted and should block PRs, keep it in Pipecat Evals. If confidence depends
on caller variation, voice or transport behavior, load, traces, or human
judgment, add simulations before launch. Once real conversations exist, add
production monitoring, then use those findings to create sharper local scenarios
and broader simulation coverage.
