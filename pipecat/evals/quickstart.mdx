...
description: "Run your first Pipecat behavioral evals against an existing agent: a scripted scenario, then a simulated caller with a goal."
---
...
- [Ollama](https://ollama.com), with `ollama pull gemma4:12b`. It runs the judge, and in the last step the simulated caller, locally with no API key. (It's the default; any other LLM works through a `factory:`, see [Scenario Configuration](/pipecat/evals/scenario-configuration#judging-with-judge).)
...
<Steps>
  ...
  <Step title="Write a scripted scenario">
    ...
    A file lists its scenarios under `scenarios:`, one here, and each has a `name:` of its own. Each turn optionally sends a user utterance and lists the events expected in response. The `eval:` field is a natural-language criterion checked by the judge LLM, so the test passes whether the agent says "Berlin is the capital of Germany" or "That would be Berlin!".
    ...
    <Note>
      ...
      Configuration](/pipecat/evals/scenario-configuration#judging-with-judge).
    </Note>
  ...
  <Step title="Run the eval">
    ...
    ```
      ...
      ✓ ws://localhost:7860 capital_question/capital_question (3402ms)

      1/1 passed  ·  3.4s
    ```

    A scenario is named after its file and then itself, `capital_question/capital_question` here. The command exits `0` when everything passes and `1` otherwise, so it slots directly into scripts and CI. Each scenario also writes a decision trace to `capital_question__capital_question.eval.log`, which shows every event the harness saw and why each assertion passed or failed.
  ...
  <Step title="Make it fail (optional but recommended)">
    ...
    ```
      ✗ ws://localhost:7860 capital_question/capital_question
  ...
  <Step title="Let a simulated caller talk to it">
    ...
    ```
      ...
      ✓ ws://localhost:7860 capital_curious/capital_curious end_call (9814ms)
...
- Put several short scenarios in one file, with shared defaults at the top, as described in [Scenario Configuration](/pipecat/evals/scenario-configuration#scenario-files).
