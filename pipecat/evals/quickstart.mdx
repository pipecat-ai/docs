...
- A judge LLM. Either:
  ...
  - **OpenAI**: set `OPENAI_API_KEY` and point the scenario's `judge:` block at it through a small factory (shown below).
...
<Steps>
  ...
  <Step title="Write a scripted scenario">
    ...
    <Note>
      ...
      has no `judge:` block. Any other LLM is a `factory:`, a dotted path to a
      callable that takes the block and returns the service, as in the OpenAI
      tab. That one needs a `my_evals.py` next to the agent:
    </Note>
