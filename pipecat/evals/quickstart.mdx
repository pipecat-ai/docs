...
- For the simulation step, an `OPENAI_API_KEY` in your agent's `.env`. The simulated caller is played by an OpenAI model, and `pipecat eval` loads the nearest `.env` before it runs.
...
<Steps>
  ...
  <Step title="Let a simulated caller talk to it">
    ...
    There are no turns to write. The `simulator:` block names the LLM that plays the caller, `persona:` and `goal:` tell it who it is and what it wants, and `success:` tells the judge what the agent must have done. The `politeness` metric is judged on every reply, and `words` bounds the longest one.
    ...
    The caller says something different every run, which is the point: it checks that the agent gets a real caller to the goal, not that one scripted path still works. When a simulation fails, reproduce the exchange where the agent lost the caller as a scripted scenario and fix against that.
