...
scenarios: # required: the simulations the file holds
  - name: available # required: this simulation's name
...
The simulation runs as `book_table/available`, its file's name and then its own. The `simulator:`, `user:`, and `judge:` blocks sit at the top of the file here, as defaults for every simulation in it, and any other key can too. A file that holds several simulations is how one caller goes on different errands; see [Several simulations in one file](#several-simulations-in-one-file).
...
Any other provider is a `factory:`, a dotted path to a callable that takes the block and returns an OpenAI-compatible LLM service, exactly as for the [judge](/pipecat/evals/scenario-configuration#judging-with-judge):
...
`trigger_disconnect: true` fires the agent's `on_client_disconnected` handler when the run ends. See [Exercising the disconnect path](/pipecat/evals/scenario-configuration#exercising-the-disconnect-path).
...
| Ending         | Meaning                                                                                                                                                                                                                                             |
| -------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `end_call`     | The persona called its `end_call` tool. When the persona hangs up on its own last line, the run waits for the agent's reply to it before ending, so the agent gets its closing turn and any final tool calls it makes are part of the conversation. |
...
```
  ...
  ✓ ws://localhost:7860 book_table/available end_call (14230ms)
...
```
  ✗ ws://localhost:7860 book_table/available end_call (16102ms)

  Failures (1 of 1):
  ✗ ws://localhost:7860 book_table/available
      • politeness 0.75 below 1.00: turn 3: the reply is curt, a bare "No." end_call

  0/1 passed, 1 failed  ·  16.1s
```

The failure shown is the first thing that went wrong, in this order: an error, then the goal ("goal not met: ..."), then the first failed metric. Every metric's score, value, and per-turn verdicts are in the decision trace at `<file>__<scenario>.eval.log`, and in `results.jsonl` when a suite ran it. The judge and the persona LLM's own logs are in `<file>__<scenario>.debug.log` under `-d`.
...
- **Put the callers of one bot in one file.** A `scenarios:` list with a caller per entry shares the `simulator:`, `judge:`, and limits, and each caller still runs on its own.
...
## Several simulations in one file

A file's `scenarios:` list can hold several simulations, and any key at the top of the file is the default for all of them. That suits a bot with one job and several kinds of caller: the file sets the `simulator:`, the `judge:`, and the limits once, and each simulation is a caller with its own `persona:`, `goal:`, and `success:`. Pipecat's own `book_table` file holds a caller whose time is free, a flexible one whose first choice is taken, and one who can only make times that are all taken, where success is a graceful no:
...
scenarios:
  - name: available
    persona: |
      Jamie, calling to book a table for tonight. Friendly and to the point: gives
      the party size and the time when asked, one detail per turn.
    goal: "Book a table for two at 6 PM tonight, then end the call."
    success: "the bot confirmed a reservation for a party of two at 6 PM"

  - name: impossible
    persona: |
      Casey, calling to book a table for tonight. Can only make 7 PM or 8 PM;
      asks for the other one if the first is unavailable, and if neither works,
      thanks the assistant and ends the call without booking anything.
    goal: "Book a table for two tonight at 7 PM or 8 PM; if neither is available, end the call without a reservation."
    success: "the bot said neither 7 PM nor 8 PM was available, offered other times, and did not book a table at a time the caller had not accepted"
```

Every simulation key can sit at the top of the file as a default, not only the blocks: `persona:`, `goal:`, `success:`, `metrics:`, `simulator:`, `max_turns:`, `max_duration_s:`, `max_silence_s:`, `runs:`, and the shared `user:`, `judge:`, and `trigger_disconnect:`. A simulation that sets the same key replaces the whole value. So when the caller is the same and only the errand differs, put `persona:` and `success:` at the top with a `goal:` per simulation:
...
When every caller is held to the same bar, put `metrics:` at the top instead. Each simulation runs on its own, against its own bot, as `book_table/available` and `book_table/impossible`, and a suite's `-s book_table` runs them all. The full rules, and a scripted scenario's side of them, are on [Scenario files](/pipecat/evals/scenario-configuration#scenario-files).
