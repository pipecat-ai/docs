...
scenarios:
  - name: book_table # the scenario's name
...
Each scenario is named `<file>/<scenario>`. A common shape for multiple scenarios: the same persona and `success` criterion at the file level, with each scenario giving the caller a different errand via `goal:`:

```yaml
name: diner
persona: "Casey, calling a restaurant. Friendly and to the point."
...
scenarios:
  ...
  - name: cancel
    goal: "Cancel tonight's booking under the name Casey, then end the call."
```
...
user:
  ...
  speech:
    service: kokoro
    voice: af_heart
    speed: 1.0 # optional; Kokoro's rate (default 1.0)
    sample_rate: 16000
