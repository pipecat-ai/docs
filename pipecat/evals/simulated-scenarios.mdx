...
A turn the judge leaves out of its verdicts counts as a no, but is recorded as a verdict of `none` rather than `no`, so a sweep can tell judge trouble from agent trouble: a metric that failed only on unanswered turns carries the `failure_kind` `judge_no_verdict`, one the judge rejected carries `judge_no`. A run in which the agent said nothing has no turns to score, so a judged metric is unscored and passes.
...
`max_turns:` caps the persona's turns (default 20), `max_duration_s:` caps the run's wall clock (default 300 seconds), and `max_silence_s:` caps a lull in which neither side does anything (default 30 seconds), so an agent that never greets, or stops answering, ends the run as `silence` instead of running out the clock. The lull is measured from either side's last activity, and a token or a spoken sentence refreshes it, so a reply in progress is never silence; only nothing happening is. They are backstops against a conversation that never ends, and a run they cut short has not succeeded, whatever the transcript says. A failure of the harness's own pipeline, the persona LLM first among them, ends the run at once as an error.
