...
The development runner is exactly that — a tool for local development. It's deliberately simple: its session-start and transport-webhook endpoints accept requests from anyone, with no authentication, no request signing, and no rate limiting, and it's a single process with no lifecycle management. That's the right set of tradeoffs on your own machine, and the wrong one on a public address, where a single request can spin up bots and paid rooms on your account.

For anything real, put a proper dispatcher or a managed runtime in front of your bots instead of exposing the runner. The next page, [Self-hosting in production](./self-hosting), walks through those options — and, if you only need something reachable for a prototype or alpha test, the minimum you'd want in front of the runner and where that stops being enough.
