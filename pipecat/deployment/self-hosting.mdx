...
<Warning>
  **The development runner (`pipecat.runner.run`) is not built for
  ...
  session-start and transport-webhook endpoints accept requests from anyone,
  with no authentication, no request signing, and no rate limiting — and a
  single request can cause it to create paid resources (rooms, bots) on your
  account. It has no flow control and is a single point of failure.

Production self-hosting means putting one of the options below in front of
