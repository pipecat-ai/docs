...
description: "Build voice agents with OpenAI audio models in Pipecat: STT, TTS, and the Realtime API, and when to use each."
---
...
A cascaded pipeline looks like this, in Pipecat code. Here's a [single-file example that uses a cascaded pipeline](https://github.com/pipecat-ai/pipecat/blob/main/examples/voice/voice-openai.py). (See below for an overview of Pipecat core concepts.)
...
Here's a speech-to-speech pipeline in Pipecat code. And here's a [single-file example that uses the OpenAI Realtime API](https://github.com/pipecat-ai/pipecat/blob/main/examples/realtime/realtime-openai.py).
...
- Pipecat service: `OpenAILLMService` ([reference docs](/api-reference/server/services/llm/openai))
...
- Model: `gpt-live-1`
- Pipecat service: `OpenAILiveLLMService` ([reference docs](/api-reference/server/services/s2s/openai-live))
- OpenAI docs ([overview](https://developers.openai.com/api/docs/guides/live))
- Full-duplex speech-to-speech with delegation to backend models for tools and reasoning

### Speech API

- Models: `gpt-4o-mini-tts`
- Pipecat service: `OpenAITTSService` ([reference docs](/api-reference/server/services/tts/openai))
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/realtime/realtime-openai.py"
  >
    A speech-to-speech implementation using OpenAI's Realtime API
  </Card>

  <Card
    title="OpenAI Live API (Responses Delegation)"
    icon="tower-broadcast"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/realtime/realtime-openai-live-responses-delegation.py"
  >
    OpenAI Live with OpenAI-hosted backend for tools and reasoning
  </Card>

  <Card
    title="OpenAI Live API (Client Delegation)"
    icon="tower-broadcast"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/realtime/realtime-openai-live-client-delegation.py"
  >
    OpenAI Live delegating to a Pipecat backend worker running any LLM
  </Card>
</CardGroup>
