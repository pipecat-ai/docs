...
When `init` asks how you want to build, choose **Scaffold a runnable bot now**. The wizard will guide you through selecting:
...
A browser-based agent with screensharing and vision capabilities, built with a Next.js client and Daily WebRTC transport.

<Note>
  This starter currently uses the legacy Voice UI Kit npm package. For new UI
  development, use [Pipecat UI](/client/pipecat-ui), its replacement with
  components installed as source through the shadcn CLI.
</Note>
...
- Next.js client with legacy Voice UI Kit components
- Resizable panels and event logging

## Deployment

Both starter projects include configuration for [Pipecat Cloud](/pipecat-cloud/introduction), which handles scaling, monitoring, and global deployment.
