...
All three frames take a `run_llm` argument. Set `run_llm=False` to add or change the context **without** prompting the bot to respond.
(Leaving `run_llm` as `None` uses the context aggregator's default behavior.) This is useful when you collect information in the background and don't want the bot to react every time:
```python
