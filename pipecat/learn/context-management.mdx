...
The system prompt is typically set via `system_instruction` in the LLM service's [Settings](/pipecat/fundamentals/service-settings), not as a message in the context. See [System Instruction vs Context System Messages](#system-instruction-vs-context-system-messages) below for details.
...
<Note>
  ...
  strategies](/api-reference/server/utilities/turn-management/user-turn-strategies) and [user
  mute strategies](/api-reference/server/utilities/turn-management/user-mute-strategies) via
