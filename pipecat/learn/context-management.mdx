...
Context can also include tools (function definitions) that the LLM can call during conversations. The preferred way to define a tool is a **direct function** — a single async function that is both the schema and the handler. List your direct functions in `tools` and Pipecat registers their handlers with the LLM automatically:

```python
from pipecat.services.llm_service import FunctionCallParams

# Define a direct function: metadata is auto-derived from the signature and docstring
