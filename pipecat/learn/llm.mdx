...
<Card
  ...
  icon="list"
  href="/api-reference/server/services/supported-services#large-language-models"
>
...
- **`function_call_timeout_secs`** (default `None`): Deadline in seconds for a
  ...
  complete. The deadline covers the handler's execution as a whole, so reporting
  an intermediate result neither clears it nor restarts it. Work the handler
  spawns into a task of its own isn't cancelled with it.
...
Run one-shot LLM inferences outside the pipeline using `run_inference()`:
...
Pass a `response_schema` to enforce a JSON structure:
...
response = await llm.run_inference(context, response_schema=schema)
# Returns JSON text matching the schema
```

Providers that can enforce response schemas (OpenAI, Anthropic, Google) return JSON text matching the schema. Providers or models that cannot (AWS Bedrock, DeepSeek, older OpenAI models) ignore it with a warning.
...
<Card
  ...
  icon="code"
  href="/pipecat/learn/function-calling"
>
