...
Downstream is `push_frame`'s default, so `await self.push_frame(EndWorkerFrame())` says the same thing.

<Note>
  The pipeline source automatically converts `EndWorkerFrame` to `EndFrame` and
  `CancelWorkerFrame` to `CancelFrame`, then pushes the converted frame
  downstream through the entire pipeline, ensuring proper termination handling.
</Note>
...
You now understand how to build, run, and properly terminate voice AI pipelines! Before moving on to multiple agents, let's look at classifiers, which answer the small decisions an agent makes without a conversation LLM turn.
...
>
  Ask yes/no, choice, and score questions and get answers with probabilities
</Card>
