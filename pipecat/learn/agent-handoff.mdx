...
To announce the transfer to the user, deliver a message via `params.result_callback()` before calling `activate_worker()`. The LLM processes the result and speaks the announcement before the handoff completes:

```python
await params.result_callback(f"Transferring you to the {agent} agent.")
await self.activate_worker(
    agent,
    args=LLMWorkerActivationArgs(
        messages=[{"role": "developer", "content": reason}],
