...
<Warning>
  Deciding that a result is unwanted is the model's judgement, and models differ
  in how readily they make it. With this enabled, a user who changes the subject
  while a call is still running may have its result cancelled rather than
  delivered, and the result is then never mentioned. Weigh that against the work
