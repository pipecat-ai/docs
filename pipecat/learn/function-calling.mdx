...
With `cancel_on_interruption=False`, the call is **asynchronous**: the conversation is not held while the function runs. If the result arrives before the conversation has moved on (before any model response, new user message, or intermediate update from the call itself), it settles in place as an ordinary tool result, indistinguishable from a synchronous call. If the result arrives after the conversation has moved on, it's injected back into the context as a developer message, triggering a new LLM inference at that point. This enables truly non-blocking calls where the conversation proceeds while the function runs in the background. Async calls can also send [intermediate updates](#intermediate-results-for-async-functions) before their final result.
