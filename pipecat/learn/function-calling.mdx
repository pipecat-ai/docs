...
The preferred way to define a tool is a **direct function**: a single async function that is _both_ the schema and the handler. Pipecat auto-derives the tool's metadata — name, description, parameter properties (with their descriptions), and which parameters are required — from the function's signature and docstring, so there's no separate schema to write or keep in sync.

The signature differs slightly from a regular function: the first parameter is always `params` (a `FunctionCallParams`), followed by the tool's own arguments. Document each argument in a Google-style docstring.
...
List your direct functions in `LLMContext(tools=[...])`. That's all that's required — Pipecat registers each direct function's handler with the LLM service **automatically**. There's no separate registration call.
...
Include your LLM service in the pipeline. Because the tools are registered through the context, no extra wiring is needed:
...
By default, a direct function is cancelled if the user interrupts and uses the LLM service's global timeout. To override either, decorate the function with `@tool_options`. The decorator only attaches call options — the schema is still auto-derived — so decorated functions can stay at module level.
...
To change the set of tools the LLM can see during a session, push an `LLMSetToolsFrame`. Its `tools` field accepts the same things as `LLMContext` — a plain list of direct functions (and/or `FunctionSchema` objects). Direct functions advertised this way are auto-registered, and any direct function dropped from the advertised set is automatically unregistered.

```python
from pipecat.frames.frames import LLMSetToolsFrame
from pipecat.processors.aggregators.llm_context import NOT_GIVEN

# Advertise a new set of tools (direct-function handlers auto-register)
...
When you use an [`LLMSwitcher`](/api-reference/server/utilities/service-switchers/llm-switcher) to swap LLM providers mid-session, direct functions listed in `LLMContext(tools=[...])` are auto-registered on **every** member LLM — active or not. The tools keep working across switches with no per-member registration.
...
# Direct functions are registered on every member LLM automatically,
# so tools keep working after a switch.
context = LLMContext(tools=[get_current_weather, get_restaurant_recommendation])
user_aggregator, assistant_aggregator = LLMContextAggregatorPair(context)
```
...
Direct functions cover most cases. Reach for the verbose `FunctionSchema` pattern when you need explicit control over the schema — for example, a strict `enum` constraint (which the direct-function generator doesn't yet emit) — or when the tool's handler isn't a direct function.

With this pattern you define the schema and the handler separately, then link them by registering the handler.
...
# 2. Define the handler (arguments come from params.arguments)
async def fetch_weather_from_api(params: FunctionCallParams):
    weather_data = {"conditions": "sunny", "temperature": "75"}
    await params.result_callback(weather_data)

# 3. Register the handler, linking it to the schema by name
...
# 4. Advertise the schema via the context
context = LLMContext(tools=[weather_function])
```

`register_function` takes the same `cancel_on_interruption` and `timeout_secs` options as [`@tool_options`](#per-tool-options-with-@tool_options), with the same [synchronous vs. asynchronous](#synchronous-vs-asynchronous-calls) semantics. As with direct functions, you can change `FunctionSchema` tools mid-conversation by pushing an `LLMSetToolsFrame`.

<Note>
  `register_direct_function()` and `unregister_direct_function()` are
  **deprecated** (since 1.4.0). Direct functions now register automatically from
  the advertised tool set — list them in `LLMContext(tools=[...])` for tools
  available at session start, or push an `LLMSetToolsFrame` to change tools
  mid-session.
</Note>
...
- **Direct functions are the preferred approach** - one async function is both schema and handler, registered automatically via `LLMContext(tools=[...])`; use `FunctionSchema` + `register_function` when you need explicit schema control
