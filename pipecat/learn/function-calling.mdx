...
- **`cancel_on_interruption`** (default `True`): When `True`, the call is cancelled if the user interrupts. When `False`, the call is treated as **asynchronous**: the conversation is not held while it runs, and results that arrive quickly settle in place like synchronous calls — see below.
- **`timeout_secs`** (default `None`): Per-tool timeout in seconds. Overrides the global `function_call_timeout_secs` for this function. Use a longer timeout for slow operations (e.g. database queries) or a shorter one for quick lookups. A call that runs past its deadline is cancelled: the handler is thrown an `asyncio.CancelledError` so it can clean up, the call settles as cancelled, and inference runs so the bot can say it didn't complete. The deadline covers the handler's execution as a whole, so reporting an intermediate result neither clears it nor restarts it — only a final result settles the call and clears the timeout. Work the handler spawns into a task of its own isn't cancelled with it.
...
With `cancel_on_interruption=False`, the call is **asynchronous**: the conversation is not held while the function runs. If the result arrives before the conversation has moved on (before a new user message, developer message, or intermediate update from the call itself), it settles in place as an ordinary tool result, indistinguishable from a synchronous call. Assistant messages — text the model generates or filler spoken with a `TTSSpeakFrame` while the call runs — do not cause deferral. If the result arrives after the conversation has moved on, it's injected back into the context as a developer message, triggering a new LLM inference at that point. This enables truly non-blocking calls where the conversation proceeds while the function runs in the background. Async calls can also send [intermediate updates](#intermediate-results-for-async-functions) before their final result.
...
If the handler carries [`@tool_options`](#per-tool-options-with-@tool_options), `register_function` honors it the same way bundling does — or pass `cancel_on_interruption` / `timeout_secs` / `cancellable_by_llm` to `register_function` directly to override.
...
Intermediate results are injected into the LLM context as async-tool developer messages. They do not close the function call; the call remains in progress until the final result is sent. If the function has a `timeout_secs` deadline, intermediate results do not clear or restart it — only the final result clears the timeout.
