...
## Suppressing Text After Tool Calls

Some models stream acknowledgement text after requesting a tool. In a voice
pipeline, that text can reach TTS before the tool result is available. Set
`tool_call_text_policy` to `ToolCallTextPolicy.SUPPRESS_AFTER_TOOL_CALL_DETECTED` to
omit text emitted after the first tool-call fragment in a response.

Text emitted before the tool call is preserved. The response triggered by the
tool result is unaffected. The default policy, `ToolCallTextPolicy.PRESERVE`,
keeps all streamed text.

This setting is available on all text-based LLM services: OpenAI-compatible
services (OpenAI, Azure, Groq, Fireworks, DeepSeek, and the rest),
`OpenAIResponsesLLMService`, `AnthropicLLMService`, `GoogleLLMService` (and
the Vertex variants), and `AWSBedrockLLMService`. Realtime speech-to-speech
services don't stream TTS-bound text, so the setting has no effect there.
...
llm = OpenAILLMService(
    ...
    settings=OpenAILLMService.Settings(
        model="gpt-4.1",
        tool_call_text_policy=ToolCallTextPolicy.SUPPRESS_AFTER_TOOL_CALL_DETECTED,
    ),
)
```
