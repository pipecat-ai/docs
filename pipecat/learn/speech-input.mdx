...
<Note>
  **Why is VAD configured on the aggregator?** VAD lives in
  `LLMUserAggregatorParams` because its speech start/stop signals feed the [user
  turn
  strategies](/api-reference/server/utilities/turn-management/user-turn-strategies)
  that decide when a turn begins and ends. VAD runs on the raw input audio, so
  where the aggregator sits in your pipeline relative to STT does not change
  what VAD sees. For advanced setups (for example, VAD-only pipelines or sharing
  one analyzer across multiple aggregators) a standalone `VADProcessor` is also
  available, but configuring `vad_analyzer` on the aggregator is the recommended
  default.
</Note>
...
<Tip>
  `confidence` and `min_volume` are the in-VAD levers for noisy audio, but they
  only raise the bar for what counts as speech. If your bot keeps reacting to
  background voices or your STT keeps transcribing noise, remove the noise
  upstream with an input audio filter such as [Krisp
  VIVA](/pipecat/features/krisp-viva) or
