...
1. **Streamed LLM tokens** via `LLMTextFrame`s:
   ...
   - Sentence detection follows the TTS `language` setting, defaulting to English when unspecified
...
<Card
  ...
  icon="list"
  href="/api-reference/server/services/supported-services#text-to-speech"
>
...
Many TTS services support pronunciation hints that let you control how specific words are spoken using the International Phonetic Alphabet (IPA). Each service provides a `pronunciation_transform_ipa()` classmethod that creates a text transform from a dictionary mapping words to their IPA pronunciations:

```python
from pipecat.services.cartesia.tts import CartesiaTTSService

# Define pronunciations for medical terms
pronunciations = {
    "Metformin": "mɛtˈfɔɹmɪn",
    "Adalimumab": "ˌædəˈlɪmumæb"
}

# Create pronunciation transform
pronounce = CartesiaTTSService.pronunciation_transform_ipa(pronunciations)

tts = CartesiaTTSService(
    api_key=os.getenv("CARTESIA_API_KEY"),
    text_transforms=[
        ("*", pronounce),  # Register last so nothing rewrites its markup
    ],
)
```
...
- Each word is matched whole and case-insensitively wherever it appears in the text
- Matched words are replaced with the service's pronunciation markup (e.g., Cartesia's `<<phoneme>>` blocks, ElevenLabs' SSML `<phoneme>` tags)
- Words that cannot be used are reported once when the transform is built and spoken as written
- The transform is automatically skipped when the service doesn't support pronunciation markup

**Important:** Register pronunciation transforms **last** in `text_transforms`. Transforms run in order, so one that runs after it (stripping markdown, replacing text) can rewrite the markup and break the hint.

**Service support:**

- **Cartesia**: Inline phoneme blocks (`<<…>>`)
- **Deepgram Aura-2**: Inline pronunciation objects (WebSocket and HTTP)
- **Deepgram Flux**: No pronunciation support
- **ElevenLabs**: SSML `<phoneme>` tags (v2 models only with `enable_ssml_parsing=True` for WebSocket)
- **ElevenLabs Dialogue**: IPA between slashes (v3 models)
- **Inworld**: IPA between slashes (single words only)
...
<Tip>
  For hundreds of overrides (medication names, product names, place names), see
  [Pronunciation Lexicons](/pipecat/fundamentals/pronunciation-lexicons).
</Tip>
...
<Card
  ...
  icon="arrow-right"
  href="/pipecat/learn/pipeline-termination"
>
