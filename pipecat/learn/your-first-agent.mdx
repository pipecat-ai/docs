...
description: "Build and run a single agent: a PipelineWorker, the WorkerRunner, and the pipeline lifecycle."
---
...
<Note>
  ...
  `PipelineTask` is a deprecated alias for `PipelineWorker`, and `PipelineRunner`
  for `WorkerRunner`, so existing `PipelineRunner().run(task)` code keeps working
  unchanged. New code should use `PipelineWorker` and `WorkerRunner` directly.
</Note>
