...
- Appends silence padding to each segment so models finish the last word
- Sends audio segments to STT service as wav files
- Transcribes segments off the audio path in a background task
- Audio frames continue flowing while transcription is in progress
- Transcripts are pushed in segment order as each completes
...
<Card
  ...
  icon="list"
  href="/api-reference/server/services/supported-services#speech-to-text"
>
...
<CardGroup cols={3}>
  ...
  <Card
    title="Soniox"
    icon="language"
    href="/api-reference/server/services/stt/soniox"
  >
    Real-time streaming STT with strong multilingual support and language hints
  </Card>
  <Card
    title="AssemblyAI"
    icon="brain"
  ...
  <Card
    title="Speechmatics"
    icon="waveform"
    href="/api-reference/server/services/stt/speechmatics"
  >
  ...
  <Card
    title="Cartesia"
    icon="bolt"
    href="/api-reference/server/services/stt/cartesia"
  >
    Low-latency real-time transcription from the Cartesia voice platform
  </Card>
  <Card
    title="Gradium"
    icon="sparkles"
    href="/api-reference/server/services/stt/gradium"
  >
    Real-time streaming STT with a low-latency transcription API
  </Card>
</CardGroup>
...
- **`audio_passthrough=True`**: Allows `InputAudioRawFrame`s (user audio) to continue downstream to other processors. `OutputAudioRawFrame`s (sound effects, recordings) are always passed through regardless of this setting.
...
<Warning>
  Setting `audio_passthrough=False` will stop `InputAudioRawFrame`s (user audio)
  from being passed downstream, which may break audio recording or other
  audio-dependent processors. `OutputAudioRawFrame`s (sound effects, recordings
  played into the pipeline) are always passed through regardless of this
  setting.
</Warning>

### SegmentedSTTService Configuration

Services that use HTTP-based segmentation (such as Whisper, OpenAI STT, Groq STT, and Moonshine STT) inherit from `SegmentedSTTService` and support an additional configuration option:

```python
stt = WhisperSTTService(
    # Service-specific options...
    trailing_silence_secs=0.5,  # Silence appended to each segment (default)
)
```

**Key option:**

- **`trailing_silence_secs`** (default `0.5`): Seconds of silence appended to each speech segment before transcription. Models tend to drop or garble the final word when audio ends abruptly at the VAD cutoff, so padding lets the model hear the end of speech and finish the last word. The padding is submitted to the provider and counts toward STT usage metrics. Set to `0` to disable, or tune to another value if needed.
...
**`language="any"`** — Gradium uses `"any"` for language detection (defaults to English otherwise):
...
stt = GradiumSTTService(
    ...
    settings=GradiumSTTService.Settings(
        language="any",  # Auto-detect language
    ),
)
```
...
<Card
  ...
  icon="gauge-high"
  href="/pipecat/fundamentals/stt-latency-tuning"
>
