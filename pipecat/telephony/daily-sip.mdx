...
<Info>
  **SIP URI dial-out does not require dial-out approval.** Daily gates approval on the destination,
  not the provider: a destination starting with `+` (a phone number) requires dial-out approval on
  your domain, a `sip:` URI does not. You still need `enable_dialout=True` on the room in both cases.
</Info>
