...
This starts a FastAPI server on port 7860 with a `/daily-dialin-webhook` endpoint. `--dialin` no longer requires `-t daily`; passing `-t daily` simply restricts the runner to the Daily transport.
...
from pipecat.transports.daily.transport import DailyTransport, DailyParams, DailyDialinSettings
...
<Warning>
  Dial-out to a phone number requires approval on your Daily domain. Fill out the
  [dial-out request form](https://docs.google.com/forms/d/e/1FAIpQLSe3hTnIchbS1NgOsO8HGrzyBk5B1WuwWT_qqfNQde14_L-Y_Q/viewform) to request it.

Dial-out to a SIP address does not require this approval. A SIP address is a URI that begins with the ‎`sip:` scheme (for example, ‎`sip:1234@sip.example.com`); you pass it to the `start_dialout` in the ‎`sip_uri` parameter. See
[Daily SIP](/pipecat/telephony/daily-sip).
...
<Note>
  The bot is the room owner, so when it leaves, the Daily room terminates and
  any remaining PSTN legs drop — there's no "bot leaves, the two callers keep
  talking" mode (`max_idle_timeout_sec` only holds a bot-less room open for a
  fixed countdown). To keep a caller and a human on Daily together, use a warm
  transfer (the bot stays as the bridge); to drop the bot entirely, use a cold
  transfer off Daily via SIP REFER.
</Note>
