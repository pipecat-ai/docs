...
<Info>
  Which approval applies depends on the destination you pass to `start_dialout`, not on Twilio. A
  `sip:` URI needs no dial-out approval. An E.164 number (`+...`) needs dial-out approval on your
  Daily domain, via the [dial-out request form](https://docs.google.com/forms/d/e/1FAIpQLSe3hTnIchbS1NgOsO8HGrzyBk5B1WuwWT_qqfNQde14_L-Y_Q/viewform).
</Info>
...
- See [Daily SIP](./daily-sip) for the provider-agnostic `provider="daily"` flow that works with any SIP carrier and includes DTMF send/receive
