...
description: "Build and run your first Pipecat voice AI bot in under 5 minutes: a simple conversational agent you can talk to in a browser."
---
...
# Scaffold the quickstart project (also writes AGENTS.md + CLAUDE.md)
...
async def on_client_disconnected(transport, client):
    ...
    # Cancel the runner when the client disconnects
    # This stops the pipeline and all processors, cleaning up resources
    await runner.cancel()
```

When the client signals it's ready over the RTVI protocol, the bot adds a greeting instruction and queues an `LLMRunFrame` to start the conversation. When the client disconnects, the runner is cancelled to clean up resources.
