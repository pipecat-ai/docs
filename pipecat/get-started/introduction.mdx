...
The bot above is a single agent. Pipecat is also a multi-agent system: the same `PipelineRunner` can coordinate many agents that each run their own pipeline and communicate over a shared message **bus**. A normal bot is just the one-agent case, so every Pipecat app is multi-agent ready.
...
<CardGroup cols={3}>
  ...
  <Card
    ...
    icon="server"
    href="/pipecat/fundamentals/distributed-agents"
  >
