...
<ParamField path="session_properties" type="SessionProperties" default="None">
  Configuration properties for the realtime session. If `None`, uses default `SessionProperties` with voice `"Ara"` and server-side VAD enabled. See [SessionProperties](#sessionproperties) below.
</ParamField>
