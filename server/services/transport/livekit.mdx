...
When an interruption occurs (typically triggered by the user starting to speak), LiveKitTransport automatically clears the internal LiveKit AudioSource buffer. This ensures the bot stops speaking immediately without continuing to play out queued audio. The interruption handling works seamlessly with Pipecat's standard interruption mechanisms like VAD-based user turn detection.
