...
description: "WebSocket client transport implementation for real-time audio communication"
---

## Overview

`WebsocketClientTransport` provides a WebSocket client implementation for real-time audio communication. It supports bidirectional audio streams and voice activity detection (VAD).

<Warning>
WebsocketClientTransport is best suited for server-side applications and prototyping client/server apps.
...
To use `WebsocketClientTransport`, install the required dependencies:
