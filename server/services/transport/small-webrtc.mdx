...
<Warning>
  ...
  production deployments with scale, consider using
  [DailyTransport](/server/services/transport/daily) or LiveKitTransport, as
  these WebRTC providers have global, low-latency infrastructure.
</Warning>
...
- Output audio can be configured to match your application's requirements
...
3. For testing across networks, consider using Daily or LiveKit which provide TURN servers
