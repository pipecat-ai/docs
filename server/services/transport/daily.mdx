...
<ResponseField name="send_message" type="async method">
  Sends a message to participants in the Daily room.
...
await transport.send_message(TransportMessageFrame(message="Hello everyone!"))
...
When using the DailyTransport, if you provide a token with owner permissions to the bot, it can control permissions for other participants in the session. With this control, the bot can modify settings for how others send and receive media, how they are seen in a session, and whether they have Administrative controls.
...
- `remote_participants`: Dictionary mapping participant IDs to their updated settings
...
- `RemoteParticipantUpdates`: Maps participant IDs to their updates
  - `ParticipantUpdate`: Contains permissions and input settings
    - `permissions`: Defines what a participant can do
      - `hasPresence`: Whether participant appears in the room
      - `canAdmin`: Administrative permissions (participants, streaming, transcription)
      - `canSend`: Media types the participant can send ("camera", "microphone", etc.)
      - `canReceive`: Controls whose media the participant can receive
        - `base`: Default receive permission for all participants
        - `byUserId`: User-specific receive permissions
        - `byParticipantId`: Participant-specific receive permissions
    - `inputsEnabled`: Controls whether inputs are enabled for the participant
...
# Configure selective audio routing with custom streams
await transport.update_remote_participants({
    "customer-id": {
        "permissions": {
            "canReceive": {
                ...
                "byUserId": {
                    "bot": {
                        "customAudio": {"hold-music": True}  # Only receive hold music
                    }
                }
            }
        }
...
# Set up user-to-user permissions mapping (e.g., for transferring calls)
await transport.update_remote_participants({
    "customer-id": {
        "permissions": {
            "canSend": ["microphone"],
            "canReceive": {"byUserId": {"agent-id": True}}
        },
        "inputsEnabled": {"microphone": True}
    },
    "agent-id": {
        "permissions": {
            "canReceive": {"byUserId": {"customer-id": True}}
        }
    }
})
```

See [update_remote_participants](https://reference-python.daily.co/api_reference.html#daily.CallClient.update_remote_participants) in the Daily API documentation for comprehensive details on permission settings.
