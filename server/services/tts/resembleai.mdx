...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/07zk-interruptible-resemble.py"
  >
...
<ParamField path="voice_id" type="str" required deprecated>
  Voice UUID to use for synthesis. _Deprecated in v0.0.105. Use
  `settings=ResembleAITTSService.Settings(voice=...)` instead._
</ParamField>
...
Runtime-configurable settings passed via the `settings` constructor argument using `ResembleAITTSService.Settings(...)`. These can be updated mid-conversation with `TTSUpdateSettingsFrame`. See [Service Settings](/guides/fundamentals/service-settings) for details.
...
Resemble AI TTS supports the standard [service connection events](/server/events/service-events):
