...
Inworld provides high-quality, low-latency speech synthesis via two implementation types: `InworldTTSService` for streaming and non-streaming using HTTP and `InworldWebsocketTTSService` for real-time, minimal-latency use-cases. Featuring support for 12+ languages, word/character timestamps, custom pronunciation and instant voice cloning.
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/foundational/07ab-interruptible-inworld-websocket.py"
  >
