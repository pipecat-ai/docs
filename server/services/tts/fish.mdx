...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/07t-interruptible-fish.py"
  >
...
Runtime-configurable settings passed via the `settings` constructor argument using `FishAudioTTSService.Settings(...)`. These can be updated mid-conversation with `TTSUpdateSettingsFrame`. See [Service Settings](/guides/fundamentals/service-settings) for details.
...
Fish Audio TTS supports the standard [service connection events](/server/events/service-events):
