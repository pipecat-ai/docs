...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/07k-interruptible-lmnt.py"
  >
...
<ParamField path="voice_id" type="str" required deprecated>
  ...
  `settings=LmntTTSService.Settings(voice=...)` instead._
</ParamField>

<ParamField path="model" type="str" default="blizzard" deprecated>
  LMNT TTS model to use. _Deprecated in v0.0.105. Use
  `settings=LmntTTSService.Settings(model=...)` instead._
</ParamField>
...
<ParamField path="language" type="Language" default="Language.EN" deprecated>
  ...
  Spanish, French, Hindi, and more. _Deprecated in v0.0.106. Use
  `settings=LmntTTSService.Settings(language=...)` instead._
</ParamField>
...
Runtime-configurable settings passed via the `settings` constructor argument using `LmntTTSService.Settings(...)`. These can be updated mid-conversation with `TTSUpdateSettingsFrame`. See [Service Settings](/guides/fundamentals/service-settings) for details.
...
LMNT TTS supports the standard [service connection events](/server/events/service-events):
