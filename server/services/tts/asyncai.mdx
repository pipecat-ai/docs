...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/07zc-interruptible-asyncai.py"
  >
...
<ParamField path="voice_id" type="str" required deprecated>
  UUID of the voice to use for synthesis. _Deprecated in v0.0.105. Use
...
<ParamField path="params" type="InputParams" default="None" deprecated>
  _Deprecated in v0.0.105. Use `settings=AsyncAITTSService.Settings(...)`
  instead._
</ParamField>
...
Runtime-configurable settings passed via the `settings` constructor argument using `AsyncAITTSService.Settings(...)`. These can be updated mid-conversation with `TTSUpdateSettingsFrame`. See [Service Settings](/guides/fundamentals/service-settings) for details.
...
Async TTS supports the standard [service connection events](/server/events/service-events):
