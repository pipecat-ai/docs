...
To disable this behavior, set `enable_endpoint_detection` in the `SonioxInputParams` to `false`.
...
To enable this behavior, set `enable_vad` to `True`.

It is generally recommended to use either Voice Activity Detection or automatic pause detection, but not both.
