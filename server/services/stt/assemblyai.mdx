...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="play"
    href="https://github.com/pipecat-ai/pipecat/blob/main/examples/foundational/07o-interruptible-assemblyai-turn-detection.py"
  >
...
### With External VAD Alignment

When using external VAD (e.g., Silero) alongside AssemblyAI's u3-rt-pro, align the VAD thresholds to prevent synchronization issues:
...
stt = AssemblyAISTTService(
    ...
    connection_params=AssemblyAIConnectionParams(
        speech_model="u3-rt-pro",
        vad_threshold=0.5,  # Match this with your external VAD's activation threshold
    ),
)
```
...
- **VAD threshold alignment**: When using external VAD (e.g., Silero) with AssemblyAI's u3-rt-pro, set `vad_threshold` in `connection_params` to match your VAD's activation threshold. This prevents the "dead zone" where AssemblyAI transcribes speech that your VAD hasn't detected yet, improving synchronization between the two systems.
