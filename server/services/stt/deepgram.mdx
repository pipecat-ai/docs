...
<ParamField path="live_options" type="LiveOptions" default="None">
  Pipecat's `LiveOptions` for detailed configuration. When provided, these
  settings are merged with the defaults. Import from
  `pipecat.services.deepgram.stt`. See [Deepgram
...
| `endpointing`      | `None`             | Endpointing sensitivity in ms, or `False` to disable. |
...
- **SDK v6 breaking change**: As of pipecat 0.0.104, `DeepgramSTTService` uses deepgram-sdk v6. The `LiveOptions` class was removed from the SDK and is now provided by pipecat. Import it from `pipecat.services.deepgram.stt` instead of `deepgram`.
- **Keepalive**: The deepgram-sdk v6 removed automatic keepalive. Pipecat now sends explicit `KeepAlive` messages every 5 seconds to prevent Deepgram's 10-second inactivity timeout.
