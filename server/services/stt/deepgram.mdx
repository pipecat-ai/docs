...
<ParamField path="live_options" type="LiveOptions" default="None">
  Deepgram `LiveOptions` for detailed configuration. When provided, these settings are merged with the defaults. See [Deepgram LiveOptions](https://developers.deepgram.com/docs/live-streaming-audio#live-streaming-options) for available options.
</ParamField>
...
<ParamField path="params" type="InputParams" default="None">
  Configuration parameters for the Flux API. See [Flux InputParams](#flux-inputparams) below.
</ParamField>
