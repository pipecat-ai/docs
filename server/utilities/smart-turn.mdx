...
<Tip>
  Smart Turn Detection works best when paired with a VAD analyzer set to a short `stop_secs` value. We recommend 0.2 seconds. This allows the VAD to quickly detect short pauses, giving the Smart Turn model the opportunity to analyze if the turn is actually complete.
