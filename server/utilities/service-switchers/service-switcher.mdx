...
You can create your own switching strategy by subclassing `ServiceSwitcherStrategy` and implementing the `handle_frame` method. This method receives a `ServiceSwitcherFrame` and should return the newly active service if a switch occurred, or `None` otherwise.
