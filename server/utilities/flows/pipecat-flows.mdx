...
<ResponseField name="NodeConfig" type="TypedDict">
  ...
  <Expandable title="Fields">
    ...
    <ResponseField name="functions" type="List[dict]" required>
      ...
      - `start_callback`: a callback to execute before the function is called.
      ...
      "functions": [
          {
              ...
              "function": {
                  ...
                  "start_callback": before_get_movies,  # Optional callback before execution
      ...
      "functions": [
          {
              ...
              "function": {
                  ...
                  "start_callback": before_collect_age,  # Optional callback before execution
...
<ResponseField name="start_callback" type="Optional[Callable]">
  Handler that executes before a function is called. Must be an async function with this signature:

```python
async def before_function_execution(
    function_name: str,
    llm: Any,
    context: Any
) -> None:
...
async def before_check_availability(function_name: str, llm: Any, context: Any) -> None:
    ...
    await llm.push_frame(
        TTSSpeakFrame("Let me check our reservation system for that time...")
    )
...
Start callbacks are useful for:

- Providing immediate feedback while long-running operations complete
- Acknowledging user requests before processing begins
- Adding wait messages for API calls or database operations
