...
You will need to download the latest model version from the following repository:

https://huggingface.co/pipecat-ai/smart-turn-v3/tree/main

At the time of writing, the latest version of the model is `smart-turn-v3.0.onnx`. Pass the path to this file into the `LocalSmartTurnAnalyzerV3` constructor.
...
smart_turn_model_path = os.getenv("ONNX_FILE_PATH")
