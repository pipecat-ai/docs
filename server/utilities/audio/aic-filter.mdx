...
<ParamField path="enhancement_level" type="Optional[float]" default="None">
  Overall enhancement strength from `0.0` (no enhancement) to `1.0` (maximum enhancement).
  If `None`, the model's default behavior is used. This parameter allows you to control
  the intensity of the speech enhancement applied by the model.
</ParamField>
