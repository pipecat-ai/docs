...
<ParamField path="end_of_turn_analyzer" type="Optional[BaseSmartTurn]" default="None">
  Smart Turn Detection analyzer for advanced end-of-turn detection. You can set this to either 
  `SmartTurnAnalyzer()` for remote inference or `LocalCoreMLSmartTurnAnalyzer()` for local inference.
  
  For best results, use with VAD enabled and configured with a short pause detection (e.g., `stop_secs=0.2`).
