...
<ParamField path="--region / -r" type="string" default="us-west">
  Region where the agent will be deployed. If not specified, defaults to
  `us-west`. Choose a region close to your users for optimal latency.
</ParamField>
...
The deploy command will look for any values declared in an adjacent `pcc-deploy.toml` file (documented [here](/deployment/pipecat-cloud/fundamentals/deploy#using-pcc-deploy-toml)).
