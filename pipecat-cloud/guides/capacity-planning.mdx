...
- **Idle Agents**: Spare agent instances that Pipecat Cloud runs on top of the limits you set. If you keep warm instances (`--min-agents` 1 or higher), some are always there, even with zero active sessions, and more are added as your active sessions grow. A new one takes about 30 seconds to become available, depending on system load and image size. You are not charged for idle agents.
...
- When **Reserved > Active**: Your warm capacity is at least your number of **Reserved Agents**
- When **Active ≥ Reserved**: Your warm capacity is at least your number of **Active Agents** (through automatically provisioned idle agents)
...
These are floors, not exact counts. The real instance count is usually a little higher because of spare instances, so plan with these numbers and don't expect the reported count to match them exactly.
