...
## WebSocket telephony session starts

The formula above sizes how many sessions can run at once. It does not set how fast new WebSocket sessions can start.

WebSocket connections from telephony providers pass through a shared ingress layer with its own capacity, separate from your agent pool. Each connection stays on the ingress instance that accepted it for the life of the call. This means `min-agents` and `max-agents` do not raise how fast new WebSocket sessions can be admitted.

When calls arrive faster than the ingress layer can admit them, sessions are admitted late. You may see:

- Sessions that start well after the call came in
- The phone leg hanging up before the bot connects
- A bot that starts for a call that has already ended

If you plan a load test or a launch with a high session-start rate (more than a few calls per second) over Twilio, Telnyx, Plivo or Exotel WebSockets, contact us at [help@daily.co](mailto:help@daily.co) ahead of time so we can provision for it.
