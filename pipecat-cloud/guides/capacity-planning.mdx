...
- **Idle Agents**: Spare agent instances that Pipecat Cloud runs for you on top of the limits you set. Some are always there, even with zero active sessions, and more are added as your active sessions grow. A new idle agent takes approximately 30 seconds to become available, though this time may vary based on system load and image size. You are not charged for these idle agents.
...
<Note>
  Because idle agents sit on top of your configured limits, the instance count
  you can observe (the `readyReplicas` value in the API response, or the count
  in the Pipecat Cloud Dashboard) is normally higher than your `max-agents`
  setting, including at zero traffic. It does not raise your session limit and
  it does not change your bill. See [Instance count vs. configured
  agents](../fundamentals/scaling#instance-count-vs-configured-agents).
</Note>
...
The system ensures you always have at least the following warm capacity available:

- When **Reserved > Active**: You get at least as much warm capacity as your number of **Reserved Agents**
- When **Active ≥ Reserved**: You get at least as much warm capacity as your number of **Active Agents** (through automatically provisioned idle agents)

To illustrate the point, here are a few scenarios showing the warm capacity you can plan on:

| Reserved | Active | Warm Capacity (at least) |
| -------- | ------ | ------------------------ |
| 10       | 1      | 10                       |
| 10       | 10     | 10                       |
| 1        | 10     | 10                       |
...
These are floors, not exact counts. The real number of running instances is usually a little higher, because Pipecat Cloud keeps spare instances on top of your limits. Plan with the numbers in this table, and don't expect the instance count you see reported to match them exactly.
