---
title: "Plivo WebSockets on Pipecat Cloud"
sidebarTitle: "Plivo Websocket Transport"
description: "Receive Plivo phone calls in Pipecat Cloud agents over Plivo's WebSocket transport: XML setup, serializers, and configuration."
---
...
<Note>
  ...
  guide](/pipecat/telephony/plivo-websockets).
</Note>
...
Pipecat Cloud implements Plivo's bidirectional [Media Streaming protocol](https://www.plivo.com/docs/voice/audio-streaming/overview). While audio streams flow through WebSockets, the call session is controlled by XML responses that tell Plivo how to handle each call.
...
Learn more about [regional endpoints](/pipecat-cloud/guides/regions#regional-websocket-endpoints).
...
[Purchase a phone number from Plivo](https://www.plivo.com/docs/numbers) if you haven't already, then:
...
<Note>
  WebSocket connections pass through a shared ingress layer with its own
  capacity, separate from your agent pool. If you plan a load test or a launch
  with a high session-start rate (more than a few calls per second), contact us
  at [help@daily.co](mailto:help@daily.co) ahead of time. See [WebSocket
  telephony session
  starts](/pipecat-cloud/guides/capacity-planning#websocket-telephony-session-starts).
</Note>
...
For complete implementation details including dial-out and advanced call control features, see the [Plivo WebSocket Integration guide](/pipecat/telephony/plivo-websockets).
