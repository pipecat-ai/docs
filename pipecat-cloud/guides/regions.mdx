...
- **Self-hosted regions** run in a Kubernetes cluster you operate and appear in your organization's region list once registered. They are available with [Pipecat Enterprise](/pipecat-cloud/enterprise/overview).
...
The listing also shows each region's supported CPU architectures and its default. Daily-hosted regions are `arm64`; a self-hosted region declares what its nodes provide. See [architecture](/pipecat-cloud/enterprise/sizing#architecture) for how deploys target one.
...
- **Secret sets are region-specific (with globally unique names)**: Create a separate secret set in each region where you deploy agents. In a self-hosted region, secrets are [referenced from your cluster](/pipecat-cloud/enterprise/secrets) rather than created.
...
- [Pipecat Enterprise](/pipecat-cloud/enterprise/overview) - Run a region in your own cluster
