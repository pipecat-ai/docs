...
description: "Deploy Pipecat Cloud agents to geographic regions for lower latency and data residency: available regions and configuration."
---

Pipecat Cloud supports deploying agents to geographic regions, allowing you to reduce latency for your users and meet data residency requirements. Regions come in two kinds:

- **Daily-hosted regions**, listed below, run on Daily's infrastructure and are available to every organization.
- **Self-hosted regions** run in a Kubernetes cluster you operate and appear in your organization's region list once registered. They are an enterprise feature; see [Self-Hosted Regions](/pipecat-cloud/self-hosted/overview).

Everything on this page applies to both kinds unless noted.
...
The listing also shows each region's supported CPU architectures and its default. Daily-hosted regions are `arm64`; a self-hosted region declares what its nodes provide. See [architecture](/pipecat-cloud/self-hosted/sizing#architecture) for how deploys target one.
...
- **Secret sets are region-specific (with globally unique names)**: Create a separate secret set in each region where you deploy agents. In a self-hosted region, secrets are [referenced from your cluster](/pipecat-cloud/self-hosted/secrets) rather than created.
...
<Tip>
  ...
  reference](/api-reference/cli/cloud/deploy#configuration-file-pcc-deploy-toml)
...
- [Self-Hosted Regions](/pipecat-cloud/self-hosted/overview) - Run a region in your own cluster
