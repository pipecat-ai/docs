...
sidebarTitle: "Session API"
description: "Send HTTP requests directly to running Pipecat Cloud agent sessions through the session API proxy for custom control."
---
...
1. Store a reference to your [`PipelineWorker`](/api-reference/server/pipeline/pipeline-worker) in a module-level variable
2. Access it from your endpoint handler to [queue frames](/api-reference/server/pipeline/pipeline-worker#frame-management) into the pipeline
...
Use [`LLMMessagesAppendFrame`](/api-reference/server/pipeline/pipeline-worker#frame-management) to add messages to the LLM's conversation context from an HTTP request. Setting `run_llm=True` tells the pipeline to immediately send the updated context to the LLM for a response.
...
Use [`TTSSpeakFrame`](/api-reference/server/pipeline/pipeline-worker#frame-management) to make the bot say something on demand:
