---
title: "Starting Pipecat Cloud Sessions"
sidebarTitle: "Starting Sessions"
description: "Start on-demand Pipecat Cloud agent sessions: how sessions are created per request and scaled by your deployment configuration."
---
...
Pool size sets how many sessions can run at once. It does not set how fast new sessions can start over a telephony WebSocket. Those connections pass through a shared ingress layer with its own capacity, so `min-agents` and `max-agents` do not raise how fast new WebSocket sessions can be admitted. See [WebSocket telephony session starts](../guides/capacity-planning#websocket-telephony-session-starts).
...
<Tabs>
  <Tab title="CLI">
    ...
    See [`--max-session-duration`](/api-reference/cli/cloud/deploy#param-max-session-duration) for details.
