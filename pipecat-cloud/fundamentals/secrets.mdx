...
description: "Store API keys and sensitive configuration in Pipecat Cloud secret sets, and expose them to your agents as environment variables."
---
...
Each secret set has one of four readiness states:
...
| `unknown` | Readiness could not be verified because the region was unreachable. Applies to [referenced secrets](/pipecat-cloud/self-hosted/secrets) in self-hosted regions; retry shortly. |
...
are gated on the `ready` state — a request to deploy with a set in any other
state is rejected with HTTP `409 Conflict` rather than failing later when the
agent starts.

Secret set responses also carry a `source` field: `managed` for sets whose
values Pipecat Cloud stores, and `referenced` for sets that point at a Secret
in a [self-hosted region](/pipecat-cloud/self-hosted/secrets).
...
<Note>
  Managed secret sets are available in Daily-hosted regions. In a self-hosted
  region, secret values stay in your cluster: create a Kubernetes Secret and
  [reference it](/pipecat-cloud/self-hosted/secrets) instead.
</Note>
