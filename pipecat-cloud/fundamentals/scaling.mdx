...
<Note>
  ...
  and memory; it does not let one instance handle more than one session. This
  covers how many sessions run at once, not how fast new sessions can start
  over a telephony WebSocket. See [WebSocket telephony session
  starts](../guides/capacity-planning#websocket-telephony-session-starts).
</Note>
...
<Note>
  Each deployment made to a Daily-hosted region has a maximum allowed pool size
  of 50. If you require more capacity, fill out the [concurrency increase
  request
  ...
  [Discord](https://discord.gg/dailyco). This limit does not apply to
  [self-hosted regions](/pipecat-cloud/enterprise/sizing#scaling-limits), where
  pool size is bounded only by your cluster's capacity.
</Note>
...
For calls that arrive over a telephony WebSocket, a high session-start rate is a separate limit from pool size. See [WebSocket telephony session starts](../guides/capacity-planning#websocket-telephony-session-starts).
