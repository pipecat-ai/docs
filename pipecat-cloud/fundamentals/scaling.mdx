...
<Warning>
  Scaling changes are applied asynchronously. A successful response means the
  change was accepted, not that new warm instances are ready. If you raise
  `min-agents` and start sessions seconds later, those sessions can still hit a
  [cold start](#cold-starts). See [Changing min-agents at
  Runtime](../guides/capacity-planning#changing-min-agents-at-runtime) for how
  to check that the new capacity is ready before you send traffic.
</Warning>
