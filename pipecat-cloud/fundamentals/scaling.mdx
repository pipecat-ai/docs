...
<Note>
  ...
  session more CPU and memory; it does not let one instance handle more than
  one session.
</Note>
...
Pipecat Cloud runs a few spare instances above the limits you set, even at zero active sessions, so the instance count you can observe is normally higher than both `min-agents` and `max-agents`. For example, a deployment made with `--min-agents 1 --max-agents 1` normally runs 2 instances at all times.
