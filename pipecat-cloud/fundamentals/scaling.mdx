...
import { SpareInstancesNote } from "/snippets/spare-instances-note.mdx";
...
<Note>
  ...
  number of running instances, which includes a few spares (see [Instance count
  vs. configured agents](#instance-count-vs-configured-agents)). Choosing a
  larger compute profile (for example `agent-2x` or `agent-3x`) gives a single
  session more CPU and memory; it does not let one instance handle more than one
  session.
</Note>
...
<Frame caption="Maximum agents is the hard limit on how many sessions your pool can run at once.">
...
If you keep warm instances (`min-agents` set to 1 or higher), part of this buffer is always there, even with zero active sessions. As your traffic increases, our system automatically:
...
Pipecat Cloud runs a few spare instances above the limits you set, so the instance count you can observe is normally higher than both `min-agents` and `max-agents`. For example, a deployment made with `--min-agents 1 --max-agents 1` normally runs 2 instances at all times, with no active sessions.

This applies whenever you keep warm instances. A deployment with `--min-agents 0` still scales down to 0 instances when it is idle (see [Scale-to-zero](#scale-to-zero)); its spare instances appear once it has sessions to run.

Spares protect you from losing an instance. If one runs out of memory, is evicted, or its machine is taken out of service, a warm replacement is already there and your next session does not pay a [cold start](#cold-starts). Pipecat Cloud decides how many spares to keep, and that number can change, so don't build your application around a specific count.

This is why `readyReplicas` in the [agent details](/api-reference/pipecat-cloud/rest-reference/endpoint/agent-list-one) API response, and the instance count in the Pipecat Cloud Dashboard, can read higher than `max-agents`. `readyReplicas: 2` on a `--min-agents 1 --max-agents 1` deployment with no active sessions is expected, not a bug.

Spare instances do **not** change two things:

- **Your concurrent session limit.** `max-agents` is still a hard cap on sessions. With `--max-agents 1`, a second session request is rejected with a `429` (or, for WebSocket connections, close code `1011` with the message "Concurrency limit reached"), no matter how many instances are running.
- **Your bill.** Reserved session minutes are metered from your configured `min-agents`, not from the instance count. Spares are not billed to you.
