---
title: "Pipecat Cloud Scaling"
sidebarTitle: "Scaling"
description: "Scale Pipecat Cloud agent deployments in production: instances, min-agents, and granular controls for compute and cost."
---
...
<Note>
  ...
  Your total concurrency is set by [`max-agents`](#maximum-agents), not by the
  number of instances you see running. Pipecat Cloud keeps a few spare instances
  above the limits you set, so your instance count can read higher than
  `max-agents`. See [Instance count vs. configured
  agents](#instance-count-vs-configured-agents). Choosing a larger compute
...
Deployments can optionally be made with a `max-agents` configuration that limits how many active sessions your pool can run at once.
...
`max-agents` is a hard limit on sessions, meaning requests made to a pool that is at capacity will receive a `429` response. See [Starting Sessions](./active-sessions#agent-capacity) for more information for how to handle this in your application code.

<Note>
  `max-agents` caps **sessions**, not the instance count you can observe.
  Pipecat Cloud keeps a few spare instances running above your limits, so the
  number of instances reported by the API or the dashboard is normally higher
  than `max-agents`. See [Instance count vs. configured
  agents](#instance-count-vs-configured-agents).
</Note>
...
Part of this buffer is always there, even with zero active sessions. As your traffic increases, our system automatically:
...
Pipecat Cloud runs a small number of spare instances above the limits you set. This is true even when you have zero active sessions, so the instance count you can observe is normally higher than both `min-agents` and `max-agents`.

The clearest example is a small pool. A deployment made with `--min-agents 1 --max-agents 1` normally runs 2 instances at all times, including when nothing is happening.

Spare instances protect you from losing an instance. If one runs out of memory, is evicted, or its machine is taken out of service, a replacement is already warm and your next session does not pay a [cold start](#cold-starts). Pipecat Cloud manages how many spares to keep, and that number can change over time, so please don't build your application around a specific count.

This is why the `readyReplicas` value in the [agent details](/api-reference/pipecat-cloud/rest-reference/endpoint/agent-list-one) API response, and the instance count shown in the Pipecat Cloud Dashboard, can read higher than the `max-agents` you configured. Seeing `readyReplicas: 2` on a deployment set to `--min-agents 1 --max-agents 1` with no active sessions is expected, not a bug.

Two things spare instances do **not** change:

- **They do not raise your concurrent session limit.** `max-agents` is still a hard cap on sessions. With `--max-agents 1`, a second session request is still rejected with a `429` (or, for WebSocket connections, a close with code `1011` and the message "Concurrency limit reached"), no matter how many instances are running.
- **They do not change your bill.** Reserved session minutes are measured from the `min-agents` value you configured, not from the instance count. Spare instances are not billed to you.

<Tip>
  To check what your deployment is actually configured to do, read back your
  `min-agents` and `max-agents` values rather than counting instances. The
  instance count includes spares and is not your concurrency limit.
</Tip>
