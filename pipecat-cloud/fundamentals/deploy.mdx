...
In a [self-hosted region](/pipecat-cloud/enterprise/overview), you can instead state CPU and memory explicitly (`--resources cpu=1,memory=2Gi`) or define your organization's own profiles, and choose the CPU architecture the agent runs on. See [Pipecat Enterprise Agent Sizing](/pipecat-cloud/enterprise/sizing).
