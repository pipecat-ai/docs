...
In a [self-hosted region](/enterprise/overview), you can instead state CPU and memory explicitly (`--resources cpu=1,memory=2Gi`) or define your organization's own profiles, and choose the CPU architecture the agent runs on. See [Agent Sizing and Architecture](/enterprise/sizing).
