---
title: "Pipecat Cloud Deployments"
...
For a complete list of deployment configuration options, see the [CLI deploy reference](/api-reference/cli/cloud/deploy).
...
<Tip>
  ...
  documentation](/api-reference/cli/cloud/deploy#configuration-file-pcc-deploy-toml).
</Tip>
...
In a [self-hosted region](/pipecat-cloud/self-hosted/overview), you can instead state CPU and memory explicitly (`--resources cpu=1,memory=2Gi`) or define your organization's own profiles, and choose the CPU architecture the agent runs on. See [Agent Sizing and Architecture](/pipecat-cloud/self-hosted/sizing).
