...
The `websocket_auth` field controls whether the [generic WebSocket endpoint](/pipecat-cloud/guides/telephony/generic-websocket) requires HMAC token authentication. Set to `"none"` for telephony providers that connect directly, or `"token"` (default for new agents) to require authentication. See the [WebSocket Authentication guide](/pipecat-cloud/guides/websocket-authentication) for details.
...
<Tip>
  ...
  documentation](/api-reference/cli/cloud/deploy#configuration-file-pcc-deploytoml).
</Tip>
