---
title: "Pipecat Enterprise Requirements"
...
- **A conforming Kubernetes cluster, version 1.29 or later.** Any distribution works — managed (EKS, GKE, AKS, OKE) or self-managed. The [reference architectures](/pipecat-cloud/enterprise/reference/aws-eks) for AWS EKS and Oracle OKE are proven layouts.
- **cgroup v2 on the nodes that run agents.** Per-session CPU and memory metrics are sampled from the node's cgroup filesystem, which must be the unified (v2) hierarchy — the default on recent distributions (Amazon Linux 2023, Ubuntu 22.04+, RHEL 9+). On cgroup v1 nodes, sessions run normally but resource metrics are unavailable and the dashboard says so; see [Session metrics](/pipecat-cloud/enterprise/logs#session-metrics).
- **Nodes for the architectures you intend to run.** Agents are scheduled on `amd64` or `arm64` nodes according to the architecture each deployment declares. Provide nodes for one or both, and [declare what the region supports](/pipecat-cloud/enterprise/sizing#architecture) when you register it.
...
| Your telephony or WebSocket clients (inbound) | 443  | Only if you [expose a WebSocket endpoint](/pipecat-cloud/enterprise/websockets)                             |
...
Capacity for the agents themselves comes on top of this. Size it from your expected concurrency and the [resources you assign per agent](/pipecat-cloud/enterprise/sizing) — a voice agent typically needs 0.5 vCPU and 1 GiB. A dedicated, autoscaled node pool for agent pods, as in the [EKS reference architecture](/pipecat-cloud/enterprise/reference/aws-eks), keeps platform and agent capacity independent.
...
- **An OpenSearch cluster** for agent logs — only if you [enable the log store](/pipecat-cloud/enterprise/logs).
