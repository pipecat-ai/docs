---
title: "Operating Pipecat Enterprise"
...
helm upgrade pipecat oci://registry.pipecat.daily.co/pcc/pcc-region \
  ...
  -f values-pce-acme-us-east.yaml \
  --set global.regionKey=pce-acme-us-east \
  --wait --timeout 10m
```
...
Running agents are not restarted by an upgrade. Platform changes that affect how agent pods are built (such as [agent pod settings](/pipecat-cloud/enterprise/configuration#agent-pod-settings)) reach each agent on its next deploy.
...
4. Re-create every [referenced secret](/pipecat-cloud/enterprise/secrets#after-rebuilding-a-cluster) in the workloads namespace; Pipecat Cloud still lists them, but they report `pending` until they exist again.
...
1. Delete or move every agent deployed to the region (`pipecat cloud agent delete <name>`), and let any active sessions end. Deleting an agent removes it from the region first and from Pipecat Cloud only once the region confirms, so if the region is unreachable the delete is refused with a retryable error and nothing changes — bring the region back, or [contact Daily](/pipecat-cloud/support) if the cluster is permanently gone.
2. Delete the region:

   ```bash
   pipecat cloud regions delete pce-acme-us-east
   ```
...
Once deleted, the region's connection is cut off within moments and it disappears from region pickers. Registering the same key again later restores the region with its previous settings (any options you pass override them) and counts against your organization's allowance again. It comes back `pending` and needs a new enrollment token.

## GitOps-managed clusters

The package works under Argo CD, Flux, and similar tools with two adjustments:

- Set `workloadsNamespace.create: false` and manage the workloads namespace yourself, with the Pod Security label described in [Requirements](/pipecat-cloud/enterprise/requirements#kubernetes-cluster). The platform adds one management label to it; do not prune that label.
- The enrollment token is consumed once, at first install. Keep it out of your repository: stage it with `kubectl` (or your secrets tooling) before the first sync, exactly as in the [setup guide](/pipecat-cloud/enterprise/setup).

- Supply the broker password as a value (`externalBroker.password`) from your secrets tooling rather than by Secret name: tools that render with `helm template` have no cluster to read a Secret from, and the chart refuses to render in that case. See [Broker](/pipecat-cloud/enterprise/configuration#broker).
- The activation secret needs nothing extra: the default pre-install job runs in Argo CD's pre-sync phase and creates it once. If your pipeline owns all Secrets, see the [supply-it-yourself option](/pipecat-cloud/enterprise/configuration#activation-secret).
