...
With the [log store](/pipecat-cloud/self-hosted/logs) enabled, agent log lines are stored in your region and read **only** when a member of your organization queries them through the CLI, dashboard, or API. Each query returns the lines that match its filters and nothing else; no log data is collected or stored by Pipecat Cloud, and Daily does not pull agent logs on its own. Log collection is off by default.
...
- **Secret values.** [Referenced secrets](/pipecat-cloud/self-hosted/secrets) are checked for existence and type only, and the status response cannot carry keys or values.
