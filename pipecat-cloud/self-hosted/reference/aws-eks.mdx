...
```mermaid
flowchart LR
  internet(("Internet")) -- "wss:// (telephony / WebSocket)" --> alb["ALB + ACM certificate<br/>hostname via external-dns"]
  pcc(("Pipecat Cloud"))

  subgraph vpc["VPC — private subnets, one NAT gateway"]
    subgraph eks["EKS"]
      sys["System node group (amd64, fixed)<br/>platform components · cert-manager<br/>Karpenter · ALB controller · external-dns · monitoring"]
      bots["Agents node pool (Karpenter)<br/>amd64 or arm64 per pod · tainted<br/>scales from zero on session demand"]
    end
    valkey[("ElastiCache Valkey<br/>TLS + auth")]
    os[("OpenSearch (optional)<br/>TLS + auth")]
  end
  ...
  sys --> os
  bots --> valkey
  sys -- "outbound only: API · registry · region endpoint" --> pcc
```

One VPC, one cluster, two kinds of nodes, two managed stores, and an optional WebSocket front door.
