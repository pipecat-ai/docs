...
- **A conforming Kubernetes cluster, version 1.29 or later.** Any distribution works — managed (EKS, GKE, AKS, OKE) or self-managed. The [AWS EKS reference architecture](/pipecat-cloud/self-hosted/aws-eks) is one proven layout.
...
- **Nodes for the architectures you intend to run.** Agents are scheduled on `amd64` or `arm64` nodes according to the architecture each deployment declares. Provide nodes for one or both, and [declare what the region supports](/pipecat-cloud/self-hosted/sizing#architecture) when you register it.
...
| `region-gateway.pipecat.daily.co`             | 8443 | The persistent, mutually authenticated connection between your region and Pipecat Cloud |
...
| Your telephony or WebSocket clients (inbound) | 443  | Only if you [expose a WebSocket endpoint](/pipecat-cloud/self-hosted/websockets)        |
...
Capacity for the agents themselves comes on top of this. Size it from your expected concurrency and the [resources you assign per agent](/pipecat-cloud/self-hosted/sizing) — a voice agent typically needs 0.5 vCPU and 1 GiB. A dedicated, autoscaled node pool for agent pods, as in the [EKS reference architecture](/pipecat-cloud/self-hosted/aws-eks), keeps platform and agent capacity independent.
...
- **An OpenSearch cluster** for agent logs — only if you [enable the log store](/pipecat-cloud/self-hosted/logs).
