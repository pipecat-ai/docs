...
<Frame caption="Management traffic flows over one outbound connection from your cluster. Session traffic goes straight to your data plane and never touches the control plane.">

```
        Your application / telephony provider
             │                           │
             │ POST /start               │ WebSocket / media
             │ (API key)                 │ (wss://ws.your-region.example.com)
             ▼                           │
 ┌──────────────────────────────┐        │
 │  CONTROL PLANE  (Daily)      │        │
 ...
 └──────────────┬───────────────┘        │
                ...
                │ by your cluster        │
 ┌──────────────▼───────────────────────▼──────────────────────────┐
 │  DATA PLANE  (your cluster)                                     │
 │                                                                 │
 │   region agent ──▶ operator ──▶ agent pods (your images)        │
 │        │                            ▲                           │
 │        └────▶ activation service ───┘  ◀── your WebSocket       │
 │               (session starts,           ingress / gateway      │
 │                WebSocket relay)                                 │
 │   autoscaler · broker · [log store]                             │
 └─────────────────────────────────────────────────────────────────┘
```
