...
<Steps>
  ...
  <Step title="Bot instantiates and joins sessuib via specified transport credentials">
...
The majority of use-cases require a way to trigger and manage a bot session over the internet. A bot runner is a HTTP service that provides a gateway for spawning bots on-demand:
...
This pseudo code defines a `/start_bot/` endpoint which listens for incoming user `POST` requests or webhook, then configures the session (such as creating rooms on your transport provider) and instantiates a new bot process.

A client will typically require some information regarding the newly spawned bot, such as a web address, and returns some JSON with the necessary details.

### Data transport

Your [transport layer](/docs/category/transports) is responsible for sending and receiving audio and video data over the internet.

You will have implemented a transport layer as part of your `bot.py` pipeline. This may be a service that you want to host and include in your deployment, or it may be a third-party service waiting for peers to connect (such as [Daily](https://www.daily.co), or a web socket.)
