...
If you need to pass custom parameters to your bot beyond the standard call information (to/from numbers, call_control_id) that Telnyx automatically provides, you'll need to deploy your own `server.py` that acts as a bridge between Telnyx and Pipecat Cloud:

1. **Receives Telnyx webhooks** and extracts custom data from the incoming call
2. **Generates TeXML** with your custom parameters included
3. **Points to Pipecat Cloud's WebSocket URL** (`wss://api.pipecat.daily.co/ws/telnyx`) instead of your own WebSocket endpoint
4. **Passes parameters** through the `serviceHost` parameter and additional custom parameters
...
**Example TeXML with custom parameters:**

```xml
<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Connect>
    <Stream url="wss://api.pipecat.daily.co/ws/telnyx?serviceHost=my-agent.my-org-name&user_id=12345&session_type=support" bidirectionalMode="rtp"></Stream>
  </Connect>
  <Pause length="40"/>
</Response>
```
