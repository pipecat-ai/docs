...
If you need to pass custom parameters to your bot beyond the standard call information (CallUUID, caller details) that Plivo automatically provides, you can modify your XML server to include additional query parameters in the WebSocket URL:

**Example XML with custom parameters:**

```python
xml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
  <Stream bidirectional="true" keepCallAlive="true" contentType="audio/x-mulaw;rate=8000">
    wss://api.pipecat.daily.co/ws/plivo?serviceHost={agent}.{org}&user_id={user_id}&session_type=support
  </Stream>
</Response>"""
```

Your bot can then access these parameters through Pipecat's WebSocket parsing utilities. This approach allows you to inject custom data while leveraging Pipecat Cloud's managed bot infrastructure.
