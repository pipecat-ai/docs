...
If your agent code lives in a subdirectory (e.g., `/bot`), modify the workflow to set the working directory and build context:
...
The `paths` filter ensures the workflow only runs when files in the `bot/` directory change, preventing unnecessary builds.
...
<Note>
  As of 2025-10-30, GitHub does not provide ARM runners for private
