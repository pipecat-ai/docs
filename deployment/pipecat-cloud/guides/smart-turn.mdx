...
To enable Smart Turn Detection in your Pipecat Cloud bot, add the `LocalSmartTurnAnalyzerV3` to your transport configuration.

Set the environment variable `SMART_TURN_ONNX_PATH` to the path of the Smart Turn v3 ONNX file containing the model weights. This can be downloaded from the "Model weights" link above.
...
# Path to the local Smart Turn model
smart_turn_model_path = os.getenv("SMART_TURN_ONNX_PATH")
