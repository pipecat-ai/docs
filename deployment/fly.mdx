...
We mentioned [before](http://localhost:3000/deployment/pattern) that you would ideally containerize the `bot_runner.py` web service and the `bot.py` separately. To keep this example simple, we'll use the same image for both services.
