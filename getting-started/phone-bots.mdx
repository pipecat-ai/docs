...
This quickstart uses three AI services working together in a pipeline and the Twilio as a telephony provider. You'll need API keys from each service:
...
```bash
uv run bot.py --transport twilio --proxy your-ngrok-url.ngrok.io
```
...
This example uses Pipecat's [development runner](/server/utilities/runner/guide) for local testing. The runner automatically handles server setup and connection management.
...
## More Twilio Calling Demos

See complete, production ready starts for Twilio—both for inbound and outbound calling:

<CardGroup cols={2}>
  <Card
    title="Twilio Inbound Calling"
    icon="phone-arrow-down-left"
    href="https://github.com/pipecat-ai/pipecat-examples/tree/main/twilio-chatbot/inbound"
  >
    Complete Twilio websocket inbound calling example
  </Card>
  ...
  >
    Complete Twilio websocket outbound calling example
  </Card>
</CardGroup>
