...
Since Pipecat is a licensed service, we can't directly provide you with the SDK or models. To gain access, you need to sign up for a Krisp developer account. Once you've done that, we've created a Pipecat Krisp module to help you build the module so you can integrated with with your application. This guide will walk you through setting that up.
...
The Pipecat Krips module is a python wrapper around Krisp's C++ SDK. To build the module, you'll need to install the following dependencies:
...
Once the build is complete, you can confirm the module is installed by running and example. To do that, first install the example's dependencies:
...
If you're running on macOS you may receive a security warning about running the script. This is expected. You can allow access by going to `System Preferences > Privacy & Security` then click `Allow Anyway` to permit the example to run.

<Frame>![Security & Privacy](/images/allow-krisp.png)</Frame>

After allowing, you may get a pop-up asking for permission. Select `Open Anyway` to run the script.
...
- Run the prepare to dist script:
...
<Note>
  ...
  for the account you registered on the real PyPI. In this case, you don’t need
