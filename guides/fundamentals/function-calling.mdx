...
# Function handler with FunctionCallParams
...
1. Receive a `FunctionCallParams` object
2. Extract needed arguments from `params.arguments`
3. Process data or call external services
4. Return results via `params.result_callback(result)`
...
async def query_database(params: FunctionCallParams):
    # Query database
    results = await db.query(params.arguments.get("query", ""))
