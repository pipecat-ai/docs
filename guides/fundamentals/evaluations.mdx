...
- **Production monitoring** — Continuous evaluation of live calls so you can catch regressions, track quality over time, and close the loop between what you test and what users experience.

Together, these form a feedback loop: simulations validate changes before deployment, and production monitoring surfaces issues that inform your next round of tests.

## Bluejay

[Bluejay](https://getbluejay.ai) is an evaluation platform purpose-built for voice AI agents. It provides no-code simulation testing and production call monitoring that integrate directly with Pipecat — whether you're running on Pipecat Cloud or self-hosting.
...
<CardGroup cols={2}>
  <Card title="API Integration" icon="key" iconType="duotone">
  ...
  <Card title="Telephony Integration" icon="phone" iconType="duotone">
...
Simulations cover pre-deployment testing, but production monitoring ensures your agent maintains quality with real users. Bluejay's [Evaluate API](https://docs.getbluejay.ai/api-reference/endpoint/evaluate) lets you send production call data for automated evaluation.

```python
import requests

# After a production call ends, send it to Bluejay for evaluation
response = requests.post(
    "https://api.getbluejay.ai/evaluate",
    headers={"Authorization": "Bearer <your-bluejay-api-key>"},
    json={
        "transcript": transcript_data,
        "metadata": {
            "agent_id": "my-pipecat-agent",
...
<Tip>
  ...
  single dashboard for every dimension of agent quality — no need to stitch
