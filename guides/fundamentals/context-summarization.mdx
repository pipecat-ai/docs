...
- **System messages**: If a system message exists in the context, the first one is always kept. When using [`system_instruction`](/guides/learn/context-management#system-instruction-vs-context-system-messages) in LLM Settings instead, the system prompt is not part of the context messages and is automatically prepended by the service on each request, so there is nothing to preserve in the context.
