...
To implement a graceful shutdown, push an `EndTaskFrame` upstream in your pipeline:
...
1. Your processor pushes an `EndTaskFrame()` upstream
2. The frame reaches the source of the `PipelineTask`
3. The task then pushes an `EndFrame()` downstream
4. `EndFrame`s are queued, so they'll process after any pending frames (like goodbye messages)
5. All processors in the pipeline will shutdown when processing the `EndFrame`
6. Once the `EndFrame` reaches the sink of the `PipelineTask`, the Pipeline is ready to shut down
7. The Pipecat processor terminates and related resources are released
...
To implement an immediate shutdown, use event handlers to detect disconnection and push a `CancelFrame`:
