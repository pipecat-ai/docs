...
4. Copy env.example to .env and add configure:
...
`server.py` is a FastAPI server that creates the meeting room where clients and bots interact, manages bot instances, and handles client connections. It's the orchestrator that brings everything together.
...
<Info>
  The Gemini Multimodal Live service requires specific audio settings: input at
  16khz and output at 24khz.
</Info>
...
transport = DailyTransport(
    ...
    "Chatbot",
    DailyParams(
        audio_in_sample_rate=16000,   # Required for Gemini
        audio_out_sample_rate=24000,  # Required for speech synthesis
...
<CardGroup cols={2}>
  <Card title="RTVI Processors" icon="message-bot">
    ...
    `RTVITranscriptionProcessor`: Handles transcription events
    `RTVIMetricsProcessor`: Tracks performance metrics
  </Card>
...
<Tip>
  The order of processors is crucial! For example, transcription processors
  should come after the LLM to capture the processed speech.
...
Our React client uses the [Pipecat React SDK](/client/reference/react/introduction) to communicate with the bot. Let's explore how the client connects and interacts with our Pipecat server.
...
<Tip>
  ...
  more](/client/reference/js/callbacks#callbacks) in the Pipecat client docs.
</Tip>
...
<Warning>
Common Issues:
- Ensure audio sample rates match Gemini's requirements (16kHz
input, 24kHz output)
