...
3. Provide immediate feedback before execution (using `start_callback`)
...
- May have a `start_callback` to provide immediate feedback
...
# Node function configuration
{
    ...
    "function": {
        ...
        "start_callback": before_select_size,  # Optional: Provides immediate feedback
...
- May execute a `start_callback` before processing begins
...
# Edge function configuration
{
    ...
    "function": {
        ...
        "start_callback": before_api_call,  # Optional: Provide feedback before execution
...
- Execute logic upon a function call's initiation

```python
# Define a start callback
async def before_check_weather(function_name: str, flow_manager: FlowManager) -> None:
    """Tell the user we're checking the weather."""
    await flow_manager.llm.push_frame(TTSSpeakFrame("Let me check the weather for you..."))
...
# Use in function configuration
{
    ...
    "function": {
        "name": "check_weather",
        "handler": check_weather_handler,
        "start_callback": before_check_weather,  # Add start callback
...
- `handler`: Process data, stay in current state
- `transition_to` (static flow) or `transition_callback` (dynamic flow): Pure transition to next state
- `start_callback`: Provide immediate feedback before execution
- Multiple properties: Combine behaviors (e.g., feedback → process data → transition)
