...
For dynamic flows, register transition handlers for specific functions:

```python
# Define transition handlers
async def handle_availability_check(args: Dict, flow_manager: FlowManager):
    """Handle transition after availability check."""
    if args["available"]:
...
# Initialize with per-function transitions
flow_manager = FlowManager(
    ...
    transition_callbacks={
        "check_availability": handle_availability_check
    }
)
```
...
2. Create transitions (using `transition_to` for static flows or registered transition handlers for dynamic flows)
...
- Do not have transitions registered
...
- Must have either transition_to (static flows) or a registered transition handler (dynamic flows)
...
- No transitions: Node function - process data with LLM completion
- Has transition: Edge function - process data (if handler provided) and transition to next node
...
# Define transition handlers
async def handle_age_collection(args: Dict[str, Any], flow_manager: FlowManager):
    """Handle age collection transition."""
    if args["age"] < 25:
...
# Initialize flow manager with transition handlers
flow_manager = FlowManager(
    task=task,
    llm=llm,
    context_aggregator=context_aggregator,
