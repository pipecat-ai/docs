...
The `state` variable in FlowManager is a shared dictionary that persists throughout the conversation. It serves two main purposes:

1. Automatically storing function results
2. Manual storage of conversation data

### Function Results in State

When a function handler returns a result, it's automatically stored in `flow_manager.state` using the pattern `{function_name}_result`. This is especially useful for making routing decisions:

```python
# 1. Function handler returns a result
async def check_availability(args: FlowArgs) -> TimeResult:
    """Check if a time slot is available."""
    is_available = await check_third_party_api(args["time"])

    # Result is automatically stored as "check_availability_result"
    return TimeResult(
        status="success",
...
# 2. Transition handler accesses the result
async def handle_availability_check(args: Dict, flow_manager: FlowManager):
    ...
    # Use result to determine next node
    if result["available"]:
...
You can also manually store and access data in the state dictionary. This is useful for:

- Building up information across multiple interactions
- Sharing data between nodes
- Tracking conversation progress

Here's an example pizza ordering flow that builds an order over multiple steps:
...
async def select_toppings(args: FlowArgs) -> FlowResult:
    ...
    # Access and update existing state
...
async def finalize_order(args: FlowArgs) -> FlowResult:
    ...
    # Use collected state to complete order
    order = flow_manager.state.get("order", {})
...
- Automatic storage of function results for routing decisions
- Manual storage for building up complex data structures
- Persistence across the entire conversation flow
