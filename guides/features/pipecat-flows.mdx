...
- Acknowledge user requests before processing
- Add wait messages for API calls or database operations

```python
# Define a start callback
async def before_check_weather(function_name: str, llm: Any, context: Any) -> None:
    """Tell the user we're checking the weather."""
    await llm.push_frame(TTSSpeakFrame("Let me check the weather for you..."))
