...
The system prompt is typically set via `system_instruction` in the LLM service's [Settings](/guides/fundamentals/service-settings), not as a message in the context. See [System Instruction vs Context System Messages](#system-instruction-vs-context-system-messages) below for details.
